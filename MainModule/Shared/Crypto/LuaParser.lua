--// Lua Parser
--// https://github.com/Rerumu/LuaON
--// Modified by trzistan
...
local Properties = setmetatable({}, {
	...
	__call = function(self, Instance, Property) -- Much neater to have both things here.
		...
		if Found then return Instance[Property] end
	end,
...
local PropertiesStored = { -- Was too lazy to list so you have fun.
	"ClassName",
	"Name", -- Name of properties, for obvious reasons you will always need a ClassName property (creating instance).
}
...
function Extract(Data)
	...
	elseif Mem["^false$"] then
		return false
		--elseif ValT == "Enum" then
		--	Val	= Concat{'Enum[', tostring(Val), ']'};
		--elseif ValT == "EnumItem" then
		--	Val	= Concat{'EnumItem[', Val.EnumType, '.', Val.Name, ']'};
	elseif Mem["^Enum%[(.+)%]$"] then
		return Enum[Mem[2]]
	elseif Mem["^EnumItem%[(.+)]$"] then
		local Key, Item = Match(Mem[2], "(.*)%.(.+)")
		return Enum[Key][Item]
	elseif Mem["^I(%[.-%])$"] then
...
function Encode(Table, Buff)
	...
	for Index, Value in next, Table do
		...
		if Type(Index) == "string" then Idx = Concat { '"', SafeString(Index, true), '":' } end
		...
		elseif ValT == "Enum" then
			Val = Concat { "Enum[", tostring(Value), "]" }
		elseif ValT == "EnumItem" then
			Val = Concat { "EnumItem[", tostring(Value.EnumType), ".", Value.Name, "]" }
		elseif ValT == "Instance" then
			...
			if #Child ~= 0 then Props.Children = Child end
