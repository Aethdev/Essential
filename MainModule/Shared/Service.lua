local realMethods = {}
local checkMethod; checkMethod = function(ins, meth, ...)
...
return function(specificTab, errHandler, Promise)
	...
	local delay = delay
	local assert = assert
	local corotCreate, corotResume, corotYield = coroutine.create, coroutine.resume,
		coroutine.yield
	...
	local function generatePassphrases(wordCount: number, doCamelCase: boolean?)
		local realWCount = (type(wordCount)=="number" and wordCount > 0 and math.floor(wordCount)) or math.random(8,10)
		local listOfWords = {
			"moon"; "cake"; "sun"; "frappe"; "cow"; "dog"; "hotel"; "cat"; "rust"; "elephant"; "moose";
			"noob"; "pro"; "warrior"; "zebra"; "vase"; "van"; "car"; "queen"; "king"; "hen"; "bus"; "nectar";
			"sunny"; "sea"; "ocean"; "waves"; "potion"; "lamp"; "lamb"; "goat"; "gallop"; "chocolate"; "euphoria";
			"terrain"; "valley"; "mountain"; "grass"; "water"; "fire"; "air"; "ground"; "dirt"; "race"; "ace"; "obby";
			"parkour"; "cold"; "hot"; "snake"; "ram"; "trace"; "train"; "aqua"; "kangaraoo"; "party"; "dance"; "drums";
			"flute"; "guitar"; "piano";
		};
		local Res = {};
		...
		for i = 1, realWCount do
			...
			if i > 1 and doCamelCase then
				chosenWord = chosenWord:sub(1):upper()..chosenWord:sub(2)
			end
	...
	local function trimString(str: string)
	...
	local function cloneTable(tab, ignoreList, errIfCircular, applyMetatable)
		...
		assert(not applyMetatable or (hasTheGivenType(tabMetatable, "nil") or hasTheGivenType(tabMetatable, "table")), "Table metatable isn't valid (expected nil/table)")
		
		local doubleClone = {}
		local doubleItem = {}
		local cacheItem = {}
		local isValueNil = function(value) return type(value) == "nil" end
		local clone; clone = function(cloneTarget)
		...
		if not table.find(ignoreList, tab) then
			table.insert(ignoreList, tab)
		end
	...
	local tasks, events, wrappers, misc = {
					...
					if isThread and taskTab.Active and taskTab.Running then
						...
						if coroutine.status(taskFunc) == 'normal' then
							repeat
								...
								task.wait()
							until
								coroutine.status(taskFunc)=='dead' or coroutine.status(taskFunc) == 'suspended'
						end
		...
		trackTask = function(name, isThread, func, ...)
			local parameters = {...}
			local newTask; newTask = service.createTask("_TTASK-"..tostring(name or func).."-"..getRandom(), isThread, function()
			...
			if isThread then
				...
				if not threadRets[1] then
					threadRets[3] = select(1, debug.traceback(newTask.Function))
				end
				...
				local funcRets = {xpcall(function()
		...
		triggerTask = function(name, isThread, func)
			return function(...)
				...
				if not rets[1] then
					warn(`TrackTask {tostring(name)} encountered an error: {tostring(rets[2])}\n{tostring(rets[3])}`)
				end
		...
		threadTask = function(func, ...)
			return service.trackTask(nil, true, func, ...)
		end;

		nonThreadTask = function(func, ...)
			return service.trackTask(nil, false, func, ...)
		end;

		loopTask = function(name, delay, func, ...)
			local runService = service.RunService
			local taskIndex = tostring(tick()+math.random()^3+math.random())
			local loopInfo; loopInfo = {
				index = taskIndex;
				name = name;
				active = false;
				created = osTime();
			...
			local function manageRegistry(typ)
				...
				elseif typ == "UnRegister" then
					if onGoingLoops[taskIndex]==loopInfo then
						onGoingLoops[taskIndex] = nil
					end
					if loopInfo.task then
						task.defer(task.cancel, loopInfo.task)
					end
				end
			end

			local function checkLoop()
				return onGoingLoops[taskIndex] == loopInfo and loopInfo.active
			end
			
			local function oneTimeEvent(event: RBXScriptSignal, func): RBXScriptConnection
				local eventCon; eventCon = event:Connect(function(...)
		...
		processQueue = function(name)
			for i,queue in pairs(queues) do
				...
				if queue.Name == name and not isProcessing then
					service.trackTask("__QUEUEPROCESSING-"..queue.Name, true, queue.Process) 
				end
			end
		end,
		...
		addQueue = function(name, ...)
			...
			if queueDirectory then
				...
				if not queueDirectory.__Processing then
					service.trackTask("_QUEUE_RUN_PROCESSOR-"..queueDirectory.Name, true, queueDirectory.Process)
				end
		...
		waitQueue = function(name, ...)
			...
			if queue then
				return queue.Returned.Event:Wait()
			end
		end;
		...
		rbxEvent = function(event, func, noPcall)
			...
			local eventCon = connectEvent(unWrap(event), function(...)
				if noPcall then
					...
					if not suc then
						warn("Event "..tostring(eventName).." function encountered an error: "..tostring(ers))
					end
				end
			end)
		...
		selfEvent = function(event, func)
			local eventCon; eventCon = service.rbxEvent(event, function(...)
				return func(eventCon, ...)
			end)
		...
		metaFunc = function(func, noPcall)
			if type(func) == "function" then
				return service.newProxy{__call = function(self, ...)
					if noPcall then
						return func(...)
					else
				...
				end; __tostring = function() return "Essential-"..getRandom() end;}
			else
				return func
			end
		end,
		...
		metaTable = function(tab, name)
			return service.newProxy{__index = function(self, ind)
				return tab[ind]
			end; __iter = function(self) return pairs, tab end; __tostring = function() return name or "Essential-"..getRandom() end;}
		end,
		...
		symbolicTable = function(tab, tabMetatable, isReadOnly)
			local symbolicMeta = {
				__index = function(self, ind) local val = tab[ind] if not isReadOnly or type(val) ~= "table" then return val else return service.tableRead(val) end end;
				__newindex = function(self, ind, val) if isReadOnly then error(`Symbolic Table is read only`, 0) return end tab[ind] = val end;
			...
			if tabMetatable then
				if tabMetatable.__unm then
					symbolicMeta.__unm = function(self)
						return tabMetatable.__unm[symbolicMeta](tab)
					end
				end
				
				if tabMetatable.__iter then
					symbolicMeta.__iter = tabMetatable.__iter
				end
			end
		...
		wrap = function(item, fullWrap)
			...
			elseif typeof(item) == "Instance" or typeof(item) == "RBXScriptSignal" or typeof(item) == "RBXScriptConnection" then
				...
				local proxData; proxData = {
					...
					SetProxy = function(self, prox, val)
						proxData[prox] = val
					end;
					...
					Disconnect = function(self)
						if itemConnection and item.Disconnect then
							item:Disconnect()
						end
					end,
					...
					Connect = function(self, func)
						if itemSignal then
							return wrap(connectEvent(item, function(...)
								return func(unpack(wrap{...}))
							end))
						end
					end,
					
					IsA = function(self, className)
						return objIsA(item, className)
					end;

					Wait = function(self, ...)
						return wrap(item.Wait)(item, ...)
					end,

					ChildAdded = function(self, func)
						local event = wrap(connectEvent(childAdded, function(...)
							return func(unpack(wrap{...}))
						end))
					...
					ChildRemoved = function(self, func)
						local event = wrap(connectEvent(childRem, function(...)
							return func(unpack(wrap{...}))
						end))
					...
					DescendantAdded = function(self, func)
						local event = wrap(connectEvent(descAdded, function(...)
							return func(unpack(wrap{...}))
						end))
					...
					DescendantRemoving = function(self, func)
						local event = wrap(connectEvent(descRem, function(...)
							return func(unpack(wrap{...}))
						end))
				...
				proxMeta.__index = function(self, ind)
					...
					elseif type(selection) == "function" then
						return function(ignore, ...)
							return unpack(wrap{checkMethod(item, ind, unpack(unWrap{...}))})
						end
					else
						return wrap(selection, fullWrap)
					end
				...
				proxMeta.__newindex = function(self, ind, val)
					item[ind] = unWrap(val)
				end

				proxMeta.__eq = rawequal
				proxMeta.__gc = function()
					if wrappedItems[item] then
						wrappedItems[item] = nil
					end
				end
					...
					__index = function(_, ind)
						...
						elseif type(selected) == "function" then
							return service.metaFunc(function(ignore, ...)
								return selected(item, unpack(unWrap{...}))
							end)
		...
		setSpecific = function(ind, val)
			if not specificLocked then
				rawset(serviceSpecific, ind, val)
			end
		end;

		removeSpecific = function(ind, val)
			if not specificLocked then
				rawset(serviceSpecific, ind, nil)
			end
		end;
		
		isTableCircular = function(...)
			return isTableCircular(...)
		end;
		...
		playerNameFromId = function(num: number)
			...
			if existCache and (os.time()-existCache.updated < 300) then
				...
				local suc,foundName = pcall(function()
					return service.Players:GetNameFromUserIdAsync(num)
				end)
				
				if suc and foundName then
					foundName = foundName:lower()
					if not table.find(existCache.possibleNames, foundName) then
						table.insert(existCache.possibleNames, foundName)
					end
					
					if userIdCache[foundName] then
						userIdCache[foundName] = nil
					end
		...
		playerIdFromName = function(str)
			...
			if existingCache and (os.time()-existingCache.updated < 300 or (existingCache.temporary and not existingCache.finished)) then
				local suc, ers = pcall(function()
					return service.Players:GetUserIdFromNameAsync(str)
				end)
				...
				if suc and ers then
					...
					if userIdCache[ers] then
						if not table.find(userIdCache[ers].possibleNames, str:lower()) then
							table.insert(userIdCache[ers].possibleNames, str:lower())
						end
					else
		...
		checkPassOwnership = function(userId, gamepassId)
			...
			elseif (currentCache and (os.time()-currentCache.lastUpdated > 120)) or not currentCache then
				...
				local suc,ers = pcall(function()
					return service.Marketplace:UserOwnsGamePassAsync(userId, gamepassId)
				end)
		...
		checkAssetOwnership = function(player, assetId)
			...
			elseif (currentCache and (os.time()-currentCache.lastUpdated > 120)) or not currentCache then
				...
				local suc,ers = pcall(function()
					return service.Marketplace:PlayerOwnsAsset(player, assetId)
				end)
		...
		insertAsset = function(assetId, bundleAssets)
			...
			if canUpdate then
				...
				local suc,actualAsset = pcall(function()
					return service.InsertService:LoadAsset(assetId)
				end)
			
				if suc and actualAsset then
					assetCache.contents = {unpack(actualAsset:GetChildren())}
				end
		...
		tableMatch = function(tab1,tab2)
			local function doCheck(t1,t2)
				...
				for i,v in pairs(t1) do
					...
					if firstTabValueType ~= secTabValueType then
						match = false
					end
		...
		getAttribute = function(obj, attribute)
			return getAttribute(obj, attribute)
		end;
		...
		safeFunction = function(func)
			...
			return function(...)
				return unpack({corotResume(thread, ...)}, 2)
			end
		end;
		...
		getPlayers = function()
			...
			for i,plr in pairs(service.Players:GetPlayers()) do
				if rawequal(typeof(plr), "Instance") and objIsA(plr, "Player") then
		...
		convertUserIdsToPlayers = function(userIdList)
			...
			for i, userid in pairs(userIdList) do
				if type(userid) == "number" and userid > 0 then
					...
					if plr and not table.find(tab, plr) then
						table.insert(tab, plr)
					end
				end
			end
		...
		roundNumber = function(number, increment)
		...
		immutable = function(...)
			...
			local coRot = rotWrap(function(...) while true do rotYield(...) end end, ...)
			coRot(...)
			return coRot
		end,
		...
		immutableEvent = function(event, func)
			...
			loopTask(eventLoopInd, true, 0.1, function()
				if eventInfo.active then
					if eventInfo.connection then
						nonThreadTask(discRbxEvent, eventInfo.connection)
					end
	...
	service = setmetatable({
		...
		Delete = function(obj, delay, deleteFunc)
			...
			if typeof(obj) == "userdata" then
				task.delay(delay or 0, function() if obj.Destroy then pcall(obj.Destroy, obj) end end)
			else
			...
			if deleteFunc and type(deleteFunc) == "function" then
				...
				local parCheck; parCheck = connectEvent(obj:GetPropertyChangedSignal"Parent", function(par)
					local parent = obj.Parent

					if not checking and parent == nil then
						checking = true
						if parCheck.Disconnect then
							parCheck:Disconnect()
						end
						...
						if not suc then
							if errHandler then
								errHandler("DeleteFunc Failed", obj, ers, osTime())
							end
						end
					end
				end)
		...
		Routine = function(func, ...)
			return corotResume(coroutine.create(func), ...)
		end;
	},{
		__index = function(self, ind) -- Used to index game services (i.g. ServerStorage) and specific table
			local match = serviceSpecific[ind] or tasks[ind] or events[ind] or wrappers[ind] or misc[ind] or cacheServices[ind]
	...
	if runService:IsServer() then
		service.Marketplace.PromptPurchaseFinished:Connect(function(player: Player, assetId: number, wasPurchased: boolean)
		...
		service.Marketplace.PromptGamePassPurchaseFinished:Connect(function(player: Player, gamepassId: number, wasPurchased: boolean)
			if not wasPurchased then return end
		...
		service.Marketplace.PromptSubscriptionPurchaseFinished:Connect(function(player: Player, subscriptionId: number, didTryPurchasing: boolean)
