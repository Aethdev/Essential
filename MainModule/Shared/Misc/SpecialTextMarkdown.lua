return table.freeze {
	{
		"Timestamp",
		{ "t:(%d+)", "t:(%d+):(%a+)" },
		function(textMatches: { [number]: string }, textSettings: { startedSince: number }, parser)
			local timestampOption = if textMatches[2] == "st"
				then "shorttime"
			...
			if textSettings.richText then return `<font color='#0088f0'>{text}</font>` end
...

	{
		...
		function(textMatches: { [number]: string }, textSettings: { startedSince: number }, parser)
			...
			if customReplacements then return tostring(customReplacements[textMatches[1]] or "-unknown-") end
