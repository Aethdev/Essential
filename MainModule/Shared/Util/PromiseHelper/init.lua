...
PromiseHelper.__index = function(_, index)
	...
	if Typeof == "function" then
		return function(...) return _promiseIndexed(...) end
	end
...
function PromiseHelper.new(executor, promise): PromiseHelper
			...
			if _promiseIndexed ~= nil then
				...
				if Typeof == "function" then
					return function(_self, ...)
						assert(
							rawequal(_self, self) or rawequal(_self, _promise),
							`Call method {index} does not belong to {_self}`
						)
						local promiseOrValue = _promiseIndexed(_promise, ...)
						if Promise.is(promiseOrValue) then return PromiseHelper.new(nil, promiseOrValue) end
	...
	_promise = promise
		or Promise.new(function(resolve, reject, onCancel)
			onCancel(function() self:_destroy() end)
...
function PromiseHelper.promisify(func)
	...
	return function(...) return PromiseHelper.new(nil, promisified(...)) end
end

function PromiseHelper.rawPromisify(func) return Promise.promisify(func) end

function PromiseHelper.wrapify(promise) return PromiseHelper.new(nil, promise) end

function PromiseHelper.prototype:unWrap() return self._parent end

function PromiseHelper.prototype:hook(executor: (Promise) -> any)
	local parentPromise = self._parent or self
	return self:_andThen(debug.traceback(nil, 2), function()
		return PromiseHelper.new(
			function(resolve, reject, onCancel) executor(parentPromise, resolve, reject, onCancel) end
		)
	end)
end
		...
		return Promise.new(function(resolve, reject, onCancel)
			...
			for i, signal in signals do
				signalPromises[i] = PromiseHelper.new(function(resolve, reject, onCancel)
					...
					if onCancel(function() signalLink:disconnect() end) then
				...
				end):andThen(finish(resolve), finish(reject))
			end

			--warn("did create signal promises?")
			if timeoutDelay then
				timeoutTask = Promise.delay(timeoutDelay):andThen(function()
