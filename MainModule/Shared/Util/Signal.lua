...
local corotYield, corotResume, corotRunning = coroutine.yield, coroutine.resume,
	coroutine.running
...
local realWarn = warn
local function warn(...)
	realWarn(`:: ESS Signal {(isServer and "Server") or "Client"} ::`, ...)	
end

local wait = function(...): number
	return (Signal.customWait or realWait)(...)
end
local cloneTable = function(tab: {[any]: any}): {[any]: any}
	local clone; clone = function(val: any)
...
local function makeOneTimeRbxConnection(rbxScriptSignal: RBXScriptSignal, func: () -> any): RBXScriptConnection
	local connection; connection = rbxScriptSignal:Connect(function(...)
...
function Signal.is(anything: any): boolean
	return getmetatable(anything) == Signal
end
...
function Signal:linkSignal(otherSignal: {[any]: any}): {[any]: any}
	if self.active and otherSignal.active then
		...
		if existingSignalLink then
			return existingSignalLink
		end
		
		local signalLink = otherSignal:connect(function(...)
			self:fire(...)
		end)
		...
		if signalLink ~= "cannot_connect" then
			...
			if not otherSignal.__wrapped then
				table.insert(otherSignal.otherSignalLinks, signalLink)
			end
		end
...
function Signal:unlinkSignal(otherSignal: {[any]: any})
	if self.active then
		for i, otherSignalLink in otherSignal.otherSignalLinks do
			if rawequal(otherSignalLink._linkedSignal, self) then
				otherSignalLink:disconnect()
			end
		end
	end
end
...
function Signal:getSignalLinkWithSignal(otherSignal)
	if self.active then
		...
		for i, signalLink in self.signalLinks do
			if signalLink._otherSignalId == otherSignalId then
				return signalLink
			end
		end
		
		for i, otherLink in self.disconnectOnLinks do
			if otherLink.signalId == otherSignal.id then
				return otherLink
			end
		end
	end
...
function Signal:stopSignalLinks(): boolean
	if self.active then
		for i,otherSignalLink in self.signalLinks do
			if otherSignalLink.Disconnect then
				otherSignalLink:Disconnect()
			end
	...
	if self.active then
		local function doDisconnect()
			if not self.disconnecting then
				...
				if self.unlinkFunc then
					task.defer(self.unlinkFunc, self)
				end
			end
		end
...
function Signal:disconnectOn(otherSignal)
	...
	assert(not self:getSignalLinkWithSignal(otherSignal), `The provided signal has an existing signal link to this signal`)
	...
	local disconnectLink = otherSignal:connectOnce(function()
		self:disconnect()
	end)
...
function Signal:connect(func: Function, deferred: boolean?): {[any]: any}
	if not rawequal(type(func), "function") then
	...
	if self.active then
		...
		function link:disconnect(delay)
			if self.active and not self.disconnecting then
				...
				if delay and delay > 0 and runService:IsRunning() then
					wait(delay)
				end
				
				if self._linkedSignal then
					self._linkedSignal:removeSignalLink(self)
				end
					...
					taskSpawn(function()
						...
						if not suc then
							...
							if signal.debugError or link.debugError then
								warn("Signal "..signal.id.." link "..self.id.." encountered an error while running:", ret, debugTrace)
							end
				...
				if foundLink then
					table.remove(signal.links, foundLink)
				end
...
function Signal:deferConnect(func: () -> any)
	return Signal:connect(func, true)
end
...
function Signal:selfConnect(func: FunctionalTest): {[any]: any}
	...
	if self.active then
		local eventCon; eventCon = self:connect(function(...)
...
function Signal:connectOnce(func: FunctionalTest): {[any]: any}
	if self.active then
		...
		local link; link = self:connect(function(...)
			if link.active then
				link:disconnect()
...
function Signal:fire(...)
	return self:_fire(false, ...)
end
Signal.Fire = Signal.fire

function Signal:deferFire(...)
	return self:_fire(true, ...)
end
...
function Signal:wait(delay: number?, idleTimeout: number?): any
	...
	if self.active then
		...
		local error
		local event = self:connectOnce(function(...)
			fired = {...}
		end)
		...
		until
			 not runService:IsRunning() or not(event.active and self.active) or fired~=nil or (idleTimeout and os.clock()-started > idleTimeout)
...
function Signal:waitOnThread(delay: number?, idleTimeout: number?): any
	...
	if self.active then
		...
		local cleanUp;
		local eventLink; eventLink = self:connectOnce(function(...)
			if self.debug then warn("Event called") end
			if cleanUp then taskSpawn(cleanUp) end
			if corotStatus(corotThread) == "suspended" then
				taskSpawn(corotThread, ...)
			end
		end)
		...
		local idleTimeoutThread: thread? = if idleTimeout and idleTimeout > 0 then taskDelay(idleTimeout, function()
		...
		cleanUp = function()
			if idleTimeoutThread and corotStatus(idleTimeoutThread) == "suspended" then task.cancel(idleTimeoutThread) end
		...
		if eventsCount > 0 then
			...
			for i,event in events do
				...
				if (eventType == "Signal" or eventType == "SignalWrap") then -- Essential signals
					table.insert(eventCons, event:connectOnce(function(...)
			...
			repeat
				wait()
			until
				not runService:IsRunning() or (idleTimeout and os.time()-stWait > idleTimeout) or (#results == eventsCount) or not self.active
	...
	if rawequal(Signal, self) then
		...
		if eventsCount > 0 then
			...
			repeat
				wait()
			until
				not runService:IsRunning() or (idleTimeout and os.time()-stWait > idleTimeout) or readyToReturn
		...
		local function doCallback(...)
			task.defer(function(...)
				if not readyToPass then
					...
					if waitCallbackData then
						waitCallbackData:delete()
					end
					
					-- Disconnect events after completion or idle timeout
					processData:stopEvents()

					if callback and processData.active then
						taskSpawn(callback, ...)
					end
				end
			end, ...)
...
function Signal:wrapConnect(): {[any]: any}
	if self.active then
		local selfSignal = self
		local wrap; wrap = setmetatable({},{
			__index = function(this, ind)
				if rawequal(ind, "__wrapped") then return true end
				
				local chosen = (rawequal(ind, 'connect') and selfSignal.connect) or
					(rawequal(ind, 'Connect') and selfSignal.connect) or (rawequal(ind, 'connectOnce') and selfSignal.connectOnce) or
					(rawequal(ind, 'ConnectOnce') and selfSignal.connectOnce) or (rawequal(ind, 'selfConnect') and selfSignal.selfConnect) or
					(rawequal(ind, 'SelfConnect') and selfSignal.selfConnect)
