...
if game:GetService("RunService"):IsServer() then game:BindToClose(function() serverRunning = false end) end
...
local isStudio = function() return game:GetService("RunService"):IsStudio() end
local realWait = wait
local wait = function(...)
	if ({ ... })[1] and ({ ... })[1] < 0 then return 0, 0 end
	if not serverRunning then return 0, 0 end
...
local function selfEvent(eventSignal, func)
	local eventCon
	eventCon = eventSignal:connect(function(...)
...
function Queue.new(processFunc: FunctionalTest?): { [any]: any }
	...
	self._queue = {}
	self._crossQueues = {}
	self.processFunc = processFunc or function() return true end
	...
	if not Queue.warnNoCustomEvent and not Queue.customEvent then
		...
		warn(
			"Queue doesn't have a custom event. It is recommended to have one for processing ques with metadata.\n-\n"
...
function Queue:warn(...)
	if self.active then warn("Queue " .. tostring(self.id), ...) end
end

function Queue:addWithPriority(priorityLevel, ...)
	local queData = self:add(...)
	if queData then queData.priority = priorityLevel end
	return queData
end
	...
	if self.active then
		...
		if self.maxQueue <= 0 or #self._queue + 1 <= self.maxQueue then
			...
			if self.priorityEnabled then
				table.sort(self._queue, function(queA, queB) return queA.priority > queB.priority end)
			end

			if not self._processing then
				coroutine.wrap(function()
					if serverRunning then wait(0.1) end

					if self._queue[#self._queue] == queData and not self._processing then
						if queueDebug then warn("Que " .. queData.id .. " - Processing queue..") end
...
function Queue:process(startInd: number, override: boolean)
	if self.active and (override or not self._processing) then
		...
		if self.initialProcessDelay > 0 and not override and serverRunning then wait(self.initialProcessDelay) end

		if
			(self.processCooldown > 0 and self.lastProcessOs)
			and (tick() - self.lastProcessOs < self.processCooldown)
			and not override
		then
			local processCooldown = self.processCooldown
			local lastProcessOs = self.lastProcessOs

			if self.debug then warn("Queue " .. tostring(self.id) .. " waiting for process cooldown..") end

			repeat
				if not serverRunning then return end

				wait()
			until not self.active or self.processId ~= processId or (tick() - lastProcessOs > processCooldown)
		...
		if not self.active or self.processId ~= processId then return end

		if self.debug then warn("Queue " .. tostring(self.id) .. " processing queue..") end
		...
		for i, que in ipairs(self._queue) do
			...
			if self.processId ~= processId then return end

			if i < startInd then continue end
			...
			if que then
				if que.active and not que._processing and (not ignoreProcessedQues or not que._processState) then
					...
					coroutine.wrap(function()
						...
						if not rets[1] and errTrace then rets[3] = errTrace end

						if not rets[1] and errMsg then rets[2] = errMsg end
						...
						if not success then
							if self.warnError or self.debug then
								warn(
									"Queue "
										.. tostring(que.id)
										.. " processor encountered an error while processing "
										.. tostring(que.id)
										.. ": "
										.. tostring(error or "[Unknown error - " .. generateGUID(httpServ) .. "]")
								)
							end
							...
							if not manualProcess then que.processed:Fire(false, unpack(que.returned)) end
							...
							if not manualProcess then que.processed:Fire(true, unpack(que.returned)) end
						end
					...
					if self.delayPerEntry and self.delayPerEntry > 0 then
						if serverRunning then wait(self.delayPerEntry) end
					end
				end
			end
		end
		...
		if self.active then self.processIdle:Fire() end
		...
		if self.debug then warn("Queue " .. tostring(self.id) .. " finished processing") end
		...
		if #self._queue > 0 and self.active then
			...
			if self.afterProcessDelay > 0 and serverRunning then
				if self.debug then warn("Queue " .. tostring(self.id) .. " waiting for afterProcessDelay..") end
		...
		local wrap = setmetatable({}, {
			__index = function(this, ind)
				...
				if choseType == "function" then
					return function(self, ...) return chosen(selfQueue, ...) end
				else
					return chosen
				end
			end,

			__newindex = function(this, ind, val) selfQueue[ind] = val end,
...
function Queue:linkCrossCommunication(crossQueueName: string, settings: { [string]: any }?)
	...
	crossQueue._sessionLockDatastore = crossQueue.settings.SessionLockDatastore
		or DataStoreService:GetDataStore("EssQueues-SessionLock", base64Encode(crossQueueName))
	...
	function crossQueue:add(value: any, expiration, priority)
		...
		local addRateLimitData =
			{ rateLimitCheck(GlobalCrossQueueRateLimit.AddAndRemove, `{queue.id}-{crossQueueName}`) }
	...
	function crossQueue:_removeQueId(itemId: string, currentAttempts: number?)
		...
		if not pcall(function() self._instance:RemoveAsync(itemId) end) then
			if currentAttempts and currentAttempts + 1 < 5 then
				if not table.find(self._cacheIgnoredQues, itemId) then table.insert(self._cacheIgnoredQues, itemId) end
	...
	function crossQueue:_startQueueCheck()
		...
		local function checkValidMemoryQueData(memoryQueData)
			...
			for ind, val in pairs(dataFormat) do
				if type(memoryQueData[ind]) ~= val then return false end
			end
		...
		local function doCheck()
			...
			local successToRead, crossQueueItems, itemId = pcall(function()
				return crossQueue._instance:ReadAsync(
					...
					false,
					crossQueue.settings.WaitTimeout
				)
			end)
			...
			if successToRead and type(crossQueueItems) == "table" and #crossQueueItems > 0 then
				if queueDebug then queue:warn(`Received {#crossQueueItems} queue items`) end
				...
				if crossQueue.settings.SessionLockItem then
					local function checkValidSessionDataFormat(sessionData)
						...
						for ind, val in pairs(dataFormat) do
							if type(sessionData[ind]) ~= val then return false end
						end
					...
					local function updateCallback(sessionLockData, dataKeyInfo)
						if _removeState then
							if
								not checkValidSessionDataFormat(sessionLockData)
								or (
									os.time() - sessionLockData.Started < GlobalCrossQueueSessionLockDeadLockSeconds
					...
					local function doDSAttempt()
						local sessionLockRateLimitData = {
							rateLimitCheck(
								GlobalCrossQueueRateLimit.SessionLock_DatastoreRL,
								`{queue.id}-{crossQueueName}`
							),
						}
						local sessionLockRatePass, sessionLockRateResetOsTime =
							sessionLockRateLimitData[1], sessionLockRateLimitData[7]
						...
						if serverRunning then wait(DatastoreGetRequestDelay "write") end

						local success, error = pcall(
							crossQueue._sessionLockDatastore.UpdateAsync,
							crossQueue._sessionLockDatastore,
							queue,
							updateCallback
						)
					...
					for dataAttempt = 1, GlobalCrossQueueSessionLockDataStoreRetryAttempts do
						...
						if success and _sessionLocked then
							if queueDebug then
								self:warn(
									"Successfully session locked memory que "
										.. tostring(itemId)
										.. ". Now adding memory que to the queue."
								)
							end
							...
							repeat
								...
								if queData == nil then
									...
									if queueDebug then
										self:warn(
											`Queue is too full to add memory que {itemId}. Waiting for idle queue to add..`
										)
									end
								...
								elseif rawequal(queData, false) and resetRateTimeOs then
									if queueDebug then
										self:warn(
											`Memory que {itemId} was rate limited by the queue. Waiting for rate limit to cool down..`
										)
									end

									wait(resetRateTimeOs - tick())

									if queueDebug then self:warn "Finished rate limit to cool down. Now retrying.." end

									if not serverRunning then break end
								else
									...
									queData.processed:Connect(function(wasSuccess)
										if wasSuccess then
											if queueDebug then
												self:warn(
													`Memory que {itemId} finished processing. Now removing session lock and from queue..`
												)
											end
									...
									if queueDebug then self:warn(`Memory que {itemId} successfully queued.`) end
								end
							until not failToAddToQueue

							break
						elseif success and not _sessionLocked then
							if queueDebug then
								self:warn(
									`DS Attempt {dataAttempt} memory que {itemId} is currently session locked. Ignoring memory que..`
								)
							end
							...
							if queueDebug then
								self:warn(
									`Failed to session lock memory que {itemId}. Waiting {dataAttempt * GlobalCrossQueueReadRetryCooldown} seconds.`
								)
							end
					...
					if not _didDSAttempt then
						...
						if queueDebug then
							self:warn(
								"Failed to session lock memory que "
									.. tostring(itemId)
									.. `. Halting cross queue check for {finishTimeout} seconds.`
							)
						end

						if queue.active then waitManager:add(finishTimeout, doCheck) end

						return
					end
				else
					for i, item in ipairs(crossQueueItems) do
						if not checkValidMemoryQueData(item) then continue end
			...
			elseif not successToRead then
				...
				if queue.active then waitManager:add(math.random(8, 15), doCheck) end
			...
			if queue.active then waitManager:add(math.random(3, 6), doCheck) end
		end
