...
local RE = script.Parent:FindFirstChildOfClass "RemoteEvent"
...
do
	...
	local function CheckMacro(macro)
		for i = 1, #macro - 1 do
			if not UserInputService:IsKeyDown(macro[i]) then return end
		end
		ToggleFreecam()
	end
	...
	local function HandleActivationInput(action, state, input)
		if state == Enum.UserInputState.Begin then
			if input.KeyCode == FREECAM_MACRO_KB[#FREECAM_MACRO_KB] then CheckMacro(FREECAM_MACRO_KB) end

			if input.KeyCode == Enum.KeyCode.F then ToggleFreecam() end
		end
	...
	RE.OnClientEvent:Connect(function(a, b, c)
		...
		if a == "Toggle" then ToggleFreecam() end
