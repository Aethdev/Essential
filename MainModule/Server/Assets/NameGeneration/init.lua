...
function NameGenerator:load()
	...
	if Names then
		for i, module in pairs(Names:GetChildren()) do
			...
			if not moduleData.FirstNames then warn(`NameGenerators: Module {category} is missing FirstNames list`) continue end
			if not moduleData.SurNames then warn(`NameGenerators: Module {category} is missing SurNames list`) continue end
...
function NameGenerator:generate(generateOptions: {
	...
	local CategoryModule: {FirstNames: {[number]: string}, SurNames: {[number]: string}} = NameGenerator._loadedList[Category]
	...
	if generateOptions.IncludeSurName then
		...
		assert(NumberOfSurNames > 0 or math.floor(NumberOfSurNames) ~= NumberOfSurNames, `Number of Sur Names is not an integer or more than zero`)
		...
		if #CategoryModule.SurNames > 0 then
			...
			for i = 1, math.clamp(NumberOfSurNames, 1, #CategoryModule.SurNames), 1 do
				...
				until
					not table.find(_chosenSurNames, ChosenSurname) or #_chosenSurNames >= #CategoryModule.SurNames
					
				table.insert(_chosenSurNames, ChosenSurname)
			end
			
			SurName = if generateOptions.NoSplitNames then table.concat(_chosenSurNames, "") else ` `..table.concat(_chosenSurNames, " ")
		end
	end
