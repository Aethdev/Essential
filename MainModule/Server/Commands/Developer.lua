...
return function(envArgs)
	...
	local cmdsList = {
		...
		testBubble = {
			...
			Function = function(plr, args)
				local messageInput = "Donec pretium vulputate sapien nec sagittis aliquam malesuada. Sed risus pretium quam vulputate dignissim. Ac felis donec et odio. Pellentesque massa placerat duis ultricies lacus."
		...
		testHint = {
			...
			Function = function(plr, args)
				local messageInput = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
		...
		testContext = {
			...
			Function = function(plr, args)
				local messageInput = args[1] or "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
		...
		testMakeUI = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", "UI Construction results", "GUI: "..args[1].." | Return count: "..tostring(#results)..". View the developer console to see the results.", 20, "Hint")
		...
		showRolesEnlistedAsPlayer = {
			...
			Function = function(plr, args)
				...
				if not target:isInGame() then
					plr:sendData("SendMessage", `{tostring(target)} is not in the server. Some dynamic validation, which roles may include, like asset ownership requires`..
						` the presence of the player in the server. Some roles with those dynamic validation will not show up in your view unless the player is in the server.`, nil, 6, "Context")
				end
				
				local updateRemId = "ViewRoles-"..target.UserId
				local updateRemData = Remote.ListData[updateRemId] or {
					Whitelist = {};
					Permissions = {"Manage_Game"};
					Function = function(plr)
						...
						for roleInd, role in ipairs(priorityRoles1) do
							...
							for i,otherPlr in ipairs({target}) do
								...
								if role:checkMember(if otherPlr:isInGame() then otherPlr else otherPlr.UserId) or (isRoleDataSaved and role.saveable) then
									memberCount += 1
									...
									if role:checkTempMember(otherPlr) or isRoleDataSaved then
										table.insert(membersList, {
											type = "Detailed";
											label = `> `..(isRoleDataSaved and "🏢" or "🔖").." "..otherPlr.DisplayName.." (@"..otherPlr.Name.." / "..otherPlr.UserId..")";
										...
										table.insert(membersList, {
											type = "Label";
											label = `> `..otherPlr.DisplayName.." (@"..otherPlr.Name.." / "..otherPlr.UserId..")";
											labelColor = Color3.fromRGB(185, 216, 27);
											hideSymbol = true;
										})
									end
							...
							table.insert(priorityRoles2, {
								type = "Label";
								label = (rolePriority==highPriority and "🔸 " or "")..role.name.." ("..memberCount..")";
								labelColor = role.permissions["Manage_Game"] and Color3.fromRGB(255, 174, 44) or nil;
								description = "Priority: "..tostring(role.priority).." | Assignable: "..tostring(role.assignable or false);
							})
				...
				if not Remote.ListData[updateRemId] then
					Remote.ListData[updateRemId] = updateRemData
				end
		...
		showUsableCommandsAsPlayer = {
			...
			Function = function(plr, args)
				...
				local updateRemData = Remote.ListData[updateRemId] or {
					...
					Permissions = {};
					Function = function()
						return Remote.ListData.Commands.Function(target)
					end;
				}
		...
		executeCode = {
			...
			Function = function(plr, args)
				...
				if type(func) == "function" then
					...
					if not suc then
						plr:sendData("SendMessage", "Loadstring function error", tostring(errMsg).."\n"..tostring(errorTrace), 30)
					else
