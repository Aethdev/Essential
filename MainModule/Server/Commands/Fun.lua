			...
			Function = function(plr, args)
				...
				for i,id in pairs(args[2]) do
					...
					if gearId and gearsBanlist[gearId] then
						if not table.find(bannedGears, gearId) then
							table.insert(bannedGears, gearId)
						end
						...
						if gearInfo.AssetTypeId == 10 or gearInfo.AssetTypeId == 19 then
							...
							for _,item in pairs(assetItems) do
								if item:IsA"Tool" then
									table.insert(assetTools, item)
								end
							end

							if #assetTools > 0 then
								if not table.find(listedGears, gearId) then
									table.insert(listedGears, gearId)
								end
				...
				if #bannedGears > 0 then
					plr:sendData("SendMessage", "Gear Insertion failed", "Gears such as "..table.concat(bannedGears, ", ").." are banned or not allowed due to insufficient permissions.", 3, "Hint")
					wait(3)	
				end

				if #listedGears > 0 then
					plr:sendData("SendMessage", "Gear Insertion success", "Gears such as "..table.concat(listedGears, ", ").." are inserted successfully.", 6, "Hint")
				else
					plr:sendData("SendMessage", "Gear Insertion failed", "There were no gears listed to be inserted.", 6, "Hint")
				end
			end;
		};
	}
