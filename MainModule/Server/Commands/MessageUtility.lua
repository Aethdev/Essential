	...
	local cmdsList = {
			...
			Function = function(plr, args)
				for i,target in pairs(service.getPlayers()) do
					...
					if parsedTarget then
						parsedTarget:sendData("SendMessage", "Message from <b>"..playerDisplayName.."</b>", args[1], math.floor(math.clamp(#args[1]*0.1, 3, 30)), "Hint")
					end
				end
			end;
		};
		...
		message = {
			...
			Function = function(plr, args)
				...
				for i,target in pairs(service.getPlayers()) do
					...
					if parsedTarget then
						parsedTarget:sendData("SendMessage", "Message from <b>"..playerDisplayName.."</b>", args[1], math.floor(math.clamp(#args[1]*0.1, 3, 30)))
					end
				end
			end;
		};
		...
		messagePrivate = {
			...
			Function = function(plr, args)
				...
				for i,target in pairs(args[1]) do
					local playerDisplayName = plr:toStringDisplayForPlayer(target)
					target:sendData("SendMessage", "Message from <b>"..playerDisplayName.."</b>", args[2], math.floor(math.clamp(#args[2]*0.1, 3, 30)))
				end
			end;
		};
		...
		bubble = {
			...
			Function = function(plr, args)
				for i,target in pairs(service.getPlayers()) do
					...
					if parsedTarget then
						parsedTarget:sendData("SendMessage", "Message from <b>"..playerDisplayName.."</b>", args[1], math.floor(math.clamp(#args[1]*0.1, 3, 30)), "Bubble")
					end
				end
			end;
		};
		...
		bubblePrivate = {
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					local playerDisplayName = plr:toStringDisplayForPlayer(target)
					target:sendData("SendMessage", "Message from <b>"..playerDisplayName.."</b>", args[2], math.floor(math.clamp(#args[2]*0.1, 3, 30)), "Bubble")
				end
			end;
		};
		...
		context = {
			...
			Function = function(plr, args)
				...
				for i,target in pairs(service.getPlayers()) do
					...
					if parsedTarget then
						parsedTarget:sendData("SendMessage", "<b>"..playerDisplayName.."</b>: "..server.Parser:filterForRichText(args[1]), playerDisplayName..": "..args[1], math.floor(math.clamp(#args[1]*0.1, 3, 30)), "Context")
					end
				end
			end;
		};
		...
		contextPrivate = {
			...
			Function = function(plr, args)
				...
				for i,target in pairs(args[1]) do
					local playerDisplayName = plr:toStringDisplayForPlayer(target)
					target:sendData("SendMessage", "<b>"..playerDisplayName.."</b>: "..server.Parser:filterForRichText(args[2]), playerDisplayName..": "..args[2], math.floor(math.clamp(#args[2]*0.1, 3, 30)), "Context")
				end
			end;
		};
		...
		directMessage = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", `Successfully direct messaged <b>{#args[1]} player(s)</b>. This process will take 60-90 seconds for the specified players to receive their message.`, nil, 10, "Context")
			end;
		};
		...
		systemMessage = {
			...
			Function = function(plr, args)
				for i,target in pairs(service.getPlayers(true)) do
					target:sendData("SendMessage", tostring(settings.systemMessage), args[1], math.floor(math.max(#args[1]*0.1, 30)))
				end
			end;
		};
			...
			Function = function(plr, args)
				if args[1].total > 0 then
					...
					service.startLoop("Countdown", 1, function()
						if count > -1 then
							for i,target in pairs(service.getPlayers(true)) do
								target:sendData("SendMessage", "Countdown", server.Parser:formatTime(count), 1, "Bubble", true, true, true)
							end
							...
							for i = 1,5,1 do
								for ind,target in pairs(service.getPlayers(true)) do
									target:sendData("SendMessage", "Countdown", "<font color='#ff3300'>00:00:00</font>", 1, "Bubble", true, true, true)
								end
								wait(1)
							end
						end
