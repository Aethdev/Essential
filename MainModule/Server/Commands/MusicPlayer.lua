return function(envArgs)
	...
	musicPlayerQueue.processFunc = function(ind, que, musicId, creatorId)
		if que.active then
			...
			local songMessage = "Now playing <b>"
				.. Parser:filterForRichText(musicInfo.Name)
				.. "</b> ("
				.. tostring(musicId)
				.. ")"
			...
			soundData.ended:connectOnce(function()
				if not movedOn then confirmSignal:fire() end
			end)
	...
	local cmdsList = {
				...
				if bannedSong then
					if plr then
						plr:sendData(
							"SendMessage",
							"Music Player",
							"Music id <b>" .. tostring(args[1]) .. "</b> is banned and cannot be played.",
							6,
							"Hint"
						)
					end
				...
				if not userCreator then
					...
					if groupInfo and groupInfo.Id then
						if Identity.checkTable(groupInfo.Owner.Id, settings.musicPlayer_BannedCreators) then
							plr:sendData(
								"SendMessage",
								"Music Player",
								"Songs from creator <b>" .. groupInfo.Owner.Name .. "</b> are banned from being played.",
								6,
								"Hint"
							)
							return
					...
					if Identity.checkTable(assetInfo.Creator.Id, settings.musicPlayer_BannedCreators) then
						plr:sendData(
							"SendMessage",
							"Music Player",
							"Songs from creator <b>" .. assetInfo.Creator.Name .. "</b> are banned from being played.",
							6,
							"Hint"
						)
						return
				...
				if queueEnabled then
						...
						for i, que in pairs(musicPlayerQueue._queue) do
							if que.arguments[1] == musicId then
								plr:sendData(
									"SendMessage",
									"Music player",
									"This song was already added in the queue. Pick another song.",
									5,
									"Hint"
								)
								return
					...
					if creationCount + 1 > maxPlayerCreation then
						plr:sendData(
							"SendMessage",
							"Music player",
							"You can't add more songs to the queue. Try removing some of your added songs from the queue via command "
								.. settings.actionPrefix
								.. "removemusic",
							5,
							"Hint"
						)
					...
					if queData then
						plr:sendData(
							...
							"Added "
								.. (songId or args[1])
								.. " to queue. Your song will play soon once it's chosen in the queue.",
							5,
							"Hint"
						)
					else
						plr:sendData(
							"SendMessage",
							"Music player",
							"Oops! The queue is currently full. Try again later.",
							5,
							"Hint"
						)
					end
					...
					if args[2] then mainSound.Looped = args[2] end

					if args[3] then mainSound.PlaybackSpeed = args[3] end

					if args[4] then mainSound.Volume = math.clamp(args[4], 0, 4) end
			...
			Function = function(plr, args)
				...
				if not mainSound then
					plr:sendData(
						"SendMessage",
						"Music player",
						"Can't perform action if there's no music in game.",
						4,
						"Hint"
					)
					return
		...
		repeatMusic = {
			...
			Function = function(plr, args)
				...
				if not args[1] then
					plr:sendData(
						"SendMessage",
						"Music player | Repeat statuses",
						"Queue: " .. tostring(musicPlayerQueue.repeatProcess) .. " | Track: " .. tostring(musicLoop),
						8,
						"Hint"
					)
					return
				...
				if repeatType == "track" then
					if not mainSound then
						plr:sendData(
							"SendMessage",
							"Music player",
							"Can't change the track's repeat status if there's no music in game.",
							4,
							"Hint"
						)
						return
					...
					plr:sendData(
						"SendMessage",
						"Music player",
						"Changed repeat status for tracks to " .. tostring(newLoop),
						4,
						"Hint"
					)
				elseif repeatType == "queue" then
					...
					plr:sendData(
						"SendMessage",
						"Music player",
						"Changed repeat status for queue to " .. tostring(newLoop),
						4,
						"Hint"
					)
				end
			...
			Function = function(plr, args)
				if #musicPlayerQueue._queue == 0 then
					plr:sendData(
						"SendMessage",
						"Music player",
						"The queue is empty. There's nothing to restart.",
						4,
						"Hint"
					)
				else
			...
			Function = function(plr, args)
				...
				if #musicPlayerQueue._queue > 0 then
					...
					plr:sendData(
						"SendMessage",
						"Music player",
						"There's nothing to clear. Are you sure there was music added to the queue?",
						5,
						"Hint"
					)
				end
				...
				if #queue > 0 then
					...
					if not queTab then
						plr:sendData(
							"SendMessage",
							"Music player",
							"The queue isn't playing and there's nothing to skip. Restart the queue by "
								.. settings.actionPrefix
								.. "restartmusic",
							3,
							"Hint"
						...
						if queTab.active then
							...
							if musicPlayerQueue._queue[newQueInd] then
								...
								plr:sendData(
									"SendMessage",
									"Music player",
									"Skipped current music selection",
									3,
									"Hint"
								)
							else
					...
					plr:sendData(
						"SendMessage",
						"Music player",
						"There's nothing in the queue. Are you sure there was music added to the queue?",
						5,
						"Hint"
					)
				end
		...
		jumpMusicQueue = {
			...
			Function = function(plr, args)
				...
				if #queue > 0 then
					...
					if jumpingQue then
						coroutine.wrap(function() musicPlayerQueue:process(args[1], true) end)()
						...
						plr:sendData(
							"SendMessage",
							"Music player",
							"Que " .. args[1] .. " doesn't exist. Unable to jump.",
							4,
							"Hint"
						)
					end
					...
					plr:sendData(
						"SendMessage",
						"Music player",
						"There's nothing to jump. Are you sure there was music added to the queue?",
						5,
						"Hint"
					)
				end
				...
				if #queue > 0 then
					...
					if existingQue then
						...
						plr:sendData(
							"SendMessage",
							"Music player",
							"Que " .. args[1] .. " doesn't exist. Unable to remove.",
							4,
							"Hint"
						)
					end
					...
					plr:sendData(
						"SendMessage",
						"Music player",
						"There's nothing to remove. Are you sure there was music added to the queue?",
						5,
						"Hint"
					)
				end
		...
		viewMusicQueue = {
			...
			Function = function(plr, args)
				...
				if #queue > 0 then
					...
					if mainSound and mainSound.IsPlaying then soundStat = "playing" end

					if mainSound then
						local timeLapData = server.Parser:getTime(mainSound.TimePosition)
						local timelapse =
							server.Parser:formatTime(timeLapData.hours, timeLapData.mins, timeLapData.secs)

						local timeLenData = server.Parser:getTime(mainSound.TimeLength)
						local timelength =
							server.Parser:formatTime(timeLenData.hours, timeLenData.mins, timeLenData.secs)
					...
					if not nowPlayingFound then addEntryToList "Nothing playing right now." end
					...
					plr:sendData(
						"SendMessage",
						"Music player",
						"There's nothing playing nor added in the queue. Add more songs to see what's playing and added ones.",
						4,
						"Hint"
					)
				end
			...
			Function = function(plr, args)
				...
				if not musicPlaylist then
					plr:sendData(
						"SendMessage",
						"Music player",
						"Playlist <b>" .. args[1] .. "</b> doesn't exist and cannot be added to the queue.",
						6,
						"Hint"
					)
					return
				...
				if #musicPlaylist == 0 then
					plr:sendData(
						...
						"Playlist <b>"
							.. args[1]
							.. "</b> doesn't have any songs added. Nothing has been added to the queue.",
						6,
						"Hint"
					)
					return
				...
				local maxSongs = math.clamp(
					maxCreations,
					0,
					math.clamp(songsCountInPlaylist, 0, settings.musicPlayer_MaxSongs - #songsQueue)
				)
				...
				if maxSongs > 0 then
						...
						if songIdFromPlaylist then
							...
							if musicInfo.AssetTypeId == 3 then
								if noSongDuplication then
									...
									if skipAddition then continue end
								end
					...
					if addedSongs > 0 then
						plr:sendData(
							"SendMessage",
							"Music player",
							"Added " .. addedSongs .. " songs from playlist <b>" .. args[1] .. "</b>",
							6,
							"Hint"
						)
					else
						plr:sendData(
							"SendMessage",
							"Music player",
							"There were no available songs from playlist <b>" .. args[1] .. "</b> to add.",
							4,
							"Hint"
						)
					end
					...
					plr:sendData(
						"SendMessage",
						"Music player",
						"There are no songs available to add. This is the amount of your added songs have exceeded the max player creations or queue is full.",
						6,
						"Hint"
					)
				end
			...
			Function = function(plr, args)
				...
				if not musicPlaylist then
					plr:sendData(
						"SendMessage",
						"Music player",
						"Playlist <b>" .. args[1] .. "</b> doesn't exist and cannot be viewed.",
						6,
						"Hint"
					)
					return
				...
				if #musicPlaylist == 0 then
					plr:sendData(
						...
						"Playlist <b>"
							.. args[1]
							.. "</b> doesn't have any songs added. There's no songs available to list.",
						6,
						"Hint"
					)
					return
				...
				for i, songId in pairs(musicPlaylist) do
					...
					if songId then
						...
						if musicInfo.AssetTypeId == 3 then canUseSong = true end

						table.insert(songsList, {
							Text = (canUseSong and "✅" or "❌") .. " | [" .. songId .. "] - " .. tostring(
								musicInfo.Name or "UNKNOWN SONG"
							),
							Desc = (not canUseSong and "Song not available and cannot be played in the queue")
								or "Id: " .. songId .. " | " .. tostring(musicInfo.Name),
			...
			Function = function(plr, args)
				...
				if #songs == 0 then
					plr:sendData(
						"SendMessage",
						"Music list",
						"There are no songs to provide. Check with the developer if they had listed available songs.",
						6,
						"Hint"
					)
				else
			...
			Function = function(plr, args)
				if args[1]:find(settings.delimiter) then
					plr:sendData(
						...
						"Playlist name must not have a delimiter",
						10,
						"Hint"
					)
				...
				elseif #args[1] > 30 then
					plr:sendData(
						"SendMessage",
						"Music player management",
						"Playlist name must have up to 30 characters",
						10,
						"Hint"
					)
					return -1, "ResetCooldown"
				...
				if not savedPlaylists[args[1]] then
					plr:sendData(
						"SendMessage",
						"Music player management",
						"Creating saved playlist " .. args[1] .. ". Please wait.",
						20,
						"Hint"
					)
					Datastore.tableUpdate(
						"Music",
						"SavedPlaylists",
						"Index",
						args[1],
						...
						function()
							plr:sendData(
								"SendMessage",
								"Music player management",
								"Successfully created saved playlist " .. args[1],
								8,
								"Hint"
							)
						end
					...
					plr:sendData(
						"SendMessage",
						"Music player management",
						"Saved playlist " .. args[1] .. " already exists.",
						8,
						"Hint"
					)
				end
		...
		viewSavedPlaylist = {
			...
			Function = function(plr, args)
				...
				if type(savedPlaylists) ~= "table" then savedPlaylists = {} end

				if not savedPlaylists[args[1]] then
					plr:sendData(
						"SendMessage",
						"Music player management",
						"Saved playlist " .. args[1] .. " doesn't exist.",
						6,
						"Hint"
					)
				else
					...
					plr:makeUI("ADONIS_LIST", {
						Title = "E. Saved Playlist "
							.. args[1]
							.. " ("
							.. service.tableCount(savedPlaylists[args[1]])
							.. "/20)",
						Table = songsList,
						RichText = true,
					})
			...
			Function = function(plr, args)
				...
				if not savedPlaylists[args[1]] then
					...
					plr:sendData(
						"SendMessage",
						"Music player management",
						"Clearing saved playlist " .. args[1] .. ". Please wait.",
						20,
						"Hint"
					)
					Datastore.tableUpdate(
			...
			Function = function(plr, args)
				...
				if not savedPlaylists[args[2]] then
					plr:sendData(
						"SendMessage",
						"Music player management",
						"Saved playlist " .. args[2] .. " doesn't exist.",
						6,
						"Hint"
					)
				else
					...
					if entryCount + 1 > 20 then
						plr:sendData(
							"SendMessage",
							"Music player management",
							"Unable to add more songs for the saved playlist " .. args[2],
							6,
							"Hint"
						)
						return
						...
						if groupInfo and groupInfo.Id then
							if Identity.checkTable(groupInfo.Owner.Id, settings.musicPlayer_BannedCreators) then
								plr:sendData(
									"SendMessage",
									"Music player management",
									"Songs from creator <b>"
										.. groupInfo.Owner.Name
										.. "</b> are banned from being played.",
									6,
									"Hint"
								)
								return
					...
					plr:sendData(
						"SendMessage",
						"Music player management",
						"Adding song " .. tostring(assetInfo.Name) .. " to the saved playlist " .. args[2],
						20,
						"Hint"
					)
				...
				if not savedPlaylists[args[2]] then
					...
					if not table.find(playlistData, args[1]) then
						plr:sendData(
							"SendMessage",
							"Music player management",
							"Song id " .. tostring(args[1]) .. " doesn't exist in the saved playlist " .. args[2],
							10,
							"Hint"
						)
					else
						...
						plr:sendData(
							"SendMessage",
							"Music player management",
							"Removing song " .. tostring(assetInfo.Name) .. " from the saved playlist " .. args[2],
							20,
							"Hint"
						)

						Datastore.tableRemove(
							"Music",
							"SavedPlaylists",
							"valueFromIndex",
							args[2],
							...
							function()
								plr:sendData(
									...
									"Song "
										.. tostring(assetInfo.Name)
										.. " was removed from the saved playlist "
										.. args[2],
									8,
									"Hint"
								)
