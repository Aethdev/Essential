	...
	local cmdsList = {
		reserveServer = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", "Reserving server <b>"..Parser:filterForRichText(args[1]).."</b>..", "Reserving server "..args[1].."..", 12, "Context")
				...
				if serversList[args[1]] then
					plr:sendData("SendMessage", "Server <b>"..Parser:filterForRichText(args[1]).."</b> is <u>already reserved</u>.", "Server "..args[1].." is already reserved.", 10, "Context")
					return
				end
				
				if service.tableCount(serversList) > maximumCreation then
					plr:sendData("SendMessage", "Unable to reserve server due to maximum amount of reserved servers.", nil, 10, "Context")
				else
					plr:sendData("SendMessage", "Creating reserved server <b>"..Parser:filterForRichText(args[1]).."</b>..", "Creating reserved server "..args[1].."..", 12, "Context")
					
					Utility:createReserveServer(args[1], plr.UserId)
					
					plr:sendData("SendMessage", "Successfully reserved server "..Parser:filterForRichText(args[1]), "Successfully reserved server "..args[1], 5, "Context")
				end
			end;
		};
		...
		joinReserveServer = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", "Joining reserved server <b>"..Parser:filterForRichText(args[1]).."</b>..", "Joining reserved server "..args[1].."..", 12, "Context")
				local serverProfile = Utility:getReserveServer(nil, args[1])

				if not serverProfile then
					plr:sendData("SendMessage", "Server <b>"..Parser:filterForRichText(args[1]).."</b> is <u>not reserved</u>.", "Server "..args[1].." is not reserved.", 10, "Context")
					return
				end
				
				plr:teleportToReserveWithSignature(serverProfile.details.serverAccessId, function()
					plr:sendData("SendMessage", "Failed to join the reserved server <b>"..Parser:filterForRichText(args[1]).."</b>", nil, 4, "Context")
				end)
			end;
		};
		...
		forceTeleportReserveServer = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", "Finding reserved server <b>"..Parser:filterForRichText(serverName).."</b>..", "Finding reserved server "..serverName.."..", 12, "Context")
				local serverProfile = Utility:getReserveServer(nil, serverName)
				if not serverProfile then
					plr:sendData("SendMessage", "Server <b>"..Parser:filterForRichText(serverName).."</b> is <u>not reserved</u>.", "Server "..serverName.." is not reserved.", 10, "Context")
					return
				end
				
				plr:sendData("SendMessage", "Found reserved server <b>"..Parser:filterForRichText(serverName).."</b>. Now teleporting them..", nil, 12, "Context")
				
				for i, target in pairs(args[1]) do
					task.defer(function()
						target:sendData("SendMessage",
							"You are now teleporting to reserved server <b>"..Parser:filterForRichText(serverName).."</b>.",
						...
						target:teleportToReserveWithSignature(serverProfile.details.serverAccessId, function(errType)
							if errType == "teleporting" then
								plr:sendData("SendMessage",
									"Uh, oh! <b>"..target.Name.."</b> cannot teleport to reserved server <i>"..Parser:filterForRichText(serverName).."</i> if they are already teleporting.",
									"Uh, oh! "..target.Name.." failed to teleport to reserved server "..serverName.." if they are already teleporting.",
									10, "Context")
							elseif errType == "error" then
								plr:sendData("SendMessage",
									"Uh, oh! <b>"..target.Name.."</b> failed to teleport to reserved server <i>"..Parser:filterForRichText(serverName).."</i>.",
									"Uh, oh! "..target.Name.." failed to teleport to reserved server "..serverName..".",
									10, "Context")

								target:sendData("SendMessage",
									"Uh, oh! You failed to teleport to reserved server <i>"..Parser:filterForRichText(serverName).."</i>.",
		...
		deleteReserveServer = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", "Finding reserved server <b>"..Parser:filterForRichText(args[1]).."</b>..", "Finding reserved server "..args[1].."..", 12, "Context")
				...
				if not serversList[args[1]] then
					plr:sendData("SendMessage", "Reserved server <b>"..Parser:filterForRichText(args[1]).."</b> doesn't exist", "Reserved server "..args[1].." doesn't exist", 6, "Context")
				else
					...
					if not canDeleteServer then
						plr:sendData("SendMessage", "You must own reserved server <b>"..Parser:filterForRichText(args[1]).."</b> or server administrator to delete it.", nil, 6, "Context")
						return
					end
					...
					if didRemoveRegistry then
						plr:sendData("SendMessage", "Successfully deleted reserved server <b>"..Parser:filterForRichText(args[1]).."</b>", "Successfully deleted reserved server "..args[1], 6, "Context")
					else
						plr:sendData("SendMessage", "Failed to delete reserved server <b>"..Parser:filterForRichText(args[1]).."</b>", "Failed to delete reserved server "..args[1], 6, "Context")
					end
				end
			end;
		};
		...
		listReservedServers = {
			...
			Function = function(plr, args)
				...
				for serverName,detail in pairs(serversList) do
					...
					local ownerName = detail.creatorId==0 and "-SYSTEM-" or "@"..service.playerNameFromId(detail.creatorId)
					
					table.insert(tabList, {
						type = "Detailed";
						Text = "[Owned by "..ownerName.."] "..serverName;
						Desc = "Created on "..formatTime.." UTC";
						Color = privateServerData and privateServerData.details.serverId==detail.serverId and
							Color3.fromRGB(121, 200, 30) or nil;
					})
				end
		...
		manageReserveMaxPlayers = {
			...
			Function = function(plr, args)
				...
				if prevMaxPlayers ~= args[1] then
					...
					if variables.privateServerData and variables.privateServerData.details.serverId == serverProfile.details.serverId then
				...
				plr:sendData("SendMessage", "Successfully set max number of players for reserved server to <b>"..args[1].."</b>. This may take a few minutes to apply the changes.", nil, 10, "Context")
			end;
		};
		...
		addWhitelistToTheReserve = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", `Whitelisting <b>{#args[2]} player(s)</b> to the reserved server..`, nil, 12, "Context")
				...
				if #failedPlayers > 0 then
					plr:sendData("SendMessage", `Failed to whitelist <b>{#failedPlayers} player(s)</b> in reserved server {serverName}`, nil, 10, "Context")
					wait(2)
				end
				
				if #successPlayers > 0 then
					plr:sendData("SendMessage", `Successfully whitelisted <b>{#successPlayers} player(s)</b> in reserved server {serverName}.  This may take a few minutes to apply the changes.`, nil, 10, "Context")
				end
			end;
		};
		...
		remWhitelistToTheReserve = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", `Unwhitelisting from <b>{#args[2]} player(s)</b> in the reserved server..`, nil, 12, "Context")
				...
				if #failedPlayers > 0 then
					plr:sendData("SendMessage", `Failed to unwhitelist <b>{#failedPlayers} player(s)</b> in reserved server {serverName}`, nil, 10, "Context")
					wait(2)
				end

				if #successPlayers > 0 then
					plr:sendData("SendMessage", `Successfully unwhitelisted <b>{#successPlayers} player(s)</b> in reserved server {serverName}.  This may take a few minutes to apply the changes.`, nil, 10, "Context")
				end
			end;
		};
		...
		addBlacklistToTheReserve = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", `Blacklisting <b>{#args[2]} player(s)</b> in the reserved server..`, nil, 12, "Context")
				...
				if #failedPlayers > 0 then
					plr:sendData("SendMessage", `Failed to blacklist <b>{#failedPlayers} player(s)</b> in reserved server {serverName}`, nil, 10, "Context")
					wait(2)
				end

				if #successPlayers > 0 then
					plr:sendData("SendMessage", `Successfully blacklisted <b>{#successPlayers} player(s)</b> in reserved server {serverName}.  This may take a few minutes to apply the changes.`, nil, 10, "Context")
				end
			end;
		};
		...
		remBlacklistToTheReserve = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", `Unblacklisting from <b>{#args[2]} player(s)</b> in the reserved server..`, nil, 12, "Context")
