...
return function(envArgs)
	...
	local cmdsList = {
		rejoin = {
			...
			Function = function(plr, args)
				if service.RunService:IsStudio() or (not variables.privateServerData and #game.PrivateServerId > 0) then
					plr:sendData("SendMessage", "Teleportation Error", "Unable to rejoin due to private or studio server you are currently in", 300, "Hint")
					return
				end
				...
				service.debounce("Rejoin server - "..plr.playerId, function()
					...
					local function errCallback(errType)
						plr:sendData("SendMessage", "Teleportation Error", "Failed to rejoin the server. Please try again later!", 6, "Hint")
					end
		...
		nowPlaying = {
			...
			Function = function(plr, args)
				...
				if mainSound then
					...
					plr:sendData("SendMessage", "Now playing", tostring(variables.music_nowPlaying_name).." ("..tostring(variables.music_nowPlaying_id)..") | <b>"..timelapse.." - "..timelength.."</b> | ("..musicStat..")", 15, "Hint")
				end	
			end;
		};
		...
		ping = {
			...
			Function = function(plr, args)
				plr:sendData("SendMessage", "Generating a ping diagnotic report. This may take a while.", nil, 10, "Context")
				...
				if robloxPingMs >= 0 and robloxPingMs < 20 then
					robloxPingInfo = "Your roblox ping is <b>excellent</b>. It can handle high-performance physics and lagless interactions."
				elseif robloxPingMs >= 20 and robloxPingMs < 50 then
					robloxPingInfo = "Your roblox ping is <b>good</b>. It's stable enough to handle decent physics and invoke interactions smoothly."
				elseif robloxPingMs >= 50 and robloxPingMs < 80 then
					robloxPingInfo = [[Your roblox ping is <b>okay</b>. It's okay to handle some physics and invoke remote objects with a short delay.
					...
					robloxPingInfo = [[Your roblox ping is <b>poor</b>. You may expect large amount of delay while interacting with physics and objects.
		...
		openUsage = {
			...
			Function = function(plr, args)
				...
				local usageInfo = {
					...
					"Start with /e then your command after a space from /e (e.g. /e "..playerPrefix.."help). This alternative doesn't require slash commands";
					...
					"The maximum amount of shortcuts you can have/create is "..tostring(Process.playerSettingsLimits.MaxShortcutCreation)..".";
					"";
					...
					"The maximum amount of action aliases you can have/create is "..tostring(Process.playerSettingsLimits.MaxAliasCreation)..".";
					"";
					"<b>CC Aliases/Names (via client settings)</b>";
					"CC Aliases are your own aliases to refer a command. For instance, you could make an alias called 'ref' to refer "..actionPrefix.."refresh.";
					...
					" aliases you can have/create is "..tostring(Process.playerSettingsLimits.MaxCustomCmdNameCreation)..".";
					"";
					...
					"@user_name - Selects players with the specified username (e.g. "..actionPrefix.."pm @"..plr.Name.." false Hey, it's me!)";
					"&role_name - Selects players with the specified role name (note: The role name CANNOT be partial. You can view the roles list by "..actionPrefix.."roleslist)";
					...
					"Some commands like "..actionPrefix.."gear allow supplying the same player twice. Therefore, the action can target them multiple times. On the other hand,";
					...
					"You can target the same player multiple times by using the same selector. For example, "..actionPrefix.."gear me,me,me,me,me 11419319";
					"";
					"<b>Command batches</b>";
					"ℹ️ Command batches CAN BE DISABLED via developer settings. The maximum amount of command batches is "..tostring(settings.MaxBatchCommands);
					"Command batches are batches of commands supplied in a message. For example,";
					"<i>"..actionPrefix.."m Hey guys. Watch me do a magic trick. "..batchSeperator.." "..actionPrefix.."invisible "..batchSeperator.." "..actionPrefix.."wait5 "..batchSeperator.." "..actionPrefix.."visible</i>";
					"";
					"<b>HandTo system</b>";
					"You can send items to players closer to you. If you want to override the character proximity restriction, you must own permission `HandTo_Utility`. To use handto, do "..playerPrefix.."handto "..Parser:filterForRichText("<player using target selectors>");
					...
					"";
					"Batches can also have a delay command. This is referred as "..playerPrefix.."wait(NUMBER). Take this as a reference, "..playerPrefix.."wait10 - Delays the next command till 10 commands";
					"";
					"--";
					"That's all information of how you can utilize Essential. For a list of commands, do "..actionPrefix.."cmds";
				}
		...
		--	Description = "Gives information about the command such as permissions, roles, etc.";

		--	Function = function(plr, args)
		--		local playerCmd, cmdMatch = args[1].command, args[1].match
		--		local cmdInfo = {
		--			"Prefix: "..Parser:filterForRichText(playerCmd.Prefix);
		--			"Command: "..Parser:filterForRichText(cmdMatch);
		--			`Category: {playerCmd.Category or "<i>Unknown</i>"}`;
		--			"";
		--			"<b>Aliases</b>";
		--			"---------------------";
		--			Parser:filterForRichText(table.concat(playerCmd.Aliases or {}, ", "));
		--			"";
		--			"<b>Description</b>";
		--			"---------------------";
		--			Parser:filterForRichText(tostring(playerCmd.Description));
		--			"";
		--			"<b>Permissions</b>";
		--			(#(playerCmd.Permissions or {}) == 0 and "<i>No permissions needed</i>") or table.concat(playerCmd.Permissions or {}, ", ");
		--			"";
		--			"<b>Roles</b>";
		--			(#(playerCmd.Roles or {}) == 0 and "<i>No roles needed</i>") or table.concat(playerCmd.Roles or {}, ", ");
		--			"";
		--		}
				
		--		table.insert(cmdInfo, "<b>Arguments</b>")
		--		if #playerCmd.Arguments == 0 then
		--			table.insert(cmdInfo, "<i>No arguments</i>")
		--		else
		--			for i, argumentInfo in ipairs(playerCmd.Arguments) do
		--				if type(argumentInfo) == "string" then
		--					table.insert(cmdInfo, i..": "..Parser:filterForRichText(argumentInfo))
		--				elseif type(argumentInfo) == "table" then
		--					table.insert(cmdInfo, i..": "..Parser:filterForRichText(argumentInfo.argument or "argument").." <"..(argumentInfo.type or "string")..">")
		--				else
		--					table.insert(cmdInfo, i..": <i>Unknown</i>")
		--				end
		--			end
		--		end
				
		--		table.insert(cmdInfo, "")
		--		table.insert(cmdInfo, "<b>Cooldown Info</b>")
		--		if not (playerCmd.PlayerCooldown or playerCmd.ServerCooldown or playerCmd.CrossCooldown) then
		--			table.insert(cmdInfo, "<i>This command has no cooldown</i>")
		--		else
		--			if playerCmd.PlayerCooldown then
		--				table.insert(cmdInfo, "PlayerCooldown: <i>"..Parser:formatTime(playerCmd.PlayerCooldown).."</i>")
		--			end
		--			if playerCmd.ServerCooldown then
		--				table.insert(cmdInfo, "ServerCooldown: <i>"..Parser:formatTime(playerCmd.ServerCooldown).."</i>")
		--			end
		--			if playerCmd.CrossCooldown then
		--				table.insert(cmdInfo, "CrossCooldown: <i>"..Parser:formatTime(playerCmd.CrossCooldown).."</i>")
		--			end
		--		end
				
		--		plr:makeUI("ADONIS_LIST", {
		--			Title = "E. Command - "..cmdMatch;
		--			Table = cmdInfo;
		--			Stacking = false;
		...
		openPlayersList = {
			...
			Function = function(plr, args)
				...
				local gameCreatorId = (game.CreatorType==Enum.CreatorType.User and game.CreatorId) or
					service.getGroupCreatorId(game.CreatorId)
				...
				for i, target in ipairs(players) do
					...
					local tagPrefix = (isFriends and "<font color='#51d93f'>[Friend]</font>".." ") or
						(isCreator and "<font color='#51d93f'>[Game Creator]</font>".." ")
					local deviceInfo = clientData and
						(if clientData.deviceType == "VR" then "📷 VR" elseif clientData.deviceType == "Console" then "🎮 Console" elseif clientData.deviceType == "Mobile"
							then "📱 Mobile" elseif clientData.deviceType == "PC" then "💻 PC" else "Unknown") 
						or "[unknown device or hasn't loaded yet]"
						
					
					local inGameDuration = clientData and Parser:formatTime(tick()-clientData.joined) or "<i>Not registered in the system</i>"
					...
					table.insert(tab, {
						...
						description = (isAGhost and "Player "..targetDisplayAndUser.." exists in the server, but hides in the game\n" or "") ..
							"Duration: "..inGameDuration;
						richText = true;
					})
		...
		handToPlayer = {
			...
			Function = function(plr, args)
				...
				if not hasOverridePerm and (target:getVar("HandToPrompt") or plr:getVar("HandToPrompt")) then
					plr:sendData("SendMessage", "HandTo Error", "You or "..target.Name.." is currently in a HandTo prompt. Unable to proceed with the HandTo prompt if there's an on-going session. Sorry!", 10, "Hint")
					return
				end
				...
				if not hasOverridePerm then
					...
					local targetTorso = targetChar and (targetChar:FindFirstChild"Torso" or targetChar:FindFirstChild"HumanoidRootPart")
					local playerTorso = playerChar and (playerChar:FindFirstChild"Torso" or playerChar:FindFirstChild"HumanoidRootPart")
				
					if not (targetTorso and playerTorso) then
						plr:sendData("SendMessage", "HandTo Error", "Unable to check character proximity. You and the target must have a torso.", 10, "Hint")
						return
					end
					...
					if theirCharDist > acceptableDist then
						plr:sendData("SendMessage", "HandTo Error", "You are not close to your target. If you want to override proximity check, you must have permission <b>HandTo_Utility</b>.", 10, "Hint")
						return
					end
				end
				...
				if not backpack then
					plr:sendData("SendMessage", "HandTo Error", "You must have a backpack to continue this process.", 5, "Hint")
					return
				end
				
				if not targetStoreLocation or targetStoreLocation.Parent == nil then
					plr:sendData("SendMessage", "HandTo Error", target.Name.." doesn't have a place to store the item.", 5, "Hint")
					return
				end
				...
				local didDebounce = service.debounce("HandTo-"..plr.playerId.."-"..target.playerId, function()
					if not hasOverridePerm then
						...
						plr:sendData("SendMessage", "Handling HandTo process..", "Sent a request to "..target.DisplayName.." (@"..target.Name.."). Please wait", 50, "Hint")
						local didPass = false
						...
						if openedNotif then
							plr:sendData("SendMessage", "Handling HandTo process..", target.DisplayName.." (@"..target.Name..") opened the prompt. Waiting for their confirmation", 10, "Hint")
							
							local checkConfirm = target:customGetData(12, "MakeUI", "Confirmation", {
								title = "HandTo Confirmation";
								desc = "Would you like to accept the tool "..currentTool.Name:sub(1,30).." from "..plr.DisplayName.." (@"..plr.Name..")?";
							...
							if checkConfirm==1 then
								didPass = true
								plr:sendData("SendMessage", "HandTo process successful", target.DisplayName.." (@"..target.Name..") accepted your request. The tool went to your backpack/character.", 10, "Hint")
								currentTool.Parent = targetStoreLocation
							else
								plr:sendData("SendMessage", "HandTo process failed", target.DisplayName.." (@"..target.Name..") declined the confirmation. The tool was returned to your backpack.", 10, "Hint")
							end
						else
							plr:sendData("SendMessage", "HandTo process failed", target.DisplayName.." (@"..target.Name..") declined your request. The tool was returned to your backpack.", 10, "Hint")
						end
						...
						plr:sendData("SendMessage", "HandTo Success", "Successfully gave the tool to "..target.DisplayName.." (@"..target.Name..")", 5, "Hint")
						wait(.5)
						currentTool.Parent = targetStoreLocation
					end
				end)
				...
				if not didDebounce then
					plr:sendData("SendMessage", "HandTo Error", "You are already in a session with "..target.Name..".", 5, "Hint")
				end
			end;
		};
		...
		joinPlayer = {
			...
			Function = function(plr, args)
				...
				if targetPData.__clientSettings.IncognitoMode and not Moderation.checkAdmin(plr) then
					plr:sendData("SendMessage", `Player {args[1]} has enabled Incognito mode. You are forbidden to join their server unless you are an in-game administrator`, nil, 5, "Context")
					return
				end
				...
				if not serverDetails then
					...
					if accessCode then
						...
						if privateServerData then
							local canJoinServer = privateServerData.creatorId == plr.UserId or not privateServerData.inviteOnly or 
								(Identity.checkTable(plr, privateServerData.whitelist) and not Identity.checkTable(plr, privateServerData.banlist))
							
							if not canJoinServer then
								plr:sendData("SendMessage", "Player "..args[1].." is in a Essential-reserved server which requires a teleport signature. You cannot join this server unless you have permission.", nil, 5, "Context")
							else
					...
					elseif serverDetails.privateServer then
						plr:sendData("SendMessage", "Player "..args[1].." is in a private/personal server. You cannot join this server.", nil, 5, "Context")
					else
						plr:sendData("SendMessage", "Joining player "..args[1], nil, 5, "Context")
		...
		showScreenshotHud = {
			...
			Function = function(plr, args)
				plr:sendData("showScreenshotHud", args[1] or false)
			end;
		};
		...
		hideScreenshotHud = {
			...
			Function = function(plr, args)
				plr:sendData("hideScreenshotHud")
			end;
		};
