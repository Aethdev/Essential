	...
	local cmdsList = {
		gravity = {
			...
			Function = function(plr, args)
				...
				if args[1] and args[1]:lower() == "fix" then
					newGravity = variables.savedEnvironment.gravity
				end
		...
		gameTime = {
			...
			Function = function(plr, args)
				service.Lighting.ClockTime = args[1]
			end;
		};
		...
		btools = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", "<b>Disclaimer</b: You are using an unofficial build of Building Tools, created by trzistan." ..
					" In the open-source version, this tool will be replaced with the original building tools from GigsD4X.", nil, 5, "Context")
			...
			Function = function(plr, args)
				...
				if latest then
					...
					if didLoad then
						...
						plr:sendData("SendMessage", "Map Management", "There was no map backups available to load. Please try again later!", 6, "Hint")
					end
				else
					plr:sendData("SendMessage", "Map Management", "Unable to restore map without a latest/specified backup. Backup the map before performing a map restore.", 6, "Hint")
				end
			end;
		};
		...
		backupMap = {
			...
			Function = function(plr, args)
				if args[2] then
					...
					if #args[2] > 60 then
						plr:sendData("SendMessage", "Map Management", "Backup name cannot exceed 60 characters", 8, "Hint")
						return
					end
				end
				...
				if backupData then
					...
					plr:sendData("SendMessage", "Map Management", "Backed up map successfully. You can load this backup <b>"..backupData.name.."</b>.", 12, "Hint")
				end
			end;
		};
			...
			Function = function(plr, args)
				...
				if backupData then
					...
					if didLoad then
						...
						plr:sendData("SendMessage", "Map Management", "Initial backup couldn't load due to an on going backup/restoration process.", 6, "Hint")
					end
				else
		...
		viewMapBackups = {
			...
			Function = function(plr, args)
				...
				if #backups == 0 then
					plr:sendData("SendMessage", "Map Management", "There are no backups available to be seen. Make sure to backup the map to see one.", 4, "Hint")
				else
					...
					if not viewMBSession then
						...
						loadBEvent._event:Connect(function(caller, backupName)
							if caller == plr and Roles:hasPermissionFromMember(plr, {"Manage_Map"}) and type(backupName) == "string" then
								local backup = (function()
									for i,bData in pairs(backups) do
										if bData.name == backupName then
											return bData
										end
									end
								end)()
								...
								if backup then
									plr:sendData("SendMessage", "Map Management", "Loading backup <b>"..backupName.."</b>..", 4, "Hint")
									Utility:loadMapBackup(backupName)
									plr:sendData("SendMessage", "Map Management", "Successfully loaded backup", 4, "Hint")
								end
							end
						end)
								...
								if foundBackup then
									plr:makeUI("List", {
										...
										List = {
											...
											{
												type = "Label";
												label = `Creator: <b>{tostring(foundBackup.creatorFullName or "-SYSTEM-")}</b>`;
												richText = true;
												selectable = true;
											};
											{
												...
												options = {
													{
														label = "Load";
														onExecute = `sessionevent://main:{viewMBSession.id}-{loadBEvent.id}||{LuaParser.Encode{foundBackup.name}}`
													}
												};
											};
										};
							...
							options = {
								{
									...
									onExecute = `sessionevent://main:{viewMBSession.id}-{viewBEvent.id}||{LuaParser.Encode{backup.name}}`
								};
								{
									label = "Load";
									backgroundColor = Color3.fromRGB(36, 168, 34);
									onExecute = `sessionevent://main:{viewMBSession.id}-{loadBEvent.id}||{LuaParser.Encode{backup.name}}`
								};
							};
						})
					end
				...
				service.startLoop("DiscoMap", 0.1, function()
					if colorC.Parent == service.Lighting then
						local tween = service.TweenService:Create(
							colorC,
							TweenInfo.new(tweenTime, Enum.EasingStyle.Quint),
							{
