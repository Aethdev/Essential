	...
	local cmdsList = {
			...
			Function = function(plr, args)
				...
				if not target.Character then
					...
					if not humanoid then
						plr:sendData("SendMessage", target.Name.."'s character doesn't have a humanoid.", nil, 4, "Context")
						--plr:sendData("SendMessage", "Camera management", target.Name.." doesn't have a humanoid.", 6, "Hint")
					else
						plr:sendData("SendMessage", "Now focusing on "..target.Name.."'s character.", nil, 4, "Context")
		...
		forceCameraForPlayers = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", ((#args[2] > 6 and tostring(#args[2]).." players")) or args[2]("concat")(", ").." are now focused to "..focusedPlr.Name..".", nil, 8, "Context")
			end,
		};
					...
					if playerGui then
						...
						elseif remoteFunc then
							service.threadTask(function()
								remoteFunc:InvokeClient(target._object, "Enable")
							end)
						end
					end
				end
				...
				for i, target in pairs(args[1]) do
					...
					if playerGui then
						...
						if freecam then
							...
							elseif remoteFunc then
								service.threadTask(function()
									remoteFunc:InvokeClient(target._object, "Disable")
								end)
							end
						end
					end
				...
				for i, target in pairs(args[1]) do
					...
					if playerGui then
						...
						if freecam then
							...
							elseif remoteFunc then
								service.threadTask(function()
									remoteFunc:InvokeClient(target._object, "Toggle")
								end)
							end
						end
					end
		...
		removeFreeCam = {
			...
			Function = function(plr, args)
				...
				if #concatPlayers > 0 and plr then
					plr:sendData("SendMessage", "Removed <b>players freecam</b> from "..table.concat(concatPlayers, ", ")..".", nil, 10, "Context")
				end
			end,
		};
	}
