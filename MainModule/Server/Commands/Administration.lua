	...
	local cmdsList = {
		cmdBlacklist = {
			...
			Function = function(plr, args)
				...
				if not specifiedCmd then
					plr:sendData("SendMessage", "Command blacklist error", "Command <b>"..args[2].."</b> doesn't exist.", 6, "Hint")
				else
					local successPlayers = {}
					local cmdBlacklist = specifiedCmd.Blacklist or (function()
						local tab = {}
					...
					if #successPlayers > 0 then
						plr:sendData("SendMessage", "Command blacklist success", "Blacklisted "..(#successPlayers).." players from the command: "..table.concat(successPlayers, 1, 50), 8)
					else
						plr:sendData("SendMessage", "Command blacklist failed", "There were nobody to blacklist. Make sure they're not already blacklisted", 8)
					end
				end
			end;
		};
		...
		unCmdBlacklist = {
			...
			Function = function(plr, args)
				...
				if not specifiedCmd then
					...
					if #successPlayers > 0 then
						plr:sendData("SendMessage", "Command blacklist success", "UnBlacklisted "..(#successPlayers).." players from the command: "..table.concat(successPlayers, 1, 50), 8)
					else
						plr:sendData("SendMessage", "Command blacklist failed", "There were nobody to unblacklist. Make sure they're not already unblacklisted", 8)
					end
				end
			end;
		};
		...
		serverCommandBlacklist = {
			...
			Function = function(plr, args)
				...
				if #successPlayers > 0 then
					plr:sendData("SendMessage", "GCommand blacklist success", "GBlacklisted "..(#successPlayers).." players from the server: "..table.concat(successPlayers, 1, 50), 8)
				else
					plr:sendData("SendMessage", "GCommand blacklist failed", "There were nobody to blacklist. Make sure they're not already blacklisted", 8)
				end
			end;
		};
		...
		unServerCommandBlacklist = {
			...
			Function = function(plr, args)
				...
				if #successPlayers > 0 then
					plr:sendData("SendMessage", "GCommand blacklist success", "UnGBlacklisted "..(#successPlayers).." players from the server: "..table.concat(successPlayers, 1, 50), 8)
				else
					plr:sendData("SendMessage", "GCommand blacklist failed", "There were nobody to unblacklist. Make sure they're not already unblacklisted", 8)
				end
			end;
		};
		...
		systemCommandBlacklist = {
			...
			Function = function(plr, args)
				...
				if #successPlayers > 0 then
					plr:sendData("SendMessage", "DCommand blacklist success", "Data-Blacklisted "..(#successPlayers).." players from the server: "..table.concat(successPlayers, 1, 20), 8)
				else
					plr:sendData("SendMessage", "DCommand blacklist failed", "There were nobody to data-blacklist. Make sure they're already undata-blacklisted", 8)
				end
			end;
		};
		...
		unSystemCommandBlacklist = {
			...
			Function = function(plr, args)
				...
				if #successPlayers > 0 then
					plr:sendData("SendMessage", "DCommand blacklist success", "Data-UnBlacklisted "..(#successPlayers).." players from the server: "..table.concat(successPlayers, 1, 50), 8)
				else
					plr:sendData("SendMessage", "DCommand blacklist failed", "There were nobody to data-unblacklist. Make sure they're already data-unblacklisted", 8)
				end
			end;
		};
		...
		viewPlayerInfo = {
			...
			Function = function(plr, args)
				...
				local targetInfo = {
					...
					"";
					"<b>Team</b>:";
					tostring((target.Team~=nil and "<font color='#"..target.Team.TeamColor.Color:ToHex().."'>"..target.Team.Name.."</font>") or "<u>not assigned</u>");
					"";
					"Joined on <b>"..server.Parser:osDate(os.time()-(plr.AccountAge*86400)).."</b>";
					"Membership type: "..tostring(target.MembershipType==Enum.MembershipType.Premium and "Premium" or "NonPremium");
					"----";
				...
				for i,roleInfo in pairs(Roles:getRolesFromMember(target)) do
					local canShowToViewer = not roleInfo.hiddenfromlist and (not roleInfo.hidelistfromlowranks or roleInfo.priority <= highestRole.priority)
				...
				if otherRoleCount > 0 then
					table.insert(targetInfo, "Roles joined: "..table.concat(joinedRoles, ", ").." +"..(otherRoleCount-20).." more")
				else
					table.insert(targetInfo, "Roles joined: <i>No roles to show. Either roles are not allowed to appear in your visibility or do not exist.</i>")
				end
			...
			Function = function(plr, args)
				...
				for i,target in pairs(args[1]) do
					...
					if playerPriority > targetPriority then
						task.spawn(function()
							local didProcessSudo = Process.playerCommand(target, args[2], {noReturn = true; dontLog = true;})

							if didProcessSudo then
								Logs.addLog("Admin", "Player "..plr.Name.." sudoed "..target.Name..": "..tostring(args[2]))
							end
						end)
					end
				end
		...
		forcePlacePlayer = {
			...
			Function = function(plr, args)
				...
				return Promise.each(successPlayers, function(target, index)
					...
					parsedTarget:sendData("SendMessage", "🏢 Teleporting to place <b>"..tostring(placeInfo.Name or args[2]).."</b>", nil, 20, "Context")
					...
					:catch(Logs.Reporters.Promise.issue(`ForcePlace`, "Process"))
			end;
		};
		...
		forceRejoin = {
			...
			Function = function(plr, args)
				...
				return Promise.each(successPlayers, function(target, index)
					...
					if args[2] then
						tpOptions.ShouldReserveServer = false
					end
		...
		clearActivityLogs = {
			...
			Function = function(plr, args)
				...
				if #successIds > 0 then
					plr:sendData("SendMessage", "Successfully cleared activity logs from <b>"..(#successIds).." players</b>", table.concat(successIds, ", "), 8, "Hint")
				end
			end;
		};
		...
		showPlayerActivityLogs = {
			...
			Function = function(plr, args)
				...
				table.sort(activeLogs, function(logA, logB)
					return (logA.sentOs or 0) > (logB.sentOs or 0)
				end)
		...
		showShutdownLogs = {
			...
			Function = function(plr, args)
				...
				if type(shutdownLogs) ~= "table" then
					shutdownLogs = {}
				end
				...
				for i,shutdownD in pairs(shutdownLogs) do
					if type(shutdownD) == "table" then
						...
						table.insert(tab, {
							...
							desc = `Server Duration: {Parser:formatTime((shutdownD.started or 0)-serverStarted)} | Reason: {tostring(reason)}`;
							sentOs = shutdownD.started or 0;
						})

						table.sort(tab, function(a, b)
							return a.sentOs > b.sentOs
						end)
					end
				end
			...
			Function = function(plr, args)
				if not server.Utility.shuttingDownState then
					service.trackTask(`Log Shutdown {plr.UserId}`, false, function()
						...
						plr:sendData("SendMessage", "Successfully logged shutdown. Now closing the server..", nil, 5, "Context")
					end)
			...
			Function = function(plr, args)
				if not server.Utility.shuttingDownState then
					if server.Studio or game.PrivateServerOwnerId>0 then
						plr:sendData("SendMessage", "❌ Studio & personal servers cannot permit soft shutdown.", nil, 5, "Context")
						return
					end
					...
					service.trackTask(`Log SoftShutdown {plr.UserId}`, false, function()
						...
						plr:sendData("SendMessage", "Successfully logged soft shutdown. Now preparing soft shutdown..", nil, 5, "Context")
					end)
					...
					plr:sendData("SendMessage", "Success! This may take a while to have everyone teleport to the new server..", nil, 5, "Context")

					for i, target in ipairs(service.getPlayers(true)) do
						local playerDisplayName = plr:toStringDisplayForPlayer(target)
						target:sendData("SendMessage", "Moderator <u>"..playerDisplayName.."</u> performed a soft shutdown. You are being teleported to the new server.", nil, 30, "Context")
						target:teleportToReserveWithSignature(reservedServerDetails.serverAccessId, function()
		...
		crossShutdown = {
			...
			Function = function(plr, args)
				plr:sendData("SendMessage", "Successfully sent shutdown command to all servers. This may take a while to register in servers.", nil, 5, "Context")
		...
		crossCommand = {
			...
			Function = function(plr, args)
				plr:sendData("SendMessage", "Successfully sent command to all servers. This may take a while to register in servers.", nil, 5, "Context")
		...
		crossMessage = {
			...
			Function = function(plr, args)
				Cross.send("PublishMessage", "Broadcast from <b>"..plr.Name.."</b>", args[1], math.clamp(#args[1]*0.1, 3, 30))
			end;
		};
			...
			Function = function(plr, args)
				...
				crossEvent:connect(function(jobId, receiverId, replyMsg, expireOs, scheduledOs)
					...
					replyPm.replied:connectOnce(function(newReply)
						...
						Cross.send("PrivateMessage", "Cross Message from "..plr.Name, filterMsg:sub(1,300), expireOs, nil, {Name = plr and plr.Name; UserId = plr and plr.UserId}, "@"..receiverName, eventId)
					end)
				end)

				Cross.send("PrivateMessage", "Cross Message from "..plr.Name, args[2]:sub(1,300), expireOs, nil, {Name = plr and plr.Name; UserId = plr and plr.UserId}, args[1], eventId)
			end;
		};
		...
		crossChat = {
			...
			Function = function(plr, args)
				Cross.send("CrossChat", plr.Name, args[1])
			end;
		};
		...
		whitelist = {
			...
			Function = function(plr, args)
				if args[1]==nil then
					plr:sendData("SendMessage", "Whitelist", "Status: "..tostring(variables.whitelistData.enabled), 5, "Hint")
				else
					plr:sendData("SendMessage", "Whitelist", "Set status to <b>"..tostring(args[1]).."</b>", 5, "Hint")
		...
		whitelist_addPlayer = {
			...
			Function = function(plr, args)
				...
				if #successPlayers > 0 then
					plr:sendData("SendMessage", "Added player to whitelist", tostring((#successPlayers < 6 and table.concat(successPlayers, ", ")) or #successPlayers.." players").." is/are added to the whitelist", 5, "Hint")
				end
			end;
		};
		...
		whitelist_removePlayer = {
			...
			Function = function(plr, args)
				...
				if #successPlayers > 0 then
					plr:sendData("SendMessage", "Removed players from whitelist", tostring((#successPlayers < 6 and table.concat(successPlayers, ", ")) or #successPlayers.." players").." is/are added to the whitelist", 5, "Hint")
				end
			end;
		};
		...
		whitelist_adminOnly = {
			...
			Function = function(plr, args)
				...
				if args[1] == nil then
					plr:sendData("SendMessage", "Whitelist status", "OnlyAdmins: "..tostring(whitelistTable.admins), 5, "Hint")
				else
					whitelistTable.admins = args[1]
					plr:sendData("SendMessage", "Whitelist status", "Set OnlyAdmins to "..tostring(args[1]), 5, "Hint")
				end
			end;
		};
		...
		clearPlayerData = {
			...
			Function = function(plr, args)
				...
				for i,username in pairs(args[1]) do
					...
					if userId and userId > 0 then
						local stat = Datastore.remove("PData_"..settings.Datastore_PlayerData:sub(1,44), tostring(userId))
				...
				if #successIds > 0 then
					plr:sendData("SendMessage", "Successfully cleared player datas from <b>"..(#successIds).." players</b>", table.concat(successIds, ", "), 8, "Hint")
				end
			end;
		};
		...
		warnPlayer = {
			...
			Function = function(plr, args)			
				...
				if #successPlayers > 0 then
					plr:sendData("SendMessage", "Successfully warned "..#successPlayers.." players for "..Parser:filterForRichText(args[2]), nil, 5, "Context")
				end
			end;
		};
		...
		removeWarnings = {
			...
			Function = function(plr, args)
				...
				if removeCount > 0 then
					targetPData._updateIfDead()
					plr:sendData("SendMessage", "Successfully removed warning <i>"..args[2].."</i> from <b>"..targetName.."</b>", nil, 10, "Context")
				end
			end;
		};
		...
		viewWarnings = {
			...
			Function = function(plr, args)
				...
				if targetId > 0 then
					...
					table.sort(warnings, function(new, old)
						return new.created < old.created
					end)

					for i,warnInfo in pairs(warnings) do
						if not (not warnInfo.category or warnInfo.category == "player") then
							continue
						end
						...
						table.insert(list, {
							...
							desc = `Type: {warnInfo.category}\nId: {warnInfo.id}\nModerator: {warnInfo.moderator.name} ({tostring(warnInfo.moderator.userId)})`;	
		...
		clearWarnings = {
			...
			Function = function(plr, args)
				...
				if type(warnings) == "table" then
					...
					plr:sendData("SendMessage", "Successfully cleared <b>"..args[1].."</b>'s warnings", nil, 10, "Context")
				end
			end;
		};
		...
		kickPlayers = {
			...
			Function = function(plr, args)
				...
				if #successPlayers > 0 and plr then
					plr:sendData("SendMessage", "Successfully kicked "..((#concatPlayers < 6 and table.concat(concatPlayers, ", ") or #concatPlayers).." players")..".", nil, 10, "Context")
		...
		resolveBanForSpecificPlayers = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", `Resolved ban case {args[2]} for {#args[1]} player(s). If this is a universal ban case, please wait 30-90 seconds for it to take effect in the game.`, nil, 15, "Context")
			end;
		};
		...
		resolveBan = {
			...
			Function = function(plr, args)
				...
				plr:sendData("SendMessage", `Resolved ban case {args[1]} for {#banCase.users} player(s). If this is a universal ban case, please wait 30-90 seconds for it to take effect in the game.`, nil, 15, "Context")
			end;
		};
		...
		universalBan = {
			...
			Function = function(plr, args, data)
				...
				if args[3].total > 0 and args[3].total < 60 then
					plr:sendData("SendMessage", `You must specify at a least minute for the time duration of the universal ban or 0 seconds for <b>PERMANENT</b>.`, nil, 10, "Context")
					return
				end
				
				local ignoredPlayers = {}
				local ignorePriorityLevel = server.Studio
				local playerPriority = not ignorePriorityLevel and server.Roles:getHighestPriorityFromPermission(plr, "Ban_Player")
				...
				if #ignoredPlayers > 0 then
					plr:makeUI("NotificationV2", {
						...
						desc = "Unable to ban players with higher/equal priority level: "..table.concat(ignoredPlayers, " ");
					})
				end
				...
				Moderation.createBan({
					...
					plr:sendData("SendMessage", `Successfully created ban case {caseId} for {#args[1]} player(s)`
						.. (if args[3].total == 0 then `.` else ` It will expire on \{\{t:{(os.time()+args[3].total)}\}\}.`), nil, 10, "Context")
				end)
			end;
		};
		...
		serverBan = {
			...
			Function = function(plr, args, data)
				...
				return universalBanCommand.Function(plr, {args[1], false, args[2], args[3]}, setmetatable({isServer = true}, {__index = data or {}}))
			end;
		};
		...
		listBans = {
			...
			Function = function(plr, args)
				...
				for i, minimizedBanCase: {
					caseId: string; moderatorId: string; users: {[number]: number}; reason: string;
					startedOn: number;
					expiresOn: number?;
				} in globalBans do
					local isActive = if minimizedBanCase.expiresOn then minimizedBanCase.expiresOn-nowTimestamp.UnixTimestampMillis > 0 else true
						...
						options = {
							{
								...
								onExecute = "playercommand://"..server.Commands.getIdFromCommand(settings.actionPrefix..`lookupban`).."||"..minimizedBanCase.caseId;
							};
							{
								label = "Resolve";
								backgroundColor = Color3.fromRGB(115, 143, 22);
								onExecute = "playercommand://"..Cmds.get(settings.actionPrefix.."resolveban").Id.."||"..minimizedBanCase.caseId
							}
							--[number]: {
							--	label: string,
							--	textColor: Color3,
		...
		lookupBanCase = {
			...
			Function = function(plr, args)
				if args[1][1]:sub(1,2):lower() == `l-` then
					plr:sendData("SendMessage", `Looking up legacy bans (pseudo ban case ids starting with <b>L-</b>) are not possible for lookups.`, nil, 10, "Context")
					return
				end
				
				plr:sendData("SendMessage", `Looking up ban case {args[1][1]}..` .. (if args[1][1]:sub(1,2) ~= `S-` then ` <i>It will take 10-45 seconds to look up.</i>` else ``), nil, 5, "Context")
				...
				local isBanCaseExpired = if not banCase.expiresOn then false else DateTime.now().UnixTimestampMillis-banCase.expiresOn >= 0
				local banCaseNotes = service.cloneTable(banCase.notes)
				local concatNotes = {}
				
				table.sort(banCaseNotes, function(noteA, noteB)
					return noteA[2] > noteB[2]
				end)
				...
				local concatUsers = {}
				local resolveBanCommandId = server.Commands.getIdFromCommand(settings.actionPrefix..`resolvebanforplayers`)
				
				for i, userId in banCase.users do
					table.insert(concatUsers, if table.find(banCase.releasedUsers, userId) or isBanCaseExpired then {
						type = "Label";
						...
						options = {
							{
								...
								onExecute = "playercommand://"..resolveBanCommandId.."||@"..service.playerNameFromId(userId);
							};
						};
						selectable = true;
					})
				...
				plr:makeUI("List", {
					...
					List = service.mergeTables(true, {
						{
							type = "Label";
							label = `Moderator: ` .. (if banCase.moderatorId == 0 then `System`
								elseif banCase.moderatorId == -1 then `AutoModerator`
								else service.playerNameFromId(banCase.moderatorId)..` ({banCase.moderatorId})`);
							selectable = true;
						};
						{
							type = "Label";
							label = `Started on \{\{t:{math.floor(banCase.startedOn/1000)}\}\}` ..
								(if not banCase.expiresOn then `` else ` and end{if isBanCaseExpired then "ed" else "s"} \{\{t:{math.floor(banCase.expiresOn/1000)}:rt\}\}`);
							richText = true;
							selectable = true;
							specialMarkdownSupported = true;
						};
							...
							options = {
								{
									label = "Resolve";
									backgroundColor = Color3.fromRGB(241, 117, 55);
									onExecute = "playercommand://"..server.Commands.getIdFromCommand(settings.actionPrefix.."resolveban").."||"..args[1][1];
								};
							};
							selectable = true;
						} else nil
		...
		unBan = {
			...
			Function = function(plr, args)
				...
				local checkConfirm = plr:makeUIGet("Confirmation", {
					...
					desc = "Are you sure you want to unban "..tostring(table.concat(players, ", ").."?\n<b>The action cannot be permanently undone.</b>");
				...
				for i,target in pairs(players) do
					local targetUserId = service.playerIdFromName(target)
					local removedBan = Moderation.removeBan(targetUserId, args[2], {name = plr.Name; userId = plr.UserId;})

					if removedBan then
						table.insert(unbanResults, target)
					end
				end

				if #unbanResults > 0 then
					plr:sendData("SendMessage", "Successfully unbanned "..((#unbanResults > 5 and #unbanResults.." players") or table.concat(unbanResults, ", "))..".", nil, 10, "Context")
					--plr:sendData("SendMessage", "Successfully unbanned", "("..(#unbanResults).." players) "..table.concat(unbanResults, ", ")..".", 5, "Hint")
				else
					plr:sendData("SendMessage", "There were nobody unbanned. The players listed are already unbanned from the specified ban type.", nil, 4, "Context")
				end
			end;
		};
		...
		legacy_checkBan = {
			...
			Function = function(plr, args)
				...
				elseif banCheck then
					if not banInfo then
						plr:sendData("SendMessage", "Ban status", "Player "..args[1].." is banned, but doesn't have any information.", 6, "Hint")
					else
						local messageInfo = {}
						...
						if settingBan then
							...
							table.insert(messageInfo, "Moderator: "..banInfo.moderator.name.." (<b>"..banInfo.moderator.userid.."</b>)")
							table.insert(messageInfo, "-----")
							table.insert(messageInfo, "Registered: <b>"..Parser:osDate(banInfo.registered).."</b>")
							table.insert(messageInfo, "Expire Os: "..(banInfo.expireTime and Parser:osDate(banInfo.expireTime) or "<font color='#e64940'>N/A</font>"))
							table.insert(messageInfo, "-----")
						...
						plr:sendData("SendMessage", "Ban Information for "..args[1], table.concat(messageInfo, "\n"), 30)
					end
				end
			end;
		};
		...
		manageGameServers = {
			...
			Function = function(plr, args)
				plr:sendData("SendMessage", "Game servers management", "Retrieving list of servers. This will take up to 15 seconds.", 15, "Hint")
				...
				if not manageGSSession then
					...
					shutdownServerEvent._event:Connect(function(caller, chosenServerId)
						if caller == plr and Roles:hasPermissionFromMember(plr, {"Manage_Game_Servers"}) and server.Utility:checkRate(sessionRL, caller.UserId) and type(chosenServerId) == "string" then
							plr:sendData("SendMessage", "Game servers management", "Attempting to shutdown "..chosenServerId..". This may take up to 15 seconds.", 18, "Hint")
							...
							for i,serverInfo in pairs(serversList) do
								if type(serverInfo) == "table" then
									...
									if serverId and serverId == chosenServerId and not serverInfo.studio then
										Cross.sendToSpecificServers({serverId}, "Shutdown", "Requested by "..playerDisplayName, 2, plr.UserId)
										plr:sendData("SendMessage", "Cross shutdown command sent!", "Shutting down game server <b>"..serverId.."</b>", 10, "Hint")
										Logs.addLog("Admin", {
											title = plr.Name.." sent a cross request to server "..tostring(serverId).." for shutdown";
							...
							if not didShutdown then
								plr:sendData("SendMessage", "Cross shutdown command failed to send.", "There was no server with id "..chosenServerId.." to shutdown.", 4, "Hint")
							end
						end
					end)
											...
											Children = {
												{
													...
													BackgroundColor3 = Color3.fromRGB(77, 77, 77);
													TextColor3 = Color3.fromRGB(222, 222, 222);
													Text = (foundServer.private and foundServer.privateId) or tostring(foundServer.id);
													TextEditable = false;
													ClearTextOnFocus = false;
												}
											};
										};
										not foundServer.studio and {
											Class = "TextButton";
											Size = UDim2.new(1, -10, 0, 30);
											Position = UDim2.new(0, 5, 0, 30*4);
											BackgroundTransparency = .4;
											BackgroundColor3 = Color3.fromRGB(77, 77, 77);
											TextColor3 = (foundServer.studio and Color3.fromRGB(162, 162, 162)) or Color3.fromRGB(222, 222, 222);
				...
				for i,serverInfo in pairs(serversList) do	
					if type(serverInfo) == "table" then
						...
						local formatTime = server.Parser:formatTime(timeData.hours, timeData.mins, timeData.secs)
						local serverId = (serverInfo.private and serverInfo.privateId) or serverInfo.id
						local isCurrentServer = (#game.PrivateServerId > 0 and variables.serverInfo.privateId==serverId) or variables.serverInfo.id==serverId
						...
						table.insert(tabList, {
							...
							BackgroundTransparency = 1;
							TextXAlignment = "Left";
							Text = (isCurrentServer and "🔸 ") or "" .. "["..formatTime.."] "..(serverInfo.studio and "[studio server]" or serverId);
		...
		disableCommands = {
			...
			Function = function(plr, args)
				...
				if #failedCmds > 0 then
					...
					plr:sendData("SendMessage", "Command Management failed", "Failed to disable "..failCount.." cmds such as "..table.concat(failedCmds, ", ")..".", 3, "Hint")
					wait(3)	
				end

				local successCount = service.tableCount(successCmds)
				if successCount > 0 then
					plr:sendData("SendMessage", "Command Management success", "Disabled "..successCount.." cmds such as "..table.concat(successCmds, ", ")..".", 5, "Hint")
				end
			end;
		};
		...
		enableCommands = {
			...
			Function = function(plr, args)
				...
				if #failedCmds > 0 then
					local failCount = service.tableCount(failedCmds)
					plr:sendData("SendMessage", "Command Management failed", "Failed to enable "..failCount.." cmds such as "..table.concat(failedCmds, ", ")..".", 3, "Hint")
					wait(3)	
				end

				local successCount = service.tableCount(successCmds)
				if successCount > 0 then
					plr:sendData("SendMessage", "Command Management success", "Enabled "..successCount.." cmds such as "..table.concat(successCmds, ", ")..".", 5, "Hint")
				end
			end;
		};
		...
		promptProductPurchase = {
			...
			Function = function(plr, args)
				...
				elseif not productInfo.CanBeSoldInThisGame then
					plr:sendData("SendMessage", "Product <b>"..productId.."</b> cannot be sold in the game", nil, 5, "Context")
					return
				end
		...
		promptGamePassPurchase = {
			...
			Function = function(plr, args)
				...
				elseif not productInfo.CanBeSoldInThisGame or not productInfo.IsForSale then
					plr:sendData("SendMessage", "GamePass <b>"..productId.."</b> cannot be sold in the game nor available for sale", nil, 5, "Context")
					return
				end
				
				local productCreatorUserId = (productInfo.CreatorType==Enum.CreatorType.User and productInfo.Creator.CreatorTargetId) or
					(productInfo.CreatorType==Enum.CreatorType.Group and service.getGroupCreatorId(productInfo.Creator.CreatorTargetId)) or 0
				local inGameCreatorId = (game.CreatorType==Enum.CreatorType.Group and service.getGroupCreatorId(game.CreatorId)) or game.CreatorId
		...
		promptAssetPurchase = {
			...
			Function = function(plr, args)
				...
				if not productInfo or not productInfo.Created then
					...
					return
				elseif not productInfo.CanBeSoldInThisGame or not productInfo.IsForSale then
					plr:sendData("SendMessage", "Asset <b>"..productId.."</b> cannot be sold in the game nor available for sale", nil, 5, "Context")
					return
				end
