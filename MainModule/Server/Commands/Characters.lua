	...
	local cmdsList = {
				...
				for userId,jailInfo in pairs(variables.jailedPlayers) do
					if jailInfo.active then
						...
						table.insert(jailList, {
							...
							description = "Duration: "..Parser:formatTime(tick()-jailInfo.started).." | Items: "..tostring(#jailInfo.items);
						})
					end
				end
			...
			Function = function(player, args)
				...
				for userId,jailInfo in pairs(variables.jailedPlayers) do
					if not args[1] or (args[1] and args[1]("getPlayer", jailInfo.suspectId)) then
						...
						local target = Parser:getParsedPlayer(targetId) or Parser:apifyPlayer({
							Name = targetName;
							UserId = targetId;
						}, true)
				...
				if jailCount > 0 then
					player:sendData("SendMessage", "Jail Management", "Removed "..tostring(jailCount).." jail(s).", 8, "Hint")
				end
			end,
		};
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					...
					if char then
						...
						if humanoid then
							humanoid.WalkSpeed = math.clamp(args[2], 0, math.huge)
						end
					end
				end
			end;
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					...
					if char then
						...
						if humanoid then
							humanoid.JumpPower = math.clamp(args[2], 0, math.huge)
						end
					end
				end
			end;
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					...
					if char then
						...
						if humanoid then
							humanoid.Health = math.clamp(args[2], 0, math.huge)
						end
					end
				end
			end;
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					...
					if char then
						...
						if humanoid then
							humanoid.MaxHealth = math.clamp(args[2], 0, math.huge)
						end
					end
				end
			end;
				...
				for i,target in pairs(args[1]) do
					...
					if char then
						...
						for d, charPart in pairs(char:GetChildren()) do
							if charPart:IsA"ForceField" then
								service.Debris:AddItem(charPart, 0.5)
							end
						end
		...
		explode = {
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					...
					if char then
						local torso = char:FindFirstChild"HumanoidRootPart" or char:FindFirstChild"Torso" or
						...
						if torso then
							...
							if args[2] then
								expl.BlastPressure = math.abs(args[2])
							end

							if args[3] then
								expl.BlastRadius = math.abs(args[3])
							end

							expl.ExplosionType = (args[4] and Enum.ExplosionType.Craters) or Enum.ExplosionType.NoCraters
							expl.Parent = torso	
						end
					end
				end
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					...
					if char then
						...
						if args[3] then
							service.Debris:AddItem(ff, args[3].total)
						end
		...
		teleport = {
			...
			Function = function(plr, args)
				...
				if not fromPlayer or not fromPlayer.Character then
					if plr then
						plr:sendData("SendMessage", "Teleportation Error", "Teleporting to the player doesn't exist or has no character", 3, "Hint")
					end
				...
				if not mainHrp or not mainHrp:IsA"BasePart" then
					if plr then
						plr:sendData("SendMessage", "Teleportation Error", "Teleporting to the player doesn't have HumanoidRootPart", 3, "Hint")
					end
			...
			Function = function(plr, args)
				...
				if target then
					...
					if targetHrp and targetHrp:IsA"BasePart" then
						...
						mainHrp.CFrame = (targetHrp.CFrame + (targetHrp.CFrame.LookVector*2)) * CFrame.Angles(0, math.rad(180), 0)
					end
				end
			end;
		};
		...
		characterize = {
			...
			Function = function(plr, args)
				...
				local success, desc = pcall(service.Players.GetHumanoidDescriptionFromUserId, service.Players, morphUserId)

				if not success then
					if plr then
						plr:sendData("SendMessage", "Characterization failure", "Unable to get morph data for "..morphUserId.."'s character")
					end
					return
				end
						...
						if torso then
							...
							if tonumber(args[5]) then
								...
								if assetInfo and assetInfo.AssetTypeId == 13 then
									...
									if didFetch then
										particleEmitter.Texture = assetContents[1].Texture
									end
								end
							else
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					...
					if char then
						...
						if char:FindFirstChild"_DEBUG_OUTLINE" then
							char._DEBUG_OUTLINE:Destroy()
						end
						...
						for d,part in pairs(char:GetDescendants()) do
							if part:IsA"BasePart" then
								...
								if part:FindFirstChild"_DEBUG_OUTLINE" then
									part._DEBUG_OUTLINE:Destroy()
								end

								local partOutline = service.New("SelectionBox")
								partOutline.Name = "_DEBUG_OUTLINE"
								partOutline.LineThickness = 0.01
								partOutline.Color3 = (part:FindFirstChildOfClass"Motor6D" and motorPartColor) or outlineColor
		...
		addSoundToCharacters = {
			...
			Function = function(plr, args)
				...
				if not assetInfo or assetInfo.AssetTypeId ~= 3 then
					plr:sendData("SendMessage", "Asset collection error", "<b>"..musicId.."</b> isn't an audio", 6, "Hint")
		...
		removeHats = {
			...
			Function = function(plr, args)
				for i,target in pairs(args[1]) do
					...
					if char then
						for d, charPart in pairs(char:GetChildren()) do
							if charPart:IsA"Accoutrement" then
								service.Debris:AddItem(charPart, 0.5)
							end
						end
					end
				...
				for i,target in pairs(args[1]) do
					...
					if char then
						...
						if clone then
							...
							if clHumanoid then
								clHumanoid.Died:Connect(function()
									service.Debris:AddItem(clone, 5)
								end)
		...
		sizePlayers = {
			...
			Description = "Sizes specified player characters (Size limit: "..tostring(tonumber(settings.sizeLimit) or 10)..")";
			...
			Function = function(plr, args)
				...
				if #failedPlayers > 0 then
					plr:sendData("SendMessage", "Failed to size players due to exceeding size limit: "..(#failedPlayers > 5 and #failedPlayers.." players" or table.concat(failedPlayers, ", ")), nil, 10, "Context")
				end
			end;
		};
			...
			Function = function(plr, args)
				for i, target in pairs(args[1]) do
					if target.Character then
						task.defer(service.convertPlayerCharacterToRig, target._object, "R6")
					end
				end
			end,
		};
		...
		r15Players = {
			...
			Function = function(plr, args)
				for i, target in pairs(args[1]) do
					if target.Character then
						task.defer(service.convertPlayerCharacterToRig, target._object, "R15")
					end
				end
			end,
		};
