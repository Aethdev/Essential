			...
			Function = function(plr, args)
				if server.dexNetwork and server.dexSession then
					if not (server.dexNetwork.connectedPlayers[plr] and server.dexNetwork:getPlayerKey(plr)) then
						...
						plr:sendData(
							"SendMessage",
							"Successfully connected you to dex network. Now creating Dex. <i>This may take a while..</i>",
							nil,
							8,
							"Context"
						)
						...
						elseif dexCode == -2 or dexCode == -3 then
							plr:sendData(
								"SendMessage",
								"Dex creation ran in a roadblock due to network/session unauthorization. Failed to initiate. (error code: "
									.. tostring(dexCode)
									.. ")",
								nil,
								4,
						...
						plr:sendData(
							"SendMessage",
							"You've already connected to the dex network. No need to connect again.",
							nil,
							5,
							"Context"
						)
					...
					plr:sendData(
						"SendMessage",
						"Dex network or/and session are missing. Unable to create dex without them.",
						nil,
						8,
						"Context"
					)
			...
			Function = function(plr, args)
				if server.dexNetwork and server.dexSession then
					...
					if #successPlrs > 0 then
						plr:sendData(
							"SendMessage",
							"Successfully removed dex from "
								.. (
									#successPlrs <= 4 and table.concat(successPlrs, ", ")
									or #successPlrs .. " players."
								),
							nil,
							8,
							"Context"
					...
					plr:sendData(
						"SendMessage",
						"Dex network or/and session are missing. Unable to delete dex without them.",
						nil,
						8,
						"Context"
					)
