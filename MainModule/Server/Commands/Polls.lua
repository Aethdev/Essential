				...
				Remote.ListData[listUpdateId] = {
					...
					Function = function(plr)
						...
						for i, target in pairs(args[1]) do
							...
							table.insert(tab, {
								Text = target.Name.." ("..target.DisplayName..") - "..tostring(confirmData and "responded ("..#selectedOpts.."): "..table.concat(selectedOpts, ", ") or "not responded");
						...
						for i, optData in ipairs(pollSession.options) do
							...
							local percent = ((playersCount==0 and confirmPlayersCount==0 and 0) or playersCount/confirmPlayersCount) * 100

							table.insert(tab, {
								Text = optData.text..": "..tostring(math.round(percent)).."% answered ("..playersCount.." player(s))";
