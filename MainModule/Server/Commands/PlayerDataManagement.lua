	...
	local cmdsList = {
		...
		startPDataTransfer = {
			...
			Function = function(plr, args)
				...
				for i, v in pairs(playerPData._table) do
					if not blockedDeleteAndTransferIndexes[i] then allowedTransferIndexes[i] = true end
				end
				...
				if
					targetSData._pDataTransfer
					or (
						targetPData._transferredDataSince
						and os.time() - targetPData._transferredDataSince < pDataTransferCooldown
					)
				then
					plr:sendData(
						"SendMessage",
					...
					return
				elseif
					playerPData._transferredDataSince
					and os.time() - playerPData._transferredDataSince < pDataTransferCooldown
				then
					plr:sendData(
						"SendMessage",
				...
				target:sendData(
					"SendMessage",
					`<b>PData Transfer</b>: Agree to the one-time confirmation if you wish to allow {plr:toStringDisplayForPlayer(
						target
					)}'s player data to merge with yours.`,
					nil,
					12,
					"Context"
				)
				...
				local checkTargetConfirm = target:customGetData(60 + 10, "MakeUI", "Confirmation", {
					...
					desc = `Would you allow {plr:toStringDisplayForPlayer(target)} to merge their player data with yours? <i>Please note that transfers cannot`
						.. ` be undone unless overriden by an in-game administrator. Each transfer has a cooldown of {Parser:relativeTimestamp(
							os.time() + pDataTransferCooldown
						)}.</i>`,
					choiceA = "Yes, I confirm.",
					returnOutput = true,
					time = 60,
		...
		viewTransferInfo = {
			...
			Function = function(plr, args)
				...
				local transferToName = targetPData._transferTo and service.playerNameFromId(targetPData._transferTo)
					or "[not available]"
				local transferFromName = targetPData._transferFrom
						and service.playerNameFromId(targetPData._transferFrom)
					or "[not available]"
			...
			Function = function(plr, args)
				...
				if user.UserId == target.UserId then
					...
					elseif not canIgnorePriorityCheck and userPriority >= targetPriority then
						plr:sendData(
							"SendMessage",
							`PData Transfer cannot forcibly initiate (due to your lack of admin permission) if user {tostring(
								user
							)} has a higher/equal priority to {tostring(target)}. Confirmation has been requested to the user and target.`,
							nil,
							5,
							"Context"
						)
						...
						for i, selector in { target, user } do
							task.spawn(function()
								local checkAgree = selector:customGetData(20 + 10, "MakeUI", "Confirmation", {
									title = "PData Transfer Confirmation",
									desc = `Are you sure you would like merge player data with {tostring(
										if selector == target then user else target
									)}'s player data?` .. ` <i>This action is irreversible</i>.`,
									choiceA = "Yes, I confirm.",
									returnOutput = true,
									time = 20,
								});
				...
				user:sendData(
					"SendMessage",
					`<b>PData Transfer</b>: An in-game administrator, {tostring(plr)}, has transferred your player data to {tostring(
						target
				...
				target:sendData(
					"SendMessage",
					`<b>PData Transfer</b>: An in-game administrator, {tostring(plr)}, has merged your player data with {tostring(
						user
					)}.` .. `Rejoin to see the new changes.`,
					nil,
					10,
		...
		clearSelfPlayerData = {
			...
			Function = function(plr, args)
				...
				if
					pData.serverData._pDataTransfer
					or (pData._transferredDataSince and os.time() - pData._transferredDataSince < pDataTransferCooldown)
				then
					plr:sendData(
						"SendMessage",
					...
					return
				elseif
					pData.serverData._pDataDeletion
					or (pData._deletedDataSince and os.time() - pData._deletedDataSince < pDataDeletionCooldown)
				then
					plr:sendData(
						"SendMessage",
				...
				local response = plr:customGetData(confirmWaitTime + 10, "MakeUI", "PrivateMessageV2", {
					...
					bodyDetail = table.concat({
						...
						(function()
							local list = {}
							for i, v in allowedTransferIndexes do
								if not blockedDeleteAndTransferIndexes[i] then table.insert(list, `- {i}`) end
							end
							return table.concat(list, "\n")
						end)(),
				...
				local defaultData = Core.defaultPlayerData()
				for i, v in pairs(pData._table) do
					if not blockedDeleteAndTransferIndexes[i] then pData[i] = nil end
				end
				for i, v in pairs(defaultData) do
					if not blockedDeleteAndTransferIndexes[i] then pData[i] = v end
				end
