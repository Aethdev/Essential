			...
			Function = function(plr, args)
				...
				if #successPlayers == 0 then
					plr:sendData(
						...
						"The specified players don't have a team to leave.",
						4,
						"Hint"
					)
				else
					plr:sendData(
						"SendMessage",
						"Team Management success",
						table.concat(successPlayers, ", ") .. " abandoned their current team.",
						8,
						"Hint"
					)
				end
			...
			Function = function(plr, args)
				...
				if availableTeam then
					...
					plr:sendData(
						"SendMessage",
						"Team Management success",
						"Assigned <b>" .. tostring(#args[1]) .. " players</b> to " .. availableTeam.Name,
						8,
						"Hint"
					)
				else
					plr:sendData(
						"SendMessage",
						"Team Management failed",
						"There's no team with a name " .. teamName,
						4,
						"Hint"
					)
				end
			...
			Function = function(plr, args)
				...
				if avTeam then
					...
					plr:sendData(
						"SendMessage",
						"Team Management failed",
						"There's no available team to destroy.",
						6,
						"Hint"
					)
				end
		...
		addPlayersToRoles = {
			...
			Function = function(plr, args)
				...
				for i, role in pairs(Roles:getAll()) do
					if role.assignable and role.priority < highPriority then table.insert(availableRoles, role) end
				end
				...
				for i, givenName in pairs(specifiedRoles) do
					for d, avRole in pairs(availableRoles) do
						if
							(givenName:lower() == "all" or avRole.name:sub(1, #givenName):lower() == givenName:lower())
							and not table.find(listedRoles, avRole)
						then
							table.insert(listedRoles, avRole)

							if avRole:hasPermission "Manage_Game" then table.insert(adminRoles, avRole) end
						end
					end
				end

				if saveInData then
					local canSaveRoles = Moderation.checkAdmin(plr)
						or Roles:hasPermissionFromMember(plr, { "Manage_Roles" })
					if not canSaveRoles then
						plr:sendData(
							"SendMessage",
							`<b>Roles Management</b> `
								.. "You must have permission <b>Manage_Roles</b> to save player roles.",
							nil,
							5,
							"Context"
						)
					...
					filterList = function()
						for i, targetedRole in ipairs(listedRoles) do
							if not targetedRole.saveable then
								table.insert(
									failedToSaveRoles,
									"<b>" .. Parser:filterForRichText(targetedRole.name) .. "</b>"
								)
					...
					if #failedToSaveRoles > 0 then
						plr:sendData(
							"SendMessage",
							`<b>Roles Management</b>: `
								.. table.concat(failedToSaveRoles, ", ")
								.. " doesn't have the option 'saveable' enabled. You cannot save these roles",
							nil,
							5,
							"Context"
						)
						wait(5)
						if #listedRoles == 0 then return end
					end
				end
				...
				if #listedRoles > 0 then
					local confirmChoice1 = plr:customGetData(10 + 30, "MakeUI", "ConfirmationV2", {
						...
						description = "Are you sure you would like to assign "
							.. tostring(#players)
							.. " player(s) to "
							.. concatListedRoles
							.. "?",
					...
					for i, givenRole in pairs(assignedRoles) do
						if i == #assignedRoles then
							if #assignedRoles > 1 then
								concatAssignedRoles = concatAssignedRoles
									.. " & <b>"
									.. Parser:filterForRichText(givenRole.name)
									.. "</b>"
							else
								concatAssignedRoles = concatAssignedRoles
									.. " <b>"
					...
					if #assignedRoles > 0 then
						...
						for i, target in pairs(players) do
							target:makeUI("NotificationV2", {
								...
								description = `{plr:toStringDisplayForPlayer(target)} assigned you to {#assignedRoles} non-administrator role(s) {if saveInData
									then `permanently`
									else `temporarily`}: {concatAssignedRoles}`
					...
					if #adminRoles > 0 then
						local confirmChoice2 = plr:customGetData(10 + 30, "MakeUI", "ConfirmationV2", {
							title = "Role Management Confirmation",
							description = "Warning! You are giving "
								.. tostring(#players)
								.. ' access to <font color="#de5252"><b>administrator roles</b></font> such as '
								.. concatAdminRoles
								.. ". Are you sure you would perform this action?",
							firstChoice = {
								label = "Yes, I confirm.",
								submissionPage = {
									title = "Role Management Confirmed",
						...
						if confirmChoice2 ~= 1 then
							...
							for i, target in pairs(players) do
								target:makeUI("NotificationV2", {
									title = `Role Management`,
									description = `{plr:toStringDisplayForPlayer(target)} assigned you to {#adminRoles} administrator role(s) {if saveInData
										then `permanently`
										else `temporarily`}: <font color=\"#de5252\">{concatAdminRoles}</font>`
										.. `\n\n<i>Note: If you are assigned to the specified roles with dynamic/fixed enlistments in developer settings, the temporary/pdata-permanent status will not affect your assignment.</i>`,
								})
							end
						end
					...
					plr:sendData(
						"SendMessage",
						`<b>Roles Management</b>: `
							.. "There are no specified assignable roles to give to "
							.. tostring(#players)
							.. " player(s)",
						nil,
						5,
			...
			Function = function(plr, args)
				...
				if saveFromData then
					...
					if not canSaveRoles then
						plr:sendData(
							...
							"You must have permission <b>Manage_Roles</b> to remove saved player roles.",
							5,
							"Hint"
						)
						return
				...
				if #failedToSaveRoles > 0 then
					plr:sendData(
						...
						"Role Management failed",
						concatFSRoles
							.. " doesn't have the option 'saveable' enabled. You cannot remove these saved roles",
						5,
						"Hint"
					)
					if #availableRoles == 0 then return end
				end
				...
				if #listedRoles > 0 then
					local confirmChoice1 = plr:makeUIGet("Confirmation", {
						title = "Role Management confirmation",
						desc = "Are you sure you would like to unassign "
							.. tostring(#players)
							.. " player(s) from "
							.. concatListedRoles
							.. "?",
					...
					if confirmChoice1 ~= 1 then
						plr:sendData(
							"SendMessage",
							"Role Management failed",
							"Declined to unassign " .. tostring(#players) .. " player(s) from " .. concatListedRoles,
							5,
							"Hint"
						)
						return
							...
							if listedRole:checkTempMember(target) then
								for permInd, perm in pairs(listedRole.members) do
									if
										type(perm) == "table"
										and perm.Temp
										and table.find(perm.List or {}, target.UserId)
									then
					...
					plr:sendData(
						...
						"Unassigned " .. tostring(#players) .. " player(s) from " .. concatListedRoles,
						5,
						"Hint"
					)
					...
					for i, target in pairs(players) do
						target:makeUI("NotificationV2", {
							title = `Role Management`,
							description = `{plr:toStringDisplayForPlayer(target)} unassigned you from {#listedRoles} role(s) {if saveFromData
								then `permanently`
								else `temporarily`}: {concatListedRoles}`
								.. `\n\n<i>Note: If you are assigned to the specified roles with dynamic/fixed enlistments in developer settings, the temporary/pdata-permanent status will not affect your assignment.</i>`,
						})
					end
				else
					plr:sendData(
						...
						"There are no specified roles to remove from " .. tostring(#players) .. " player(s)",
						5,
						"Hint"
					)
				end
				...
				for roleInd, role in ipairs(priorityRoles1) do
					...
					if rolePriority <= highPriority or role.allowlowrankstoviewlist then
						...
						table.insert(priorityRoles2, {
							type = "Detailed",
							label = `{if rolePriority == highPriority then "🔸 " else ""}({tostring(
								role.priority or 0
							)}) <b>{Parser:filterForRichText(role.name)}</b> ({memberCount})`,
							labelColor = role.permissions["Manage_Game"] and Color3.fromRGB(255, 174, 44) or nil,
							description = "Assignable: "
								.. tostring(role.assignable and true or false)
								.. " | Saveable: "
								.. tostring(role.saveable and true or false),
							richText = true,
							hideSymbol = true,
						})
