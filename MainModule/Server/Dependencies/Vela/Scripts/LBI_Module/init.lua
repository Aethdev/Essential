...
if handler then
	...
	if fiOne then
		local suc, func =
			coroutine.resume(coroutine.create(function() coroutine.yield(require(fiOne)(source, env)) end))

		if type(func) == "function" then
			local ran, error = coroutine.resume(coroutine.create(setfenv(func, env)))

			if not ran and (not error or type(error) == "string") then realError("LBI Error: " .. tostring(error)) end
		end
	end
end
