...
function vela:create(scrType, source, data)
	...
	if scrType == "Server" or scrType == "Module" then
		...
		do
			...
			handlerMeta.__newindex = function(self, ind, val) error("Attempting to write a read only userdata", 0) end
		end

		local errorCount = 0
		local errorEv
		errorEv = service.ScriptContext.Error:connect(function(msg, trace, failedScr)
		...
		scr:SetAttribute(
			"Source",
			tostring((data.publicSource and source) or "[VELA Script] This script has a private source.")
		)
	...
	elseif scrType == "Client" then
		...
		local onInvoke = function()
			...
			if scrData.Active then return bytecode end
		end
