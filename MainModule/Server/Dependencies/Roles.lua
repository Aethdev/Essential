...
local function cloneTable(tab)
	local clone; clone = function(val)
...
function roles:create(name: string, priority: number?, color: Color3?, members: {[any]: any}?, permList: {[any]: any}?): {[any]: any}
	...
	if listedRoles[name] then
		...
		if members and type(members) == "table" then
			for i,member in pairs(members) do
				...
				if typ == "table" or typ == "string" or typ == "number" then
					table.insert(roleTab.members, member)
				end
			end
		end
			...
			if #perms > 0 then
				for ind,perm in pairs(perms) do
					if type(perm) == "string" then
						if not checkList[perm] and self:checkPermission(perm) then
							checkList[perm] = true
						end
					end
				end
		...
		function roleTab:setPermission(perm, bool)
			if type(perm) == "string" and type(bool) == "boolean" then
				self.permissions[perm] = bool
			end
		end
		...
		function roleTab:getMemberCount()
			return #self.members
		end
...
function roles:get(name: string): {[any]: any}?
	if name then
		for i,role in pairs(listedRoles) do
			if rawequal(role.name, name) or rawequal(role.id, name) then
				return role
			end
		end
	end
...
function roles:getRolesFromColor(color: Color3): {[any]: any}
	...
	for i,role in pairs(listedRoles) do
		if rawequal(role.color, color) then
			table.insert(results, role)
		end
	end
...
function roles:getRolesFromMember(member: any): {[any]: any}
	...
	for i,role in pairs(listedRoles) do
		if role:checkMember(member) then
...
function roles:checkMemberInRoles(member: any, memberRoles: {[any]: any}, acceptOnlyOne: boolean?): boolean|table?
	memberRoles = (type(memberRoles)=="string" and {memberRoles}) or (type(memberRoles)=="table" and memberRoles) or {}
			...
			if not checkList[memberRoleName] then
				if not acceptOnlyOne then
					for d,missingRole in pairs(memberRoles) do
						if not checkList[missingRole] then
							table.insert(failChecklist, missingRole)
						end
					end
	...
	for i,role in pairs(roles:getRolesFromMember(member)) do
		for d,perm in pairs(perms) do
			...
			if permFindSlash then
				...
				if not checkList[perm] and role:checkPermissions{perm} then
					checkList[perm] = true
				end
			end
		end
...
function roles:getHighestPriorityFromPermission(member: any, rolePerm: string): number
	...
	for i,role in pairs(roles:getRolesFromMember(member)) do
		if role:checkPermissions{rolePerm} and role.priority > latestPriority then
			latestPriority = role.priority
		end
	end
...
function roles:getHighestPriority(member: any): number
	...
	for i,role in pairs(roles:getRolesFromMember(member)) do
		if role.priority > latestPriority then
			latestPriority = role.priority
		end
	end
...
function roles:getTemporaryRolesFromMember(member: any)
	...
	for i, role in roles:getAll() do
		if role:checkTempMember(member) then
			table.insert(list, role)
		end
	end
...
function roles:getDefaultPermissions(): {[any]: any}
	return cloneTable(rolePermissions)
end
...
function roles.Init(env)
	...
	local creatorRole = roles:create("creator", math.huge, nil, {(game.CreatorId<=0  and "@everyone") or (game.CreatorType==Enum.CreatorType.Group and "Group:"..game.CreatorId..":255") or game.CreatorId}, {
		Manage_Game = true;
	})
