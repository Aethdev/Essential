...
function filter:safeString(str: string, senderId: number, targetId: number, filteringType: Enum.TextFilterContext?|string?, filterKeywords: {[any]: any}?, onlyUseCustom: boolean?)
	...
	local filteringName = (typeof(filteringType) == "Enum" and filteringType.Name) or
		(type(filteringType)=="string" and filteringType) or "CustomFilter"
	
	if not filteringEnum then
		filteringEnum = defaultFilterType
	end
	
	if filterKeywords then
		filteringName = "CustomFilter-"..tostring(filterKeywords)
	end
	...
	local senderCache: {[any]: any} = filter.cache[senderCacheId] or filter:registerSenderCache(filter.cache, senderCacheId)
	...
	do
		...
		for i,phrase in pairs(strPhrases) do
			...
			if string.rep("#", #phrase) == phrase then
				continue
			end
			...
			if filterKeywords then
				...
				for i,keywordD in pairs(filterKeywords) do
					if keywordD.stringMatch then
						...
						if keywordD.lowerCase then
							checkKeyword = checkKeyword:lower()
						end
						
						local safeMatchPhrase = filterForStrPattern(phrase)
						if string.match(safeMatchPhrase, checkKeyword) then
							keywordData = keywordD
						end
					else
						if not keywordD.caseSensitive then
							if keywordD.keyword:lower() == phrase:lower() then
								keywordData = keywordD
							end
						else
							if keywordD.keyword == phrase then
								keywordData = keywordD
							end
						end
					end
			...
			if not (onlyUseCustom and filterKeywords) then
				...
				if canUpdateCache then
					...
					if not table.find(checkedPhrases, phraseCache) then
						table.insert(checkedPhrases, phraseCache)
					end
					
					if not dontCachePhrase then
						targetCache.phrases[phrase:lower()] = phraseCache
					end
					
					local fPSuccess: boolean, fpFilterAsync: TextFilterResult = retryFunction(cacheUpdateTime.filterFunctionReachTries,
						service.TextService.FilterStringAsync,
						service.TextService, phrase:lower(), senderId, filteringEnum)
					
					local nonChatFPSuccess: boolean?,nonChatFilterPhrase: string
					
					if fPSuccess then
						nonChatFPSuccess,nonChatFilterPhrase = retryFunction(cacheUpdateTime.filterAttemptTries,
							fpFilterAsync.GetNonChatStringForUserAsync,
							fpFilterAsync, targetId)
						
						phraseCache.filtered = (not nonChatFPSuccess and true) or nonChatFilterPhrase:lower()~=phrase:lower();
					else
					...
					if phraseCache.filtered then
						strPhrases[i] = string.rep("#", #phrase)
					end
				elseif cache then
					if not table.find(checkedPhrases, cache) then
						table.insert(checkedPhrases, cache)
					end
					
					if cache.filtered then
						strPhrases[i] = string.rep("#", #phrase)
					end
				end
			end
	...
	if not msgFiltered and #strPhrases > 1 and not (onlyUseCustom and filterKeywords) then
		local msgFSuccess: boolean, msgFFilterAsync: TextFilterResult = retryFunction(cacheUpdateTime.filterFunctionReachTries,
			service.TextService.FilterStringAsync,
			service.TextService, newString:lower(), senderId)
		
		local nonChatFMsgSuccess: boolean?,nonChatFilterMessage: string
		
		if msgFSuccess then
			nonChatFMsgSuccess,nonChatFilterMessage = retryFunction(cacheUpdateTime.filterAttemptTries,
				msgFFilterAsync.GetNonChatStringForUserAsync,
				msgFFilterAsync, targetId)
			
			msgFiltered = not nonChatFMsgSuccess or nonChatFilterMessage:lower()~=newString:lower()
			if msgFiltered and nonChatFMsgSuccess then
				newString = nonChatFilterMessage
			end
		end
	end
...
function filter:safeStringForPublic(str: string, senderId: number, filterKeywords: {[any]: any}?, onlyUseCustom: boolean?)
		...
		for i,phrase in pairs(strPhrases) do
			...
			if not (onlyUseCustom and filterKeywords) then
				...
				if canUpdateCache then
					...
					local fPSuccess: boolean, filterPhrase: TextFilterResult = retryFunction(cacheUpdateTime.filterFunctionReachTries,
						service.TextService.FilterStringAsync,
						service.TextService, phrase:lower(), senderId)

					local nonChatFBSuccess: boolean?,nonChatFilterPhrase: string

					if fPSuccess then
						nonChatFBSuccess,nonChatFilterPhrase = retryFunction(cacheUpdateTime.filterAttemptTries,
							filterPhrase.GetNonChatStringForBroadcastAsync,
							filterPhrase, senderId)
						
						phraseCache.filtered = (not nonChatFBSuccess and true) or nonChatFilterPhrase:lower()~=phrase:lower();
					else
						phraseCache.filtered = true
					end
					...
					if not dontCachePhrase then
						filterPhrases[phrase:lower()] = phraseCache
					end
				elseif cache then
					if not table.find(checkedPhrases, cache) then
						table.insert(checkedPhrases, cache)
