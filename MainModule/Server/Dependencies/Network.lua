return function(envArgs)
	...
	local function Init()
		...
		if networkServer then
			...
			service.rbxEvent(networkServer.ChildAdded, function(child)
				if child:IsA "ServerReplicator" then Network.registerReplicator(child) end
			end)
	...
	local function logViolationAttempt(plr: ParsedPlayer, network: Network, reason: string?)
		if not breachViolatorAttempts[plr.UserId] then breachViolatorAttempts[plr.UserId] = 0 end
	...
	server.Network = {
		...
		newCreate = function(networkName: string, creationData: {
			...
			local netInstance: Instance = (
				netInvokable and (netServer and service.New "BindableFunction" or service.New "RemoteFunction")
			)
				or (not netInvokable and (netServer and service.New "BindableEvent" or service.New "RemoteEvent"))
			...
			function networkData:setupSecurity()
				if self.active then
					...
					table.insert(
						self.securityEvents,
						wrap(netInstance:GetPropertyChangedSignal("Name"):Connect(function()
							if self.securityRunActive and self.securityRunId == securityRunId then
								local newName = netInstance.Name

								if newName ~= self.decoyInfo.name then netInstance.Name = self.decoyInfo.name end
							end
						end))
					)
					...
					table.insert(
						self.securityEvents,
						wrap(netInstance:GetPropertyChangedSignal("Parent"):Connect(function()
							if self.securityRunActive and self.securityRunId == securityRunId then
								local curParent = netInstance.Parent
								local lastParented = self.parented

								if lastParented then
									if (os.time() - lastParented) < 0.5 then
					...
					table.insert(
						self.securityEvents,
						wrap(netInstance.AttributeChanged:Connect(function(attribute)
							if self.securityRunActive and self.securityRunId == securityRunId then
								...
								if attribute == "Id" and netServer then
									if self.id ~= newValue then netInstance:SetAttribute(attribute, self.id) end
								end
							end
						end))
					)
						...
						if not firewallType then
							warn(
								"NETWORK "
									...
									.. " ("
									.. self.name
									.. ") couldn't create a firewall with an invalid type. (strict/high types expected)"
							)
						end
					end
				end
			...
			function networkData:processFirewall(player: Player, modAction: string, ...)
				...
				if modAction == "suspicious_access" then
					...
					if isPlayerOnTrustCheck then
						warn(
							"[SECURITY ISSUE] Uh, oh! "
								.. tostring(parsedPlayer)
								.. " has an active trust check status without valid access. Make sure to remove their trust key data before revoking their access."
						)
					...
					local globalAttemptRatePass, globalAttemptThrottle =
						Utility:checkRate(globalNetFirewall.suspiciousAccess, parsedPlayer.playerId)
					...
					local attemptRatePass, attemptThrottle =
						Utility:checkRate(self.firewallModRecords.suspiciousAccess, parsedPlayer.playerId)
						...
						if netFirewallType == "high" then
							if not attemptThrottle then
								...
								Logs.addLog(
									...
									"[🔨 Ban] "
										.. tostring(parsedPlayer)
										.. " attempts several times to access a private network"
								)
							else
								Logs.addLog(
									"Exploit",
									"[🦵 Kick] "
										.. tostring(parsedPlayer)
										.. " attempts a few times to access a private network"
								)
							end
						else
							Logs.addLog(
								...
								"[🦵 Kick] "
									.. tostring(parsedPlayer)
									.. " attempts a few/several times to access a private network"
							)
						end
				...
				if modAction == "invalid_index" then
					...
					if canIgnoreIndex then return end
					...
					if indexCheckStat == -1 then
						...
						table.insert(
							self.firewallLogs,
							formatLog {
								title = tostring(parsedPlayer)
									.. " tried invoking/firing with an invalid index type "
									.. tostring(firstArg)
									.. " ("
									.. type(firstArg)
									.. ")",
					...
					elseif indexCheckStat == -2 then
						...
						table.insert(
							...
							formatLog {
								title = tostring(parsedPlayer)
									.. " tried invoking/firing with a non-existent index "
									.. tostring(firstArg),
							}
						)
					elseif indexCheckStat == -3 then
						...
						table.insert(
							self.firewallLogs,
							formatLog {
								title = tostring(parsedPlayer) .. " tried invoking/firing with index " .. tostring(
									firstArg
								) .. " with an invalid call type",
							}
						)
					...
					local attemptRatePass, attemptThrottle =
						Utility:checkRate(self.firewallModRecords.invalidIndexes, parsedPlayer.playerId)
					if not attemptRatePass then
						...
						if netFirewallType == "high" then
							if not attemptThrottle then
								...
								Logs.addLog(
									...
									"[🔨 Ban] "
										.. tostring(parsedPlayer)
										.. " attempts to access a private index on a private network"
								)
								self.firewallTriggered:fire(modAction, player, "ban")
							end
						end
			...
			function networkData:canRunIndex(player: Player, index: string | number, invoke: boolean)
				...
				if not isValidIndex then failToCheck = true end
				...
				if failToCheck then
					...
					if not networkCmd or type(networkCmd) ~= "table" then
						...
						if not (networkCmd.Can_Invoke or networkCmd.Can_Fire) then networkCmd.Can_Fire = true end

						local canInvokeOrFireCmd = (invoke and networkCmd.Can_Invoke)
							or (not invoke and networkCmd.Can_Fire)
							or false
						if not canInvokeOrFireCmd then return -3 end
						...
						local userHasPermissions = (
							permissions and Roles:hasPermissionFromMember(parsedPlayer, permissions)
						) or false
			...
			function networkData:isPlayerOnTrustCheck(player: ParsedPlayer)
				local existingCheckData: { [any]: any }? = self.waitingForTrustCheck[player.playerId]
				if
					not existingCheckData or (existingCheckData.expireMs and tick() - existingCheckData.expireMs > 0)
				then
					if existingCheckData then self.waitingForTrustCheck[player.playerId] = nil end
					return false
				else
					return true
				end
			...
			function networkData:createPlayerKey(player, expireOs)
				...
				function keyInfo:isActive() return self.active and (not self.expireOs or os.time() - self.expireOs < 0) end
			...
			function networkData:getPlayerKey(player): { [any]: any }? return self.networkKeys[player.playerId] end
						...
						if self.active then
								...
								if not hasAccess then
									...
									if netFirewallRequireAccessKey then
										...
										if type(givenAccessKey) == "string" then
											local isUsingTheRightKey = (
												netFirewallAllowRemoteKeyForAccess
												and globalPlayerKey
												and givenAccessKey == globalPlayerKey
											) or (plrKeyId and givenAccessKey == plrKeyId)
							...
							if not suc then
								...
								if self.warnError then
									warn(
										"Network " .. networkName .. " encountered an error: " .. tostring(rets[2]),
										"\n",
										rets[3]
									)
								...
								if netInvokable then return unpack(rets, 2) end
							end
						else
							if netInvokable then return "Network_Inactive" end
						end
					end
			...
			function networkData:setupInstance()
				if self.active then
					if not self.parented then
						if netInstance.Parent ~= self.parent then netInstance.Parent = self.parent end
			...
			function networkData:changeParent(newParent: Instance)
				if self.active and self.parent ~= newParent then
					...
					task.delay(changeWaitTime, function()
						if netInstance.Parent ~= newParent then netInstance.Parent = newParent end
					end)
			...
			if not netServer then networkData.publicId = netPublicId or getRandom(40) end
		...
		newDecoy = function(decoyName: string, creationData: { [any]: any })
			...
			local networkData = Network.newCreate("DECOY_" .. tostring(decoyName), {
				...
				networkFunc = function(plr)
					...
					if not (plr and typeof(plr) == "Instance" and plr:IsA "Player") then plr = nil end
		...
		create = function(networkName, parent, invoke, serverNetwork, func, publicId)
			...
			networkInfo = {
				...
				Reload = function(self, timeOut, override)
					...
					if override or (networkInfo.Active and not override) then
						...
						local newNetwork = Network.create(
							networkName,
							networkInfo.Parent,
							invoke,
							serverNetwork,
							func,
							publicId or networkInfo.publicId
						)
				...
				ChangeParent = function(self, newParent)
					if newParent and networkInfo.Parent ~= newParent then
						if networkInfo.ChangingParent then networkInfo.ChangedParent.Event:Wait() end
						...
						if anInstance then
							...
							for i = 1, 3, 1 do
								if not networkInfo.Active or remoteObject.Parent == newParent then
									...
									local suc, ers = pcall(function() remoteObject.Parent = newParent end)
								...
								if not didParent then
									wait(0.5)
									if not networkInfo.Active then return end
								end
							end
			...
			if not serverNetwork then networkInfo.publicId = publicId or getRandom(40) end
			...
			networkInfo.Security1 = wrap(remoteObject:GetPropertyChangedSignal("Name"):Connect(function()
				...
				if newName ~= networkInfo.DecoyName then remoteObject.Name = networkInfo.DecoyName end
			end))
			...
			networkInfo.Security3 = wrap(remoteObject.AttributeChanged:Connect(function(attribute)
				...
				if attribute == "PublicId" then
					...
					if publicId and publicId ~= newValue then remoteObject:SetAttribute(attribute, publicId) end
				end
			end))

			remoteObject:SetAttribute("ESSNetwork", true)

			if not serverNetwork then remoteObject:SetAttribute("PublicId", networkInfo.publicId) end
			...
			service.loopTask(trackLoopName, 15, function() -- Check activity
				if networkInfo.Active then
					...
					repeat
						wait()
					until not server.Running or networkInfo.Active or (os.time() - start > 120) -- Check two minutes of inactivity

					if not networkInfo.Active or not server.Running then
						service.stopLoop(trackLoopName)

						if not networkInfo.Stopped then networkInfo:Disconnect() end
					end
				end
			end)
		...
		createDecoy = function(name, invoke, parent)
			...
			decoyInfo = {
				...
				Reload = function(self, timeOut, override)
					...
					setmetatable(oldDecoy, {
						__index = function(self, ind) return newDecoy[ind] end,
					})
				...
				ChangeParent = function(self, newParent)
					if newParent and decoyInfo.Parent ~= newParent then
						if decoyInfo.ChangingParent then decoyInfo.ChangedParent.Event:Wait() end
						...
						if anInstance then
							...
							for i = 1, 3, 1 do
								if not decoyInfo.Active or decoyInstance.Parent == newParent then
									...
									local suc, ers = pcall(function() decoyInstance.Parent = newParent end)
								...
								if not didParent then
									wait(0.5)
									if not decoyInfo.Active then return end
								end
							end
			...
			decoyInfo.Security1 = wrap(decoyInstance:GetPropertyChangedSignal("Name"):Connect(function()
				...
				if newName ~= decoyName then decoyInstance.Name = decoyName end
			end))
			...
			service.loopTask(trackLoopName, math.random(200, 400), function() -- Check activity
				if decoyInfo.Active then
					...
					if invoke then decoyInstance.OnServerInvoke = decoyInfo.RemoteFunction end
				else
					service.stopLoop(trackLoopName)
				end
			end)
		...
		getDecoys = function(name)
			...
			for i, info in pairs(createdDecoys) do
				if info.Name == name or info.Index == name then table.insert(list, info) end
			end
			...
			if justName then
				for i, cli in pairs(service.NetworkServer:children()) do
					if cli:IsA "ServerReplicator" then
						...
						if plr and plr.Name:lower() == userIdOrName:lower() then return cli, plr end
					end
				end
			elseif justId then
				for i, cli in pairs(service.NetworkServer:children()) do
					if cli:IsA "ServerReplicator" then
						...
						if plr and plr.UserId == userIdOrName then return cli, plr end
					end
				end
			end
		end,
		...
		cacheClients = function()
			for i, cli in pairs(service.NetworkServer:children()) do
				if cli:IsA "ServerReplicator" then Network.registerReplicator(cli) end
			end
		end,
		...
		registerReplicator = function(rep)
			local existCache = (function()
				for i, repli in pairs(serverReplicators) do
					if repli.instance == rep then return repli end
				end
			end)()
		...
		deregisterReplicator = function(rep)
			local existCache = (function()
				for i, repli in pairs(serverReplicators) do
					if repli.instance == rep then return i end
				end
			end)()

			if existCache then table.remove(serverReplicators, existCache) end
		end,
		...
		getReplicators = function() return service.cloneTable(serverReplicators) end,
		...
		isPlayerOnTrustCheckWithAnyNetwork = function(player: ParsedPlayer | Player): (boolean, network?)
			...
			for i, network in pairs(createdNetworks) do
				if network.active and network:isPlayerOnTrustCheck(parsedPlayer) then return true, network end
			end
			...
			for i, regNetwork in pairs(createdNetworks) do
				if regNetwork.active then
					for playerId, keyData in pairs(regNetwork.networkKeys) do
						if keyData.id == trustKey and keyData:isActive() then return keyData, regNetwork end
					end
				end
			end
		end,
		...
		canIgnoreIndex = function(player: ParsedPlayer, index: string | number): boolean
			...
			for i, indexProcess in ipairs(indexCheckProcesses) do
				...
				if not suc then
					warn(
						"Index processor "
							.. tostring(indexProcess)
							.. " encountered an error while checking index "
							.. tostring(index)
							.. ":",
						retStat
					)
