...
local DEFAULT_CLIENT_POLICIES = {
	...
	HIDDEN_PLAYERS = {},
	INCOGNITO_PLAYERS = {},
	MUTED_PLAYER = BOOL_FALSE,
	DEAFENED_PLAYER = BOOL_FALSE,
...
local function trimString(str: string): string return string.match(str, "^%s*(.-)%s*$") end
...
function policyManager:_updateClientPolicies(
	player: ParsedPlayer,
...
function policyManager:_updatePlayerDataClientPolicies(
	player: ParsedPlayer,
	inheritedPolicies: { [string]: string | boolean | { [any]: any } }?
)
	inheritedPolicies = inheritedPolicies or self:_getClientPolicies(player)
...
function policyManager:_updateDynamicClientPolicies(
	player: ParsedPlayer,
	inheritedPolicies: { [string]: string | boolean | { [any]: any } }?
)
	inheritedPolicies = inheritedPolicies or self:_getClientPolicies(player)
	...
	policyManager:setPoliciesForPlayer(player, {
		{
			...
			if settings.minimizedPlayerView
				then not server.Roles:hasPermissionFromMember(player, { "Use_Utility" })
				else DEFAULT_CLIENT_POLICIES.MINIMIZED_TOPBARICONS,
			"DYNAMIC",
		},
		{
			"CONSOLE_ALLOWED",
			if settings.consoleEnabled
				then (settings.consolePublic == true or server.Roles:hasPermissionFromMember(
					player,
					{ "Use_Console", "Manage_Game" },
					true
				))
				else DEFAULT_CLIENT_POLICIES.CONSOLE_ALLOWED,
			"DYNAMIC",
		},
...
function policyManager:setPolicyForPlayer(
	player: ParsedPlayer,
	policyName: string,
	value: string | boolean | { [any]: any } | nil,
	enforcementType: string?
)
	assert(
		type(policyName) == "string" and trimString(policyName:upper()) == policyName,
		`Policy name {tostring(policyName)} is not valid`
	)
	assert(
		type(value) == "string" or type(value) == "boolean" or type(value) == "table" or type(value) == "nil",
		`Policy value must be a string/boolean/table/nil`
	)
	assert(
		(type(enforcementType) == "string" and trimString(enforcementType:upper()) == enforcementType)
			or type(enforcementType) == "nil",
		`Enforcement type must be a string/nil`
	)
	assert(
		GLOBAL_CLIENT_POLICIES[policyName] == nil,
		`Unable to enforce policy {policyName} for this player {tostring(player)} due to a current global policy`
	)
	...
	if not (inheritedPolicies[policyName] and inheritedPolicies[policyName].value == value) then
		...
		if player:isVerified() then
			self.networkPolicyChanged:fireToSpecificPlayers(
				{ player },
				policyName,
				value,
				if enforcementType then enforcementType:upper() else `MANDATORY`
			)
		end
	end
...
function policyManager:setPoliciesForPlayer(
	player: ParsedPlayer,
	listOfPolicies: {
		[number]: { policyName: string, policyValue: string | boolean | { [any]: any } | nil, enforcementType: string? },
	}
)
	...
	for i, policyInfo in listOfPolicies do
		local policyName, value, enforcementType, onlyChangeWithMatchingEnforcement =
			policyInfo[1], policyInfo[2], policyInfo[3], policyInfo[4]
		...
		if GLOBAL_CLIENT_POLICIES[policyName] ~= nil then
			if server.Studio then
				warn(`An attempt occurred in PolicyManager bulk change to modify a policy over the overriden policy.`)
			end
			continue
		end
		...
		if
			not (inheritedPolicies[policyName] and inheritedPolicies[policyName].value == value)
			and (not onlyChangeWithMatchingEnforcement or inheritedPolicies[policyName].type == enforcementType)
		then
...
function policyManager:setPolicyForPlayers(
	players: { [number]: ParsedPlayer },
	policyName: string,
	value: string | boolean | { [any]: any } | nil,
	enforcementType: string?
)
...
function policyManager:setPoliciesForPlayers(
	players: { [number]: ParsedPlayer },
	listOfPolicies: {
		[number]: { policyName: string, policyValue: string | boolean | { [any]: any } | nil, enforcementType: string? },
	}
...
function policyManager:crossUpdatePlayerPolicy(
	playerOrUserId: ParsedPlayer | number,
	policyName: string,
	value: string | boolean | { [any]: any } | nil
)
	assert(type(policyName) == "string" and trimString(policyName:upper()) == policyName, `Policy name is not valid`)
	...
	assert(
		server.Parser:isParsedPlayer(playerOrUserId)
			or (
				type(playerOrUserId) == "number"
				and math.floor(playerOrUserId) == playerOrUserId
				and playerOrUserId > 0
			),
		`Player must be specified`
	)

	if server.Parser:isParsedPlayer(playerOrUserId) then playerOrUserId = playerOrUserId.UserId end
...
function policyManager:setup()
	...
	local networkRetrievePolicies = networkSession:makeCommand(
		`GetPolicies`,
		function(plr) return policyManager:_getClientPolicies(plr) end
	)
	...
	server.Cross.commands.UpdatePlayerPolicy = function(
		jobId: string,
		userId: number,
		policyName: string,
		value: string | boolean | { [any]: any } | nil
	...
	server.Cross.commands.UpdateGlobalPolicy = function(
		jobId: string,
		policyName: string,
		value: string | boolean | { [any]: any } | nil
	)
