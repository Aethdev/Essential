...
function utility:getPlayersCount(): number
	return #service.getPlayers()
end

function utility:lockdown()
	server.Core.lockdown = true
end
...
function utility:shutdown(reason: string?, secsTillShutdown: number?, moderatorId: number?): boolean
	...
	if not (utility.shuttingDownState or utility.shutdownState) then
		...
		if shutdownTime > 0 then
			...
			repeat
				if not utility.shuttingDownState then
					break
				end
				...
				for i,plr in pairs(service.getPlayers(true)) do
					plr:sendData("SendMessage", "Shutdown Alert", "Shutting down in "..math.floor(secsTillShutdown).."...", 1, "Hint")
				end

				wait(1)
			until
				os.time()-start > shutdownTime
			...
			for attempt = 1, 3 do
				for i,plr in pairs(service.getPlayers(true)) do
					...
					task.delay(2, function()
						...
						plr._object:Kick(server.Parser:replaceStringWithDictionary(tostring(server.Settings.shutdownMessage), {
	...
	if client and loader then
		...
		for i, desc in ipairs(folderContents) do
			if desc ~= holder then
				...
				table.insert(secureCheck_Cons, desc:GetPropertyChangedSignal"Archivable":Connect(function()
					desc.Archivable = lockArchivable
					--client.tamperedFolder = true
				...
				table.insert(secureCheck_Cons, desc.AttributeChanged:Connect(function(attrName)
					client.tamperedFolder = true
					client.tamperedFolderReason = "Attribute "..tostring(attrName).." from object "..desc:GetFullName().." was tampered"
					--warn("Tampered remote")
					stopSecuring()
					service.Debris:AddItem(holder, 0)
				end))
		...
		if not playerGui then
			...
			local childAdded; childAdded = plr.ChildAdded:connect(function(child)
			...
			guiAdded:connectOnce(function(gui)
				holder.Parent = gui
			end)
		else
...
function utility:readRate(rateLimit: {[any]: any}, rateKey: string|number|userdata|table)
	...
	if not rateData then
		...
		local didThrottleReset = if canThrottle and not didPassRL and rateData.LastThrottled then currentOs-rateData.LastThrottled >= throttleReset else false
		local didThrottleRL = if canThrottle and not didPassRL and not didThrottleReset and rateData.Throttle then rateData.Throttle<=throttleMax else false
...
function utility:checkRate(
	...
	rateKey: string|number|userdata|table, deferCheck: boolean?
) -- Rate limit check
	-- Ratelimit: {[any]: any}
...
function utility:checkRateInPlayerData(
	...
	local function doCheckRate()
		...
		local rateCache: {
			...
			LastUpdated: number,
			LastThrottled: number?
		}|nil = playerDataRateLimits[hashedRateLimitId]
		local throttleCache
		if not rateCache then
			rateCache = {
				Rate = 0;
		...
		if rateExceeded and not ratePass then
			rateExceeded:Fire(`PLAYERDATA`, rateCache.Rate, maxRate)
		end

		if ratePassed and ratePass then
			ratePassed:Fire(`PLAYERDATA`, rateCache.Rate, maxRate)
		end
		...
		if not ignoreRateDataCleanup then
			...
			local clearListThread; clearListThread = task.delay(5, function()
				if clearPlayerDataThreads[playerDataRateLimits] == clearListThread then
					...
					if didUpdate then
						...
						if playerDataRateLimits._reviveIfDead then 
							playerDataRateLimits._reviveIfDead()
						end
					end
				end
			end)
...
function utility:getMainSound(createIfNotAdded: boolean): Sound
	...
	if (not mainSound and createIfNotAdded) or (mainSound and mainSound.Parent ~= workspace) then
		...
		if not soundData then
			soundData = {
				...
				changePitch = function(self, newPitch)
					...
					if soundObj then
						soundObj.PlaybackSpeed = newPitch
					end
				end;
		...
		table.insert(soundData.events, mainSound.Ended:Connect(function(...)
			soundData.ended:fire(...)
		end))

		table.insert(soundData.events, mainSound.Paused:Connect(function(...)
			soundData.paused:fire(...)
		end))

		table.insert(soundData.events, mainSound.Played:Connect(function(...)
			soundData.played:fire(...)
		end))

		table.insert(soundData.events, mainSound.Resumed:Connect(function(...)
			soundData.resumed:fire(...)
		end))

		table.insert(soundData.events, mainSound.Stopped:Connect(function(...)
			soundData.stopped:fire(...)
		end))

		table.insert(soundData.events, mainSound:GetPropertyChangedSignal"Volume":Connect(function(...)
		...
		table.insert(soundData.events, mainSound:GetPropertyChangedSignal"Parent":Connect(function()
...
function utility:isMuted(playerName: string): boolean
	...
	if existingPlr then
		...
		if parsed:getVar("MuteChat") then
			return true
		end
	end
...
function utility:mutePlayer(playerName: string, durationInSeconds: number?, silent: boolean?)
	...
	if not self:isMuted(playerName) and not self:isDeafened(playerName) then
		...
		if parsedPlayer then
			parsedPlayer:sendData("SetCore", "ChatBarDisabled", true)
		end
...
function utility:deafenPlayer(playerName: string, duration: duration?)
	if not self:isDeafened(playerName) then
		...
		local parsedPlayer = server.Parser:apifyPlayer(service.getPlayer(playerName))
		if parsedPlayer then
			parsedPlayer:sendData("SetCoreGuiEnabled", Enum.CoreGuiType.Chat, false)
		end
...
function utility:unmutePlayer(playerName: string, silent: boolean?): boolean
	if self:isMuted(playerName) then
		...
		if parsedPlayer then
			...
			if pData.serverData.ToggleMuteOnAFK then
				pData.serverData.ToggleMuteOnAFK = false
			end
		end

		if variables.deaflist[playerId] then
			if parsedPlayer then
				parsedPlayer:sendData("SetCoreGuiEnabled", Enum.CoreGuiType.Chat, true)
			end
...
function utility:makeMapBackup(slotName: string, slotInd: number, exceptions: {[any]: any}, dataTab: {[any]: any}): {[any]: any}
	dataTab = dataTab or variables.mapBackups

	local backups: {[any]: any} = dataTab.backups or (function()
		local tab = {}
		dataTab.backups = tab
		return tab
	end)()
	...
	if not (dataTab.loadingMap or dataTab.backingUpMap) then
		...
		local backupData = (function()
			for i,data in pairs(backups) do
				if data.name == slotName or i == slotInd then
					return data
				end
			end
		end)() or {
...
function utility:loadMapBackup(slotName: string, slotInd: number, exceptions: {[any]: any}, dataTab: {[any]: any}, savedBackup: {[any]: any}?)
	dataTab = dataTab or variables.mapBackups
	...
	local backupData = savedBackup or (function()
...
function utility:purgeMap()
	if not self.purgingMap then
		...
		for i,desc in pairs(workSChildren) do
			if desc:IsA"Terrain" then
				table.remove(workSChildren, i)
			end
		end
		...
		for i,plr in pairs(service.getPlayers()) do
			...
			if char then
				...
				if charFromChildren then
					table.remove(workSChildren, charFromChildren)
				end

				if charFromDescs then
					table.remove(workSDescs, charFromDescs)
				end
				...
				for d,desc in pairs(char:GetDescendants()) do
					...
					if descFromChildren then
						table.remove(workSChildren, descFromChildren)
					end

					if descFromDescs then
						table.remove(workSDescs, descFromDescs)
					end
				end
			end
		end
			...
			:andThen(function()
				return Promise.new(function(resolve, reject, onCancel)
					...
					local function characterAdded(character)
						local mainHrp = character and (character:FindFirstChild("HumanoidRootPart") or character:FindFirstChild("Torso"))
			...
			:tap(function(locationCF)
				...
				local function makeExile()
					if jailInfo.active then
						...
						if jailInfo.exileBox then
							service.Delete(jailInfo.exileBox, 1)
						end
						...
						local checkProperties = {
							"CanTouch", "Transparency",
							"CFrame", "Archivable", "Locked", "CanCollide", "Anchored",
							"Parent"
						}
			...
			:andThen(function(locationCF)
				service.loopTask(loopCheckInd, 0.5, function()
					...
					if inGameTarget then
						...
						local curHrp = curCharacter and (curCharacter:FindFirstChild"HumanoidRootPart" or curCharacter:FindFirstChild"Torso")
...
function utility:unJailPlayer(player: {[any]: any}?, fakePlayer: boolean): boolean
	...
	if variables.jailedPlayers[jailIndex] then
		...
		if jailInfo._setupProcess then
			jailInfo._setupProcess:cancel()
		end
		
		if jailInfo._playerAddedListener then
			jailInfo._playerAddedListener:disconnect()
		end
		...
		if jailInfo._playerCharacterAdded then
			jailInfo._playerCharacterAdded:Disconnect()
		end
		...
		if exileInfo then
			...
			if exileInfo._object then
				service.Delete(exileInfo._object, 0.5)
			end
		end
		...
		if backpack then
			...
			Promise.each(jailInfo.items, function(item: Tool, index)
				return Promise.promisify(function()
					...
					:catch(function(err) failedItems += 1 end) --// Catch silent error
			end)
				:andThen(function()
				...
				:finally(function()
					if jailInfo.containedItemsFolder then
						service.Delete(jailInfo.containedItemsFolder, 1)
					end
				end)
		end
...
function utility:toggleMuteOnAfk(player: ParsedPlayer, state: boolean)
	local sData = player:getPData().serverData
	if state == nil then
		state = not sData.ToggleMuteOnAFK
	end
...
function utility:createReserveServer(publicName: string?, creatorId: number?, createOpts: {}?)
	...
	if creatorId then
		Datastore.addUserIdToData("PrivateServerProfile", reservedServerId, creatorId)
	end
	
	if publicName then
		Datastore.tableUpdate(nil, reserveListName, "Index", publicName, serverDetails)
	end
...
function utility:deleteReserveServer(privateServerId: string?, publicName: string?)
	...
	if not privateServerProfile and publicName then
		...
		if type(reserveList) == "table" then
			...
			if serverDetails then
				...
				server.Cross.sendToSpecificServers({serverDetails.serverId}, "Shutdown", "Private server was deleted.")
	...
	elseif privateServerProfile then
		...
		server.Cross.sendToSpecificServers({privateServerProfile.details.serverId}, "Shutdown", "Private server was deleted.")
...
function utility:addWhitelistUserToReserveServer(publicName: string, userId: number)
	...
	if reserveData then
		...
		if not table.find(players, userId) and #reserveData.whitelist+1 <= variables.serverCreationSettings.reserveMaxWhitelistsAndBlacklists then
	...
	if reserveData then
		...
		if table.find(players, userId) then
			...
			if not reserveData.closed and not reserveData.publicJoin then
				server.Cross.sendToSpecificServers({reserveData.details.serverId}, "KickPlayers", {userId}, "You've been removed from the reserved server")
			end
...
function utility:addBlacklistUserToReserveServer(publicName: string, userId: number)
	...
	if reserveData then
		...
		if not table.find(players, userId) and #reserveData.banlist+1 <= variables.serverCreationSettings.reserveMaxWhitelistsAndBlacklists then
			Datastore.tableUpdate(nil, reserveListName, "tableAdd", "banlist", userId)
			if not reserveData.closed then
				server.Cross.sendToSpecificServers({reserveData.details.serverId}, "KickPlayers", {userId}, "You've been blacklisted from the reserved server")
			end
			return true
		end
	end
...
function utility:getVRPlayers()
	...
	for i, player in service.getPlayers(true) do
		...
		if cliData and cliData.deviceType == "VR" then
