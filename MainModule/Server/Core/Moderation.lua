...
return function(envArgs)
	...
	local function Init()
		...
		local function checkGlobalBans()
			...
			if type(dataGlobalBans) ~= "table" then
				dataGlobalBans = {}
			end
			...
			if type(dataLegacyGlobalBans) ~= "table" then
				dataLegacyGlobalBans = {}
			end
	...
	server.Moderation = {
		...
		checkServerEntry = function(plr)
			...
			local whitelistIgnore = server.Studio or placeOwner or Moderation.checkAdmin(plr) or
				(not whitelistData.admins and Identity.checkTable(plr, settings.WhiteList_Players or {}) or Identity.checkTable(plr, whitelistData.whitelisted or {} or
				server.Roles:hasPermissionFromMember(plr, {"Manage_Server"})))
			...
			if variables.privateServerData then
				...
				if privateServerData.adminLock then
					if not Moderation.checkAdmin(plr) then
						return false,"EssPrivate-AdminOnly",originJobId
					end
				end
			end
			
			if Utility.shutdownState then
				return false,"Shutdown",Utility.shutdownReason
			end
			...
			--// Whitelist check
			if serverWhitelisted and not whitelistIgnore then
		...
		checkBanStatus = function(player: ParsedPlayer|Player|string|number, checker: "BanCase"|"Settings"|"Legacy"|"Compatible"|nil,
			...
			local playerUserId = if _playerType == "number" then math.floor(player)
			...
			if (not playerUserId or playerUserId < 0) and not customBanCase then
				return false
			end
			...
			local playerBanCase = if customBanCase then customBanCase else (function()
				for i, serverBanCase in Moderation.serverBanCases do
					if serverBanCase.status == "active" and table.find(serverBanCase.users, playerUserId) and not table.find(serverBanCase.releasedUsers, playerUserId) then
			...
			if not (Moderation.isBanDataFormatted(playerBanCase, "PlayerData") or Moderation.isBanDataFormatted(playerBanCase, "BanCase")) then
				return false
			end
		...
		checkBan = function(plr, checkInfo)
			...
			local function checkBanInfo(ban)
				if ban.type ~= "Time" then
					if ban.offender.name == plr.Name or ban.offender.userid == plr.UserId or ban._offenderId == plr.UserId then
						return true,ban
					end
				else
		...
		isBanDataFormatted = function(banData, checker: "Legacy"|"PlayerData"|"UniversalBans"|"List"|"BanNote"|nil)
			...
			if type(banData) ~= "table" then
				...
				local function checkType(givenType, value)
					...
					if valType == "table" and type(givenType) == "table" then
						for i, type in pairs(givenType) do
							if not checkType(type, value[i]) then
								return false
							end
						end
				...
				if not checker or (checker == "Legacy") then
					for i, type in pairs(legacy_formatTableType) do
						if not checkType(type, banData[i]) then
							return false
						end
					end
				end
				...
				if checker == "BanNote" then
					return type(banData[1]) == "number" and type(banData[2]) == "number"
		...
		addBan = function(userName: string, 
			banType: string,
			reason: string,
			registered: number?,
			...
			if banType == "Server" then
				...
				if userid > 0 then
					...
					server.Events.banAdded:fire(banType, {name = userName; userId = userid;}, banInfo, moderator, false)
						...
						if userid > 0 and Datastore.canWrite() then
							...
							Datastore.tableAdd(nil, Datastore_Scopes.LEGACY_BANCASES, banInfo, false, function()
								Logs.addLog("Admin", {
									...
									desc = "Id: "..banInfo.id.." | ExpireTime: "..banInfo.expireTime.." | Reason: "..tostring(reason or "-undefined-");
								})
		...
		createBan = function(banRegisterInfo: {
			...
			banRegisterInfo.useRobloxApi = if type(banRegisterInfo.useRobloxApi) == "boolean" then banRegisterInfo.useRobloxApi else true; --// Default: true
			banRegisterInfo.reason = if banRegisterInfo.reason then banRegisterInfo.reason:sub(1,200) else `No reason specified`
			banRegisterInfo.moderatorId = banRegisterInfo.moderatorId or 0
			
			assert(type(banRegisterInfo.moderatorId) == "number" and math.floor(banRegisterInfo.moderatorId) == banRegisterInfo.moderatorId and math.abs(banRegisterInfo.moderatorId) ~= math.huge, `Moderator id must be an integer`)
			assert(not banRegisterInfo.type or table.find({"Universal", "Server"}, banRegisterInfo.type), `Ban type {banRegisterInfo.type}`)
			banRegisterInfo.type = if not banRegisterInfo.type then `Server` else banRegisterInfo.type
			
			local banTimestamp = DateTime.now()
			local banStatusCode = if banRegisterInfo.type == "Universal" then (banRegisterInfo.expiresOn~=nil and 11002) or 11001
				else 11004 -- Server
			
			-- Check notes
			do
				assert(not banRegisterInfo.notes or
					(type(banRegisterInfo.notes)=="table" and service.isTableAnArray(banRegisterInfo.notes)), `Ban notes must be an array of messages`)
			...
			do
				assert(type(banRegisterInfo.users)=="table" and service.isTableAnArray(banRegisterInfo.users), "Banned users must be an array of userids/usernames")
				assert(#banRegisterInfo.users > 0 and #banRegisterInfo.users < 10, `Banned users cannot be empty and must list up to 10 users or less`)
				for i, userIdOrName: number|string in banRegisterInfo.users do
					local userValueType = type(userIdOrName)
					assert(userValueType=="string" or userValueType=="number", "Banned users list values doesn't contain a string or integer")
					assert(userValueType == "number" and math.floor(userIdOrName)==userIdOrName and userIdOrName > 0, "Banned users list values contains a decimal value")
					
					if userValueType == "string" then
						local originalName = userIdOrName
						userIdOrName = service.playerIdFromName(userIdOrName)
						if userIdOrName <= 0 then
							error(`User {originalName} does not exist`, 2)
						end
						
						banRegisterInfo.users[i] = userIdOrName
					else
						assert(userIdOrName > 0 and service.playerNameFromId(userIdOrName) ~= `[unknown]`, `User with id {userIdOrName} does not exist`)
					end
				end
				
				if banRegisterInfo.expiresOn then
					assert(type(banRegisterInfo.expiresOn)=="number" and banRegisterInfo.expiresOn > banTimestamp.UnixTimestampMillis
			...
			assert(not successCallback or type(successCallback) == "function" or Promise.is(successCallback), `Success callback must be a function/Promise/nil`)
			...
			local creationPromise = Promise.promisify(function()
				...
				Datastore.readAndWrite(nil, Datastore_Scopes.UNIVERSAL_BANS, function(listOfUniversalBans)
					...
					until
						#listOfUniversalBans == 0 or #listOfUniversalBans+1 <= BanList_Limits.MaxEntries
								...
								if player then
									...
									parsed:_kick(Parser:filterStringWithSpecialMarkdown(settings.banMessage, nil, {
										customReplacements = {
											...
											["moderator"] = if creationInfo.moderatorId > 0 then service.playerNameFromId(creationInfo.moderatorId)..` ({creationInfo.moderatorId})`
												elseif creationInfo.moderatorId == -1 then `AutoModerator` else `System`;
											...
											["startDate"] = Parser:osDate(math.floor(creationInfo.startedOn/1000), nil, "longdatetime").." UTC";
											["dueDate"] = if creationInfo.isPermanent then `Unknown` else Parser:osDate(math.floor(creationInfo.expiresOn/1000), nil, "longdatetime").. " UTC";
											["relativeStartTime"] = Parser:relativeTimestamp(math.floor(creationInfo.startedOn/1000));
											["relativeEndTime"] = Parser:relativeTimestamp(math.floor(creationInfo.expiresOn/1000));
										}
									}))
								end
							end)
				...
				:andThen(function()
					...
					if not isBanCaseOnlyOnServer and banRegisterInfo.useRobloxApi then
						service.Players:BanAsync({
							...
							Duration = if banRegisterInfo.expiresOn then
								math.max(math.floor((banRegisterInfo.expiresOn-banTimestamp.UnixTimestampMillis)/1000), 1)
								else -1;
							DisplayReason = "[Essential Ban "..banStatusCode.."]\n".."Case Id: "..banCaseId.."\n"
								.."Moderator Id: "..banRegisterInfo.moderatorId.."\n"
								..""
								.."\nContact the in-game administrators for ban appeal or the full details of your "..banRegisterInfo.type.." ban";
				...
				:catch(function(err)
					warn(`Moderation createBan encountered an error while registering ban case for {table.concat(banRegisterInfo.users, ", ")}:\n{tostring(err)}`)
				end)
			
			if Promise.is(successCallback) then
				creationPromise = creationPromise:andThen(function()
					return Promise.resolve(banCaseId):andThen(successCallback)
				end)
		...
		addNoteToBanCase = function(caseId: string, noteMessage: string)
				...
				if type(banCase) == "table" then
					if type(banCase.notes) == "table" then
						...
						if lengthOfBanNotes+1 > BanCase_Limits.Notes_Length then
							...
							until
								#banCase.notes+1 >= BanCase_Limits.Notes_Length
						end
						
						
						table.insert(banCase.notes, {newNoteId, DateTime.now().UnixTimestampMillis, noteMessage:sub(1,200)})
					end
			...
			if caseId:sub(1,2) == `S-` then
				...
				if serverBanCase then
					updateNotesInBanCase(serverBanCase)
				end
				
				return
			end
			
			Datastore.readAndWrite(Datastore_Scopes.BAN_CASES, caseId, function(banCase: string|number|boolean|{[any]: any}|nil)
				if Moderation.isBanDataFormatted(banCase, "BanCase") then
					updateNotesInBanCase(banCase)
				end
		...
		resolveBanCase = function(caseId: string, updateCallback)
						...
						if not isCaseIdServerOnly then
							Promise.each(banCase.users, function(userId, index)
								...
								:andThen(function()
									local banCache = Moderation.globalBanCases[caseId]
									if banCache then
										Moderation.globalBanCases[caseId] = nil
									end
									
									local listedBanCache = Moderation.globalBanCasesList[caseId]
									if listedBanCache then
										Moderation.globalBanCasesList[caseId] = nil
									end
			...
			Datastore.readAndWrite(nil, Datastore_Scopes.UNIVERSAL_BANS, function(listOfUniversalBans)
				...
				for i, minimizedBanCase in listOfUniversalBans do
					if Moderation.isBanDataFormatted(minimizedBanCase, "UniversalBans") and minimizedBanCase.caseId == caseId then
		...
		resolveBanCaseForPlayers = function(caseId: string, userIds: {[number]: number})
			...
			local function resolveBanCase(banCase, ignoreCase)
				if type(banCase) == "table" or ignoreCase then
					if not isCaseIdServerOnly then
						Promise.each(userIds, function(userId, index)
							if type(userId) ~= "number" then end
					...
					for i, userId in userIds do
						if type(userId) == "number" and table.find(banCase.users, userId) and not table.find(_resolvedUserIds, userId) then
		...
		getBanCase = function(caseId: string)
			...
			if not existingCache or tick()-existingCache._cacheUpdated > 300 then
				...
				if not dataBanCase or not Moderation.isBanDataFormatted(dataBanCase, "BanCase") then
					...
					if existingCache.status == "active" and existingCache.expiresOn
		...
		removeBanProcess = function(process)
			...
			if processInd then
				table.remove(Moderation.banProcessors, processInd)
			end
		end;
				...
				for i,ban in pairs(serverBans) do
					if type(ban) == "table" then
						if ban.type == banType and ban.offender.userid == userid then
							serverBans[i] = nil
							server.Events.banRemoved:fire(banType, {name = ban.offender.name; userId = ban.offender.userid;}, ban, responsibleMod, false)
							didAttempt = true
						end
			...
			if banType == "Game" or banType == "Time" then
				...
				if dataBan and type(dataBan) == "table" then
					if (banStatus == true or banStatus == -1) and dataBan.type:lower() == banType:lower() then
						...
						server.Events.banRemoved:fire(banType, {name = service.playerNameFromId(userid) or "[unknown]"; userId = userid;}, dataBan, responsibleMod, true)
						return true
					end
		...
		warnPlayer = function(player: ParsedPlayer, warnOptions: {[any]: any})
			...
			if type(pData.warnings) ~= "table" then
				pData.warnings = {}
			end
		...
		checkAdmin = function(check)
			check = (typeof(check)=="Instance" and check:IsA"Player" and check.UserId) or
				(getmetatable(check)=="EP-"..check.UserId and check.UserId) or check
		...
		hasPermissions = function(check, perms)
			return server.Roles:hasPermissionFromMember(check, perms)
		end;
		...
		getIncognitoPlayers = function()
			...
			for i, player in service.getPlayers() do
				...
				if parsedPlayer and parsedPlayer:isPrivate() then
					table.insert(list, parsedPlayer)
				end
			end
				...
				:andThen(function()
					return Promise.each(service.getPlayers(), function(player)
						...
						if parsedPlayer and parsedPlayer:isInGame() then
							server.PolicyManager:setPolicyForPlayer(parsedPlayer, "INCOGNITO_PLAYERS", listOfIncognitosInUserId, `DYNAMIC`)
						end
					end)
				end)
		end;
