...
return function(envArgs)
	...
	local checkSelectors = {
		{
			...
			runCheck = function(target, matchArgs)
				local matchType = type(matchArgs[1])
				
				
			end;
			
			tableCheck = function(target, tab)
	...
	server.Identity = {
				...
				if type(cache) == "table" then
					service.trackTask("UPDATE GROUP "..i.." CACHE", true, function()
						local suc,ret = pcall(function()
							return service.GroupService:GetGroupsAsync(i) or {}
						end)
			...
			if type(uid) == "number" then
				...
				if canUpdate then
					...
					local suc,result = pcall(function()
						return service.UserService:GetUserInfosByUserIdsAsync({uid}) or {}
					end)
		...
		getFriends = function(uid, updateCache)
			...
			if type(uid) == "number" then
				...
				if canUpdate then
					...
					local suc,friendsPage = pcall(function()
						return service.Players:GetFriendsAsync(uid) or {}
					end)
		...
		checkMatch2 = function(target, check)
			...
			local playerId = (targetType=="number" and target) or (targetType=="userdata" and tostring(getmetatable(target)):sub(1,3)=="EP-" and target.UserId) or
				(typeof(target)=="Instance" and target:IsA"Player" and target.UserId) or nil
			...
			local function doCheck(userId)
				if checkType == "string" then
					...
					elseif check:match("^Group:(.+)") then
						...
						if tonumber(match) then
							...
							local playerRank = (tonumber(groupId) and Identity.getGroupLevel(userId, tonumber(groupId))) or 0

							if tonumber(groupId) then
								if ranks:sub(1,2) == ">=" then
									local minimumRank = tonumber(match:sub(3))

									if minimumRank and playerRank >= minimumRank then
										return true
									end
								elseif ranks:sub(1,2) == "<=" then
									local maximumRank = tonumber(match:sub(3))

									if maximumRank and playerRank <= maximumRank then
										return true
									end
								elseif ranks:sub(1,2) == "==" then
									local specifiedRank = tonumber(match:sub(3))

									if specifiedRank and playerRank == specifiedRank then
										return true
									end
								elseif ranks:match("^(%d+)-(%d+)") then
									...
									if minRank and maxRank then
										if playerRank >= minRank and playerRank <= maxRank then
											return true
										end
									end
								end
					...
					elseif check:match("^Gamepass:(%d+)") then
						...
						if match then
							return service.checkPassOwnership(userId, match)
						end
					...
					elseif check:match("^Asset:(%d+)") then
						...
						if tonumber(match) then
							return service.Marketplace:PlayerOwnsAsset(plr, tonumber(match))
						end
					elseif check:match("^FriendsWith:(%d+)") then
						local match = check:match("^FriendsWith:(%d+)")

						if tonumber(match) then
							return Identity.friendsWith(userId, tonumber(match))
						end
					elseif check:match("^*PrivateServerOwner$") then
						if #game.PrivateServerId > 0 then
							return game.PrivateServerOwnerId == userId
						end
					...
					elseif check:match("^User:(%w+)$") or check:match("^Player:(%w+)$") then
						...
						if checkUserId > 0 and checkUserId == userId then
							return true
						end
					elseif check:match("^UserId:(d+)$") or check:match("^PlayerId:(d+)$") then
						...
						if match > 0 and match == userId then
							return true
						end
					else
						local checkUserId = service.playerIdFromName(check) or 0
				...
				elseif checkType == "table" then
					...
					if selectType == "Membership" and plr then
						...
						local hasPremium = (checkInPlayer and plr.MembershipType == Enum.MembershipType.Premium) or false
						...
						for i,v in pairs(list) do
							if type(v) == "string" then
								...
								elseif (v:lower() == "donator" or v:lower() == "donor") and Identity.checkDonor(userId, plr) then
									return true
								end
							end
						end
					...
					if selectType == "Users" or selectType == "Players" then
						...
						for i,userName in pairs(list) do
							...
							elseif type(userName) == "number" then
								if userName > 0 and userName == userId then
									return true
								end
							end
						end
		...
		checkTable = function(check, table)
			...
			for i,v in pairs(table) do
				if Identity.checkMatch(check, v) then
					return true
				end
			end
		...
		checkPlaceOwner = function(checker)
			if Parser:isParsedPlayer(checker) or (typeof(checker) == "Instance" and checker:IsA"Player") then
				return (game.CreatorType==Enum.CreatorType.User and game.CreatorId==checker.UserId) or
					(game.CreatorType==Enum.CreatorType.Group and Identity.getGroupLevel(checker.UserId, game.CreatorId)==255) or
					false
			elseif type(checker) == "number" then
				return (game.CreatorType==Enum.CreatorType.User and game.CreatorId==checker) or
					(game.CreatorType==Enum.CreatorType.Group and Identity.getGroupLevel(checker, game.CreatorId)==255) or
					false
			else
				return false
			end
