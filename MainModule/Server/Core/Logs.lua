	...
	server.Logs = {
					...
					for i, catg in pairs(categories) do
						...
						if logList then
							...
							if logsCount + 1 > (customMaxLogs or settings.MaxLogs) then
								...
								until #logList == 0 or #logList + 1 <= maxLogsCount
							end

							table.insert(logList, logData)

							table.sort(logList, function(logA, logB) return (logA.sent or 0) > (logB.sent or 0) end)
						end
					end
		...
		addLogForPlayer = function(
			...
			if type(log) == "string" then
				...
				log.data._original = {
					...
					title = Parser:filterStringWithSpecialMarkdown(
						log.title or log.desc,
						logDelimiter,
						specialTextSettings_Private
					),
					userId = player.UserId,
				}

				if not log.data._original.title then log.data._original.title = log.data._original.desc end

				log.desc = Parser:filterStringWithSpecialMarkdown(log.desc, logDelimiter, specialTextSettings_Public)
				log.title = Parser:filterStringWithSpecialMarkdown(
					log.title or log.desc,
					logDelimiter,
					specialTextSettings_Public
				)
			end
		...
		Reporters = {
			Promise = {
				...
				issue = function(
					reportPrefix: string,
					reportCategories: { [number]: string } | string,
					chainedHandler: () -> any?
				)
					assert(type(reportPrefix) == "string", `Report prefix must be a string`)
					assert(
						type(reportCategories) == "string" or type(reportCategories) == "table",
						`Report categories must be a string/table`
					)
					...
					return function(err: string | PromiseError)
						...
						if server.Studio then warn(`A promise encountered an error:`, tostring(err)) end

						if chainedHandler then task.spawn(chainedHandler, err) end
					end
				end,
