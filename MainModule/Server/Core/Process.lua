	...
	server.Process = {
		...
		playerAdded = function(plr: Player)
			if Core.clients[plr] then
				...
				if not clientData.replicator then warn(`Unable to setup client data replicator for the player {tostring(parsedPlayer)}`) pcall(plr.Kick, plr, "Network replicator unavailable") return end
				...
				clientData._processTask = Promise.promisify(function()
					...
					if not canJoin then
						if denyType == "Ban" then
							...
							parsedPlayer:_kick(Parser:filterStringWithSpecialMarkdown(settings.banMessage, nil, {
								customReplacements = {
									["statusCode"] = if banInfo.caseId:sub(1,2) ~= "S-" then (banInfo.expiresOn~=nil and 11002) or 11001
										elseif banInfo.caseId:sub(1,2) == "L-" then 11003
										else 11004;
									["moderator"] = if banInfo.moderatorId > 0 then service.playerNameFromId(banInfo.moderatorId)..` ({banInfo.moderatorId})`
										elseif banInfo.moderatorId == -1 then `AutoModerator` else `System`;
									["mod_id"] = banInfo.moderatorId;
									["moderatorId"] = banInfo.moderatorId;
									["id"] = banInfo.caseId;
									["caseId"] = banInfo.caseId;
									["reason"] = banInfo.reason;
									["startDate"] = Parser:osDate(math.floor(banInfo.startedOn/1000), nil, "longdatetime").." UTC";
									["dueDate"] = if banInfo.isPermanent then `Unknown` else Parser:osDate(math.floor(banInfo.expiresOn/1000), nil, "longdatetime").. " UTC";
									["relativeStartTime"] = Parser:relativeTimestamp(math.floor(banInfo.startedOn/1000));
									["relativeEndTime"] = Parser:relativeTimestamp(math.floor(banInfo.expiresOn/1000));
								}
							}))
						end
						...
						if denyType == "EssPrivate-Invite" then
							...
							if originJobId then
								...
								tpInitFailed:connect(function(failedPlr, tpResult, tpErrMessage, tpPlaceId, usedTpOptions)
						...
						if denyType == "Whitelist" then
							...
							parsedPlayer:_kick(Parser:replaceStringWithDictionary("\n"..tostring(settings.lockMessage), {
								["{user}"] 			= plr.Name.." #"..plr.UserId;
								["{name}"] 			= plr.Name;
								["{displayname}"] 	= plr.DisplayName;
								["{userid}"] 		= plr.UserId;
								["{moderator}"] 	= (whitelistData.moderator.name.." #"..whitelistData.moderator.userid) or "SYSTEM";
								["{mod}"] 			= (whitelistData.moderator.name.." #"..whitelistData.moderator.userid) or "SYSTEM";
					...
					if didProcessBan then
						return
					end
				end)()
					:catch(function(err)
						parsedPlayer:_kick(`[Essential Process Error] An issue occurred while verifying your join request to the game:\n{tostring(err)}`)
					end)
				
				clientData._loadingPData = Promise.promisify(Core.getPlayerData)(plr.UserId)
					:andThen(function(playerData)
						if playerData._dataUpdate then
						...
						do
							...
							if not server.Studio then
								activityLogs._pushToSet(Logs.formatLog("Joined server "..serverType.." "..serverId))
					...
					:andThen(function(playerData)
						local banCheck; banCheck = playerData._updated:connect(function()
						...
						if not serverPData.joined then
							serverPData.joined = os.time()
						end
					...
					:andThen(function()
						if parsedPlayer:isPrivate() then
							task.spawn(Moderation.updateIncognitoPlayersDynamicPolicy)
						end
					end)
				...
				if parsedPlayer:isInGame() then
					...
					Logs.addLogForPlayer(parsedPlayer, `Process`,
			...
			if clientData then
				...
				if playerData then
					...
					for i, role in pairs(Roles:getAll()) do
						if role:checkTempMember(plr) then
				...
				-- Check for incognito
				if parsed:isPrivate() then
					Moderation.updateIncognitoPlayersDynamicPolicy()
				end
				
				if playerData and playerData._dataUpdate and (Utility.shutdownState or (server.Studio and playerData._dataChanged) or (not server.Studio and server.Running)) then
		...
		remoteCall = function(plr, ignoreIndexCheck, invoke, key, ...)
			...
			local ratePass,didThrottle,canThrottle,curRate,maxRate,throttleResetOs = Utility:deferCheckRate(rateData, rateKey)
			...
			if clientData and ratePass then
				...
				local realRemoteServerKey = if endToEndEncryption then HashLib.sha1(clientData.remoteServerKey) else clientData.remoteServerKey
				...
				if isKeyValidType and realRemoteServerKey == key then
					...
					if cmdName then
						...
						if cmd and not cmd.Disabled then
							Logs.addLog("Remote", {
								title = plr.Name..": Indexed command "..tostring(cmdName).." ("..type(cmdName)..")";
								desc = "Index: "..tostring(cmdName).." | Index type: "..type(cmdName).." | Invoke: "..tostring(invoke);
							})
							...
							local userWhitelisted = ignoreIndexCheck or (whitelist and Identity.checkTable(plr, whitelist)) or false
							local userBlacklisted = not ignoreIndexCheck and (whitelist and Identity.checkTable(plr, blacklist)) or false
							local userHasPermissions = ignoreIndexCheck or (permissions and Roles:hasPermissionFromMember(plr, permissions))
								or false

							local userAdmin = ignoreIndexCheck or Moderation.checkAdmin(plr)
							local canAccess = ignoreIndexCheck or userAdmin or ((publicUse or userHasPermissions or userWhitelisted) and not userBlacklisted)
							...
							if (not lockdown or (lockdown and cmd.Lockdown_Allowed)) and canAccess then
								Logs.addLog("Remote", {
									title = plr.Name..": Granted access to command "..tostring(cmdName).." ("..type(cmdName)..")";
									desc = "Index: "..tostring(cmdName).." | Index type: "..type(cmdName).." | Invoke: "..tostring(invoke);
								})
								...
								if not (cmd.Can_Invoke or cmd.Can_Fire) then
									cmd.Can_Fire = true
								end
								...
								local rL_Data		= cmd.RL_Data or (function()
									local data = {}
								...
								local canUseCommand = (invoke and cmd.Can_Invoke) or (not invoke and cmd.Can_Fire) or false

								if canUseCommand and cmdFunction then
									-- Command rate limit check
									if rL_Enabled then
										local passCmdRateCheck,curRemoteRate,maxRemoteRate = Utility:deferCheckRate(rL_Data, rateKey)

										if not passCmdRateCheck then
											Logs.addLog("Remote", {
												title = plr.Name..": Failed command "..tostring(cmdName).." ("..type(cmdName)..")'s rate limit.";
												desc = "Current rate: "..tostring(curRemoteRate).." | Max rate: "..tostring(maxRemoteRate);
											})

											warn("Player "..plr.Name.." is sending too many requests to "..cmdName.." (Threshold: "..curRemoteRate-maxRemoteRate..")")
											return (type(rL_Error)=="string" and rL_Error) or nil;
										end
									end

									Logs.addLog("Remote", {
										title = plr.Name..": Accessing command "..tostring(cmdName).." ("..type(cmdName)..")";
										desc = "Index: "..tostring(cmdName).." | Index type: "..type(cmdName).." | Invoke: "..tostring(invoke);
									})

									local parsed = Parser:apifyPlayer(plr)
									local rets = {service.trackTask("_REMCOMMAND-"..cmdName.."-Invoke:"..tostring(invoke).."-"..plr.UserId, false, cmdFunction, parsed, {unpack(params, 2)}, remoteData)}

									if not rets[1] then
										Logs.addLog("Remote", {
											title = plr.Name..": Encountered an error while running command "..tostring(cmdName).." ("..type(cmdName)..") successfully";
											desc = "Returned arguments count: "..tostring(#{unpack(rets, 2)});
										})

										warn("Player "..plr.Name.." encountered an error while running remote "..cmdName..": "..tostring(rets[2]))
										-- Don't return the error ret to the client. It's never a good thing for them to see the error
									else
										Logs.addLog("Remote", {
											title = plr.Name..": Required command "..tostring(cmdName).." ("..type(cmdName)..") successfully";
											desc = "Returned arguments count: "..tostring(#{unpack(rets, 2)});
										})
								...
								elseif canUseCommand and not cmdFunction then
									warn("Player "..plr.Name.." is sending a request to "..cmdName.." with a missing function")
								end
							end
						elseif not cmd then
							Logs.addLog("Remote", {
								title = plr.Name..": Attempted to access an non-existent command "..tostring(cmdName).." ("..type(cmdName)..")";
						...
						Logs.addLog("Remote", {
							title = plr.Name..": Attempted to invoke/fire remote call "..tostring(params[1]).." ("..type(params[1])..")";
							desc = "Suspicious activity was noted by system.";
						})
					end
				end
			elseif not ratePass then
				if didThrottle then
					warn("Player "..plr.Name.." throttled remote call (+"..curRate-maxRate.."). Try sending less remote calls to prevent throttling !!")
					local warnRatePass, warnDidThrottle, warnCanThrottle = Utility:deferCheckRate(Process.remoteCall_WarnBlockRateLimit, rateKey)
					...
					Moderation.addBan(plr.Name, "Time", "Spammed remote network", os.time(), {name = "SYSTEM"; userid = -1;}, os.time()+1200, false)
				end
			end
		end;
		...
		playerCommand = function(plr, msg, data)
			...
			if msg and msg:find(settings.batchSeperator) then
				if not (data.noBatch or settings.NoCommandsInBatch) then
					local maxCommandsInABatch = math.floor(math.clamp(tonumber(data.maxCommands or settings.MaxBatchCommands), 1, 50))
					...
					if not data._ranPlayerCommands then
						data._ranPlayerCommands = {}
					end
					...
					for i = 1,maxCommandsInABatch,1 do
						...
						if batch then
							local aliasCmdLineFromBatch = if PolicyManager:getPolicyFromPlayer(plr, "ALIASES_ALLOWED").value ~= false then Core.getCmdAliasFromBatch(plr, batch) else nil
							local ran = Process.playerCommand(
								plr,
								aliasCmdLineFromBatch or batch,
								(aliasCmdLineFromBatch and setmetatable({aliasRan = true; _ranPlayerCommands = data._ranPlayerCommands;}, {__index = data;})) or data
							)
				...
				-- Alias check
				local aliasCmdLineFromMsg = if msg and PolicyManager:getPolicyFromPlayer(plr, "ALIASES_ALLOWED").value ~= false then Core.getCmdAliasFromBatch(plr, msg) else nil
				if aliasCmdLineFromMsg then
					return Process.playerCommand(plr, aliasCmdLineFromMsg, setmetatable({aliasRan = true; _ranPlayerCommands = data._ranPlayerCommands;}, {__index = data;}))
				end
				...
				if msg then
					for coreName,coreCmd in pairs(Cmds.CoreCommands) do
						...
						if match and #match > 0 then
							...
							local canAccess = not plr or coreCmd.Public or Roles:hasPermissionFromMember(plr, coreCmd.Roles) or (function()
								local checkedRoles = {}
								...
								for i,neededRole in pairs(coreCmd.Roles) do
									if not checkedRoles[neededRole:lower()] then
										return false
									end
								end
							...
							if canAccess then
								...
								if not coreCmdFunc then
									warn("Unable to execute core command "..coreName.." from player "..tostring(plr)..". Missing function?")
								else
									...
									local coreCmdFuncRets = {service.trackTask("_CORECOMMAND_"..coreName:upper().."_"..plr.UserId, false, coreCmdFunc, plr, match, data)}
									local success,error  = coreCmdFuncRets[1],coreCmdFuncRets[2]

									if not success then
										warn("Core command "..coreName.." ["..tostring(plr and plr.UserId).."] encountered an error: "..tostring(error))
									else
										if error == 0 then
											return "Return"
										end
				...
				if not commandProcessed then
					...
					if command then
						...
						local messageArgs = data.commandInputArgs or Parser:getArguments(msg:sub(utf8.len((aliasMatch or cmdMatch)..delimiter)+1), delimiter, {
							maxArguments = math.max(#cmdArgs, 1);
						})
						
						local canIgnoreCooldown = Roles:hasPermissionFromMember(plr, {"Ignore_Command_Cooldown"})
						local parsedArgs,missingArg,missingArgType,missingArgReason = Parser:filterArguments(messageArgs, cmdArgs, delimiter, plr)
						local canUse,checkError,checkErrorArg1 = Core.checkCommandUsability(plr, command, canIgnoreCooldown, data)

						if canUse then
							if not parsedArgs then
								if returnOutput then
									return false,cmdMatch,"Args_NotParsed",missingArg,missingArgType,missingArgReason,cmdArgs[missingArg]
								end

								local argumentName = tostring((type(cmdArgs[missingArg])=="table" and cmdArgs[missingArg].argument) or "unknown")
								...
								plr:sendData("SendMessage",
									"Command <u>"..Parser:removeRichTextTags(if command.Hidden then "[hidden]" else tostring(cmdMatch)).."</u> requires a valid argument: "..Parser:removeRichTextTags((missingArgReason or "Expected "..missingArgNameAndType)),
									"Command "..(if command.Hidden then "[hidden]" else tostring(cmdMatch)).." requires a valid argument: "..(missingArgReason or "Expected "..missingArgNameAndType),
									5, "Context")
							...
							if cmdRequireArgs and #parsedArgs < #cmdArgs then
								if returnOutput then
									return false,cmdMatch,"Args_NotFilled"
								end

								plr:sendData("SendMessage",
									"Command <u>"..Parser:filterForRichText(if command.Hidden then "[hidden]" else tostring(cmdMatch)).."</u> requires all arguments filled in",
									"Command "..(if command.Hidden then "[hidden]" else tostring(cmdMatch)).." requires all arguments filled in",
									5, "Context")
							...
							if cmdFilterArgs then
								for i,parsedArgument in pairs(parsedArgs) do
									...
									if argType == "string" then
										parsedArgs[i] = select(2, Filter:safeString(parsedArgument, plr.UserId, plr.UserId))
									end
								end
							end
							...
							if cmdFunction then
								...
								-- Add log
								if not (data.NoLog or data.noLog or data.DontLog or data.dontLog) and
									not (command.DontLog or command.Loggable==false or command.NoLog) then

									local concatArguments = {}
									local wholeArgumentsHidden = if data.hideCommandArguments then true else false
									local commandLogPrefix = if customAliasRan then "[Action Alias] "
									...
									for i,arg in ipairs(messageArgs) do 
										local isArgumentHidden = wholeArgumentsHidden or if type(cmdArgs[i]) == "table" and cmdArgs[i].private then true else false
										
										table.insert(concatArguments, if isArgumentHidden then string.rep("*", math.clamp(utf8.len(arg), 1, 15)) else arg)
									end
									
									Logs.addLogForPlayer(plr, `Commands`,
										`\{\{$targetusername\}\} {cmdMatch}{delimiter}{Parser:filterForSpecialMarkdownTags(table.concat(concatArguments, delimiter))}`
									)
								...
								local cmdFuncRets = {service.trackTask("_COMMAND_"..cmdMatch:upper().."_"..plr.UserId, false, cmdFunction, plr, parsedArgs, data)}
								...
								if not success then
									if returnOutput then
										return false,cmdMatch,"CmdError",error
									end
									...
									plr:sendData("MakeUI", "NotificationV2", {
										...
										desc = `Please report this error to the game developer or Essential maintenance team`
											.. `\n\n<font color='#ff5c5c'>{Parser:filterForSpecialMarkdownAndRichText(tostring(error))}</font>`
											.. `\n<font color='#ed7b39'>{Parser:filterForSpecialMarkdownAndRichText(tostring(errorTrace))}</font>`;
										highPriority = true;
										priorityLevel = 5;
									})
									...
									Logs.addLogForPlayer(plr, "Process", {
										title = "{{$target}} encountered an error while running command "..tostring(cmdMatch)..": "..tostring(error):sub(1,400);
										desc = "Error: "..Parser:filterForSpecialMarkdownTags(tostring(error):sub(1,200));
									})

									if server.Studio or server.debugProcess then
										warn("Player command "..cmdMatch.." ["..plr.UserId.."] encountered an error: "..tostring(error), errorTrace)
									end
									...
									if retType == -1 then
										Core.manageCommandUsability(plr, command, manageType)
									end
							...
							if type(failFunc) == "function" then
								local failFuncRets = {service.trackTask("_COMMAND_FAIL_"..cmdMatch:upper().."_"..plr.UserId, false, failFunc, plr, data)}
								...
								if not failFuncRan then
									...
									if rawequal(failFuncRets[2], true) then
										return false,cmdMatch
									end
								end
							end
							...
							if not (command.Silent) then
								if returnOutput then
									return false,cmdMatch,"CmdInaccessible",checkError,checkErrorArg1,command.Hidden
								else
									local isCmdHidden = command.Hidden
									if not data.noReturn then
										if (checkError == "ServerCooldown" or checkError == "PlayerCooldown" or checkError == "CrossCooldown") and plr then
											plr:sendData("SendMessage",
												"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: You're on the cooldown for  "..tostring(checkErrorArg1).." seconds.",
												"Command "..tostring(cmdMatch)..": You're on the cooldown for  "..tostring(checkErrorArg1).." seconds.",
												5, "Context")
										elseif (checkError == "PlayerDebounce") and plr then
											plr:sendData("SendMessage",
												"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: This command is already in process previously. You must wait for its execution done before running this command again.",
												"Command "..tostring(cmdMatch)..": This command is already in process previously. You must wait for its execution done before running this command again.",
												5, "Context")
										elseif (checkError == "ServerDebounce") and plr then
											plr:sendData("SendMessage",
												"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: This command is already being used in the server. You must wait for its execution done before running this command again.",
												"Command "..tostring(cmdMatch)..": This command is already being used in the server. You must wait for its execution done before running this command again.",
												5, "Context")
										elseif (checkError == "Chat") and plr and not isCmdHidden then
											plr:sendData("SendMessage",
												"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: You cannot use this in chat.",
												"Command "..tostring(cmdMatch)..": You cannot use this in chat.",
												5, "Context")
										elseif (checkError == "MissingPerms") and plr and not isCmdHidden then
											plr:sendData("SendMessage",
												"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: You must have these permissions <b>"..Parser:filterForRichText(table.concat(checkErrorArg1, ", ")).."</b> to run.",
												"Command "..tostring(cmdMatch)..": You must have these permissions <b>"..table.concat(checkErrorArg1, ", ").." to run.",
												5, "Context")
										elseif (checkError == "MissingRoles") and plr and not isCmdHidden then
											plr:sendData("SendMessage",
												"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: You must have one of these roles <b>"..Parser:filterForRichText(table.concat(checkErrorArg1, ", ")).."</b> to run.",
												"Command "..tostring(cmdMatch)..": You must have one of these roles <b>"..table.concat(checkErrorArg1, ", ").." to run.",
												5, "Context")
										elseif (checkError == "CommandBlacklist") and plr and not isCmdHidden then
											plr:sendData("SendMessage",
												"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: You're blacklisted from using this command.",
												"Command "..tostring(cmdMatch)..": You're blacklisted from using this command.",
												5, "Context")
										elseif (checkError == "Disabled") and not isCmdHidden then
											plr:sendData("SendMessage",
												"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: This command is disabled via setting.",
												"Command "..tostring(cmdMatch)..": This command is disabled via setting.",
												5, "Context")
										elseif (checkError == "GlobalBlacklist") and plr then
											plr:sendData("SendMessage", "<font color='#e65545'><b>You are not permitted to run commands</b></font>", nil, 5, "Context")
										elseif (checkError == "RanTwice") and plr then
											plr:sendData("SendMessage", `<font color='#e65545'><b>You are not permitted to run the command <u>{Parser:filterForRichText(cmdMatch)}</u> twice </b></font>`, nil, 5, "Context")
										elseif (checkError == "SocialMediaPoliciesDisallowed") and plr then
											plr:sendData("SendMessage", `<font color='#e65545'>Your current social media policies are prohibiting you from running the command <u>{Parser:filterForRichText(cmdMatch)}</u>.</font> Required policies: {table.concat(checkErrorArg1, ", ")}`, nil, 5, "Context")
										else
											if plr then
												if command.Error then
													...
													if not suc then
														warn("Command "..cmdMatch.." error function encountered an error: "..tostring(res), 10, "Hint")
													end
												else
													...
													plr:sendData("SendMessage",
														"Command <u>"..Parser:filterForRichText(tostring(cmdMatch)).."</u>: You have insufficient permissions to use this.",
														"Command "..tostring(cmdMatch)..": You have insufficient permissions to use this.",
														5, "Context")
												end	
											end
										end
		...
		playerVerified = function(player: ParsedPlayer)
			...
			playerObj.Chatted:connect(function(msg)
				Logs.addLogForPlayer(player, `Chat`,
				...
				if Utility:isMuted(player.Name) then
					...
					if #msg > 0 then
						...
						if chatCommands and Utility:deferCheckRate(Process.chatProcessCommand_RateLimit, player.UserId) then
							Process.playerCommand(player, msg, {noReturn = true; chatted = true; robloxChat = true; maxCommands = settings.chatMaxCommands})
						end
					end
				end
			end)
			...
			playerObj.CharacterAdded:connect(function(char)
				server.Events.characterAdded:fire(player, char)
			end)

			if playerObj.Character then
				server.Events.characterAdded:fire(player, playerObj.Character)
			end

			local lastJoinedLog = player:getPData().serverData.lastJoinedLog
			local isPlayerAnAdmin = Moderation.checkAdmin(playerObj)
			
			local expectedWelcomeMessage = if isPlayerAnAdmin then settings.welcomeMessages.admins else settings.welcomeMessages.nonAdmins
				...
				player:sendData("MakeUI", "NotificationV2", {
					...
					desc = Parser:filterStringWithSpecialMarkdown(settings.welcomeMessages.admins, nil, {
						customReplacements = {
							...
							["membership"] = if isPlayerAnAdmin then `administrator` elseif Identity.checkDonor(player) then `donator` else `non-administrator`;
						}	
					}) ..
						(if not lastJoinedLog then `` else ` You last joined the game on \{\{t:{lastJoinedLog.sentOs}:ldt\}\}.`);
					richText = true;
			...
			if deafened or muted then
				...
				if mainChannel then
					mainChannel:MuteSpeaker(player.Name)
				end
			end

			if variables.jailedPlayers[tostring(player.UserId)] and variables.jailedPlayers[tostring(player.UserId)].active then
			...
			if playerData._encryptionEnabled and not playerData.seenEncryptMessage then
				...
				impPM.opened:connectOnce(function()
					playerData.seenEncryptMessage = true
				end)
			...
			local lastUpdated, updateDuration, updateVers, updateInfo = changelog.lastUpdated, changelog.updateDuration,
			changelog.updateVers, changelog.updateInformation
			...
			do
				...
				service.loopTask(`AUTOUPDATE_DYNAMICPOLICIES-{player.UserId}`, 300, function()
					if not player:isInGame() then service.stopLoop(`AUTOUPDATE_DYNAMICPOLICIES-{player.UserId}`) return end
						...
						for i, messageBody in ipairs(messages._table) do
							if not watchedMsgIds[messageBody.id] then
								...
								task.defer(function()
									...
									local receiverName = receiverUserId and service.playerNameFromId(receiverUserId) or "[SYSTEM]"
									local privateMessage = Remote.privateMessage({
										receiver = player,
										topic = "Direct Message - "..tostring(messageBody.title or "[no title]")..(messageBody.isAReply and " [reply]" or "");
										desc = "<i>This message was sent directly from your player data. Opening this has marked your dm on read.</i>";
										message = table.concat({
											messageText;
											"";
											"";
											"<i>Sent on "..Parser:osDate(messageBody.sent).." UTC by player <b>"..(receiverName).."</b> </i>";
											"<i>-------</i>";
											"<i>Replied to:</i>";
											...
											task.spawn(function()
												...
												if not canSendMessage then
													player:sendData("SendMessage",
														"Your private message for a player with user id <b>"..receiverUserId.."</b> cannot send due to non-existent target",
														nil,
														8, "Context")
												else
													...
													if targetPlayer then
														...
														player:sendData("SendMessage",
															"Successfully replied to your dm with player <b>"..tostring(targetPlayer).."</b>",
															nil,
															4, "Context")
													end
												end
			...
			do
				if not server.Studio then
					...
					service.loopTask(sessionUpdateloopIndex, 180, function()
						if player:isInGame() then
							playerData.serverDetails = {
								...
								serverAccessCode = variables.privateServerData and variables.privateServerData.serverAccessId or nil;
					...
					player.disconnected:connectOnce(function()
						service.stopLoop(sessionUpdateloopIndex)
					end)
				end
			end
		...
		playerCheckIn = function(player)
			...
			if cliData and cliData.verified and not cliData.checkingIn then
				...
				if not lastCheckIn or (os.time()-lastCheckIn > 10) then
					...
					local link; link = server.Events.playerCheckIn:connect(function(checkInPlr)
					...
					repeat
						wait()
					until
					readyToMoveOn or not parsed:isInGame() or (os.time()-stWaiting > 120)
					...
					if not readyToMoveOn and not parsed:isInGame() then
						return
					end
