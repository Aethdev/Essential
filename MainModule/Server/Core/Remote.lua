...
return function(envArgs)
	...
	local function checkTableFormat(tab, formatTable)
		local function checkValue(value, givenType)
			...
			if type(givenType) == "table" then
				...
				if isRequired and valueType ~= "table" then
					return false
				end
					
				if valueType == "table" then
					for i, subType in givenType do
						if type(i) == "string" and i:sub(1,2):lower() == "__" then
							continue
						end
						if not checkValue(value[i], subType) and not isRequired then
							return false
						end
					end
					
					for i, value in tab do
						if checkValue(value, givenType[i]) == false and not isRequired then
							return false
						end
					end
				end
					
				if not isRequired and valueType ~= "table" and valueType ~= "nil" then
					return false
				end
				...
				for i, possibleType in possibleTypes do
					if valueType == possibleType:lower() then
						return true
					end
				end
		...
		for index, indexedType in formatTable do
			if not checkValue(tab[index], indexedType) then
				return false
			end
		end
	...
	local function createLogLibraryFunction(libraryName: string)
		return function(plr)
			...
			for i, logData: {
				title: string, desc: string, data: {_original: {title: string, desc: string}}?
			...
			table.sort(tab, function(new, old)
				return new.sent > old.sent
			end)
	...
	server.Remote = {
			...
			Commands = {
				...
				Function = function(plr)
					...
					for i,cmd in pairs(Commands.Library) do
						if not (cmd.Disabled or (cmd.Hidden and not cmd.DontHideFromList)) and Core.checkCommandUsability(plr, cmd, true) then
							...
							for i,arg in pairs(cmd.Arguments) do
								if type(arg) == "table" then
									table.insert(concatArguments, Parser:filterForRichText("<"..tostring(arg.argument or i)..">"))
								else
							...
							for i, roleName in (cmd.Roles or {}) do
								...
								if actualRole then
									...
									table.insert(concatRoles, if not hiddenFromList and (not hiddenFromLowerRank or playerPriority >= actualRole.priority) then
										Parser:filterForRichText(roleName) else "<i><font color=\"#4E4D50\">hidden</font></i>")
								end
							end
							...
							table.insert(availableCmds, {
								...
								description = table.concat({
									...
									`<b>Category</b>: {if cmd.Category then Parser:filterForRichText(cmd.Category) else "<i>none</i>"}`;
									`<b>Aliases ({#cmd.Aliases-1})</b>: {if #concatAliases == 0 then `<i>none</i>` else concatAliases}`;
		...
		Commands = {
			...
			AddClientLog = {
				...
				Function = function(plr, args)
					...
					elseif logType == "table" then
						Logs.addLog(`Client`, {
							title = if type(logMessageOrData.title) == "string" and #logMessageOrData.title > 0 then `[{tostring(plr)}]: {logMessageOrData.title}` else nil;
							desc = if type(logMessageOrData.desc) == "string" and #logMessageOrData.desc > 0 then logMessageOrData.desc else nil;
							group = if type(logMessageOrData.group) == "string" and #logMessageOrData.group > 0 then logMessageOrData.group else nil;
				...
				Function = function(plr, args)
					if type(args[1]) == "string" then
						...
						if listD and Utility:checkRate(listD.RateLimit, plr.UserId) then
							local hasPermissionToView = Roles:hasPermissionsFromMember(plr, listD.Permissions) or Identity.checkTable(plr, listD.Whitelist)

							if hasPermissionToView then
								return listD.Function(plr, unpack(args, 2))
							end
						end
					end
				end;
			...
			HasPermissions = {
				...
				--> Supported command functions: Function, Run, Execute, Call
				Function = function(plr, args)
					local checkTab = (type(args[1])=="table" and args[1]) or (type(args[1])=="string" and {args[1]}) or nil

					if checkTab then
						return Roles:hasPermissionsFromMember(plr, checkTab, args[3])
					end
				end;
			};
			...
			HasRoles = {
				...
				--> Supported command functions: Function, Run, Execute, Call
				Function = function(plr, args)
					local checkTab: {[any]: any} = (type(args[1])=="table" and args[1]) or (type(args[1])=="string" and {args[1]}) or nil
					local onlyAcceptOne: boolean = (args[2] and true) or false

					if checkTab then
						return Roles:checkMemberInRoles(plr, checkTab, onlyAcceptOne)
					end
				end;
			};
			...
			HasAdmin = {
				...
				--> Supported command functions: Function, Run, Execute, Call
				Function = function(plr, args)
					return Moderation.checkAdmin(plr)
				end;
			};
			...
			Verify = {
				...
				Function = function(plr, args)
					...
					if not cliData.verified then
						...
						assert(type(args[2]) == "table" and
							type(args[2].Reset) == "number" and
							type(args[2].Rates) == "number",
							...
							if msgData.active and not msgData:isExpired() then
								...
								if not msgData.noReply then
									...
									if not msgData.dontMessageSender and receiver then
										...
										local safeReply,filteredReply = server.Filter:safeString(replyMsg, plr.UserId, receiver.UserId)
			...
			ExecuteConsoleCommandV2 = {
				...
				Function = function(plr, args)
					...
					--if canExecute then
						...
						Process.playerCommand(plr, nil, {console = true; commandId = commandId; commandInputArgs = commandInputArgs;})
					--end
				end;
			};
			...
			GetPlayers = {
				...
				Function = function(plr, args)
					...
					if isAdmin and not Moderation.checkAdmin(plr) then
						return {}
					end
			...
			FirePlayerEvent = {
				...
				RL_Enabled 	= false; -- Rate limit enabled?
				RL_Rates 	= 3; -- (interval) (min: 1) Rate amount of requests
				RL_Reset 	= .6; -- (number) (min: 0.01) Interval seconds to reset rate's cache.
				RL_Error	= nil; -- (string) Error message returned after passing the rate limit
						...
						local foundEvent = (function()
							for i,event in pairs(PlayerEvents) do
								if event.active then
									if (event.noName and event.id == args[1]) or (not event.noName and event.name == args[1]) then
										return event
									end
								end
							end
				...
				Function = function(plr, args, remoteData)
					...
					if type(sessionId) == "string" then
						...
						if existingSession and existingSession:isActive() then
							if existingSession.network and remoteNetwork~=existingSession.network then
								return
							end
			...
			ConnectSession = {
				...
				Function = function(plr, args, remoteData)
					...
					if type(sessionId) == "string" and type(playerFireKey) == "string" and (#playerFireKey <= 40 and #playerFireKey >= 20) then
						local existingSession = ConnectedSessions[sessionId]
						...
						if existingSession and existingSession:isActive() then
							...
							local canConnect = existingSession.canConnect and (not currentConnectData or (currentConnectData.expireOs and os.time()-currentConnectData.expireOs >= 0))
							...
							if canConnect then
								...
								local selfConnectData; selfConnectData = {
									active = true;
									player = plr;
				...
				Function = function(plr, args, remoteData)
					...
					if type(sessionId) == "string" then
						...
						if existingSession and existingSession:isActive() then
							...
							if (remoteNetwork and sessionNetwork~=remoteNetwork) then
							...
							if existingSession:hasPermission(plr) then
								...
								elseif manageType == "ConnectEvent" and invokedCall then
									...
									if existingEv and existingEv:isActive() and existingEv:hasPermission(plr) and existingEv.canConnect then
										...
										if #playerFireId < 20 then
											...
											local connectData = existingEv:makeConnection(plr, playerFireId, (expireDuration>0 and os.time()+expireDuration) or nil)
								...
								elseif manageType == "DisconnectEvent" then
									...
									if existingEv and existingEv:isActive() then
										...
										for i, connectionData in pairs(existingEv.playerConnections) do
											...
											if conPlayer == plr then
												table.insert(playerEvents, connectionData)
											end
										end
									...
									if not eventId then
										...
										if existingEv then
											...
											if #playerConnections > 0 then
												existingEv:killConnections(plr.UserId)
											end
										end
									end
								...
								elseif manageType == "RunCommand" then
									...
									if existingCmd and existingCmd.active then
										...
										if (invokedCall and canInvoke) or (not invokedCall and canFire) then
											...
											if Identity.checkTable(plr, allowedTriggers) or cmdConnectedPlayers[plr] then
												local cmdFuncRets = {service.trackTask("_SESSION-"..sessionId.."-CMD-"..existingCmd.id:upper().."-"..plr.UserId, false, cmdFunction, plr, unpack(args, 4))}
												local success,error,errTrace = cmdFuncRets[1],cmdFuncRets[2],cmdFuncRets[3]

												if not success then
													warn("Session "..sessionId..", command "..existingCmd.id.." encountered an error: "..tostring(error or "[Unknown error]"))
													warn(errTrace)	
								...
								elseif manageType == "GetCommand" and invokedCall then
									for i,cmd in pairs(existingSession.commands) do
										if (cmd.name and cmd.name == args[3]) or cmd.id == args[3] then
											return cmd
										end
									end
								end
					...
					if type(args[1]) == "string" then
						for sessId, sessionData in pairs(ConnectedSessions) do
							...
							if easyFind then
								...
								if sessionNetwork and remoteNetwork~=sessionNetwork then
									continue
								end

								if sessionName and rawequal(sessionName, args[1]) then
									return sessId
								end
							end
						end
									...
									coroutine.wrap(function()
										...
										if openedNotif then
											...
											if playerChar and targetChar then
												...
												local playerHrp = playerChar:FindFirstChild"HumanoidRootPart" or playerChar:FindFirstChild"Torso"

												if (targetHrp and targetHrp:IsA"BasePart") and (playerHrp and playerHrp:IsA"BasePart") then
													playerHrp.CFrame = (targetHrp.CFrame + (targetHrp.CFrame.LookVector*2)) * CFrame.Angles(0, math.rad(180), 0)
													...
													plr:makeUI("Notification", {
														...
														desc = "You or "..otherPlr.Name.." didn't have a valid character";
														time = 2;
													})	
												end
											end
			...
			ManageClientSettings = {
				...
				Function = function(plr, args, remoteData)
					...
					if table.find(ignoreValTypes, type(settingVal)) then
						 return
					end
			...
			RejoinServer = {
				Disabled = not variables.privateServerData and (server.Studio or game.PrivateServerOwnerId>0) and true; -- Is this remote command disabled? Enabling this will block all requests' call indexing this command
				Public = true; -- Allow this command to run publicly? This ignores whitelist and permissions.
				...
				--> Supported command functions: Function, Run, Execute, Call
				Function = function(plr, args, remoteData)
					local debounceRets = {service.debounce("Rejoin server - "..plr.playerId, function()
					...
					if type(args[1]) == "string" then
						for originalName, subNetwork in pairs(SubNetworks) do
							if subNetwork and subNetwork.active then
								...
								if easyFind and subNetworkName and (subNetworkName==args[1] or originalName == args[1]) then
					...
					if type(args[1]) == "string" then
						for netId, subNetwork in pairs(SubNetworks) do
							if subNetwork and subNetwork.active and  subNetwork.id==args[1] then
								if not table.find(subNetwork.disallowedPlayerIds, plr.playerId) and (subNetwork.joinable or subNetwork.connectedPlayers[plr] or Identity.checkTable(plr, subNetwork.allowedTriggers)) then
			...
			CreateCommandAlias = {
				...
				Function = function(plr, args, remoteData)
					...
					if type(alias) == "string" and type(cmdLine) == "string" then
						...
						if (#shortenAlias>=2 and #shortenAlias<=playerSetsLimits.AliasNameCharLimit) and (#shortenCmdLine>0 and #shortenCmdLine<=playerSetsLimits.AliasAndShortcutCmdLineCharLimit) then
							local pData = plr:getPData()
							...
							if aliasCreationCount+1 > playerSetsLimits.MaxAliasCreation then
								return -1
							end
							...
							if selfActionAliases[shortenAlias:lower()] then
								...
								if not Filter:safeString(shortenAlias, plr.UserId) then
									return -2.5
								end
								
								if shortenAlias:find(cmdBatchSeparator) then
									return -3
								end
			...
			CreateCommandButton = {
				...
				Function = function(plr, args, remoteData)
					...
					if type(shortcutName) == "string" and type(cmdLine) == "string" then
						...
						if (#shortenShortcutName>=2 and #shortenShortcutName<=playerSetsLimits.ShortcutNameCharLimit) and (#shortenCmdLine>0 and #shortenCmdLine<=playerSetsLimits.AliasAndShortcutCmdLineCharLimit) then
							local pData = plr:getPData()
							local selfCmdShortcuts = pData.__shortcuts
							local shortcutCreationCount = service.tableCount(selfCmdShortcuts._table)

							if shortcutCreationCount+1 > playerSetsLimits.MaxShortcutCreation then
								return -1
							end
							...
							if selfCmdShortcuts[shortenShortcutName] then
								...
								if not Filter:safeString(shortenShortcutName, plr.UserId) then
									return -2.5
								end
			...
			DeleteCommandButton = {
				...
				Function = function(plr, args, remoteData)
					...
					if type(shortcutName) == "string" then
						...
						if (#shortenShortcutName>=2 and #shortenShortcutName<=playerSetsLimits.ShortcutNameCharLimit) then
							local pData = plr:getPData()
							local selfCmdShortcuts = pData.__shortcuts
				...
				Function = function(plr, args, remoteData)
					...
					if type(shortcutName) == "string" then
						...
						if (#shortenShortcutName>=2 and #shortenShortcutName<=playerSetsLimits.ShortcutNameCharLimit) then
							...
							if PolicyManager:getPolicyFromPlayer(plr, "SHORTCUTS_ALLOWED").value ~= false and selfCmdShortcuts[shortenShortcutName] then
				...
				Function = function(plr, args, remoteData)
					...
					if type(alias) == "string" and type(cmdLine) == "string" then
						...
						if (#shortenAlias>=2 and #shortenAlias<=playerSetsLimits.CustomCmdNameAliasCharLimit) then
							...
							if aliasCreationCount+1 > playerSetsLimits.MaxCustomCmdNameCreation then
								return -1
							end
							...
							if selfCmdAliases[shortenAlias:lower()] then
								...
								if not commandFromCmdLine then
									return -5
								end
			...
			CreateCmdKeybind = {
				...
				--> Supported command functions: Function, Run, Execute, Call
				Function = function(plr, args, remoteData)
					local keybindName,hotkeys,holdDuration,cmdLine,enabled = args[1], args[2], args[3], args[4], args[5]
					local playerSetsLimits = Process.playerSettingsLimits

					if type(keybindName) == "string" and type(hotkeys) == "table" and
						type(holdDuration) == "number" and holdDuration >= 0 and type(cmdLine) == "string" and #cmdLine > 0
					then
						local shortenKeybindName = Parser:trimString(keybindName)
						local shortenCmdLine = Parser:trimString(cmdLine)

						if (#shortenKeybindName>=2 and #shortenKeybindName<=playerSetsLimits.KeybindNameCharLimit) and #shortenCmdLine > 0 and #shortenCmdLine <= playerSetsLimits.AliasAndShortcutCmdLineCharLimit then
							local pData = plr:getPData()
							local selfCmdKeybinds = pData.__cmdKeybinds
							local keybindCreationCount = service.tableCount(selfCmdKeybinds._table)
							
							if keybindCreationCount+1 > playerSetsLimits.MaxKeybindCreations then
								return -1
							end
							...
							if selfCmdKeybinds[shortenKeybindName:lower()] then
								...
								if not Filter:safeString(shortenKeybindName, plr.UserId) then
									return -2.5
								end
			...
			RunCmdKeybind = {
				...
				Function = function(plr, args, remoteData)
					...
					if type(keybindName) == "string" then
						...
						if (#shortenKeybindName>=2 and #shortenKeybindName<=playerSetsLimits.KeybindNameCharLimit) then
							local pData = plr:getPData()
							local selfCmdKeybinds = pData.__cmdKeybinds
							local selfCliSettings = pData.__clientSettings
							local shortcutCreationCount = service.tableCount(selfCmdKeybinds._table)
							
							if PolicyManager:getPolicyFromPlayer(plr, "KEYBINDS_ALLOWED").value ~= false and selfCliSettings.KeybindsEnabled and selfCmdKeybinds[shortenKeybindName] then
								local keybindData = selfCmdKeybinds[shortenKeybindName]
			...
			GetCustomKeybinds = {
				...
				--> Supported command functions: Function, Run, Execute, Call
				Function = function(plr, args, remoteData)
					return plr:getPData().__customKeybinds._table
				end,
			};
			...
			ToggleMuteOnAFK = {
				...
				Function = function(plr, args, remoteData)
					if type(args[1]) == "boolean" then
						Utility:toggleMuteOnAfk(plr, args[1])
					end
				end,
			};
				...
				Function = function(plr, args, remoteData)
					if type(args[1]) == "boolean" then
						if not Utility:checkRate(Process.toggleIncognito_RateLimit.Global, plr.UserId) then
							plr:sendData("SendMessage", "<b>Global Roadblock</b>: Too many players are toggling incognito mode right now. Try again later!", nil, 5, "Context")
							return 0
						end
						
						if not Utility:checkRate(Process.toggleIncognito_RateLimit.Player, plr.UserId) then
							plr:sendData("SendMessage", `You can't update incognito mode too quick. Wait {Parser:relativeTimestamp(select(4, Utility:readRate(Process.toggleIncognito_RateLimit.Player, plr.UserId)))}`, nil, 5, "Context")
							return 0
						end
						...
						if incogStatus then
							plr:sendData("SendMessage", `Incognito mode is now enabled! Your display name is replaced with your corresponding code name in logs and messages.`, nil, 10, "Context")
							return true
						else
							plr:sendData("SendMessage", `You disabled incognito mode. Your display name in logs and messages will use your current Roblox display name.`, nil, 10, "Context")
							return false
						end
					end
				end,
			...
			EditDeviceType = {
				...
				--> Supported command functions: Function, Run, Execute, Call
				Function = function(plr, args, remoteData)
					plr:getClientData().deviceType = tostring(args[1])
				end,
			};
		};
		...
		privateMessage = function(creationData: {[any]: any}?)
			...
			if onRead then
				messageData.opened:connectOnce(onRead)
			end
			
			if onReply then
				messageData.replied:connectOnce(onReply)
			end
			...
			sessOpenNotif.execute = function(plr)
				if messageData:isExpired() then
					plr:sendData("SendMessage", "The private message you tried to open is expired and can no longer accept any replies.", nil, 5, "Context")
				else
			...
			function messageData:showToPlayer(plr: ParsedPlayer)
				if not self.openSince then
					self.openSince = os.time()
				end
				...
				plr:makeUI("PrivateMessageV2", {
					...
					time = (messageData.expireOs and math.clamp(messageData.expireOs-os.time(), 0, math.huge)) or
						(messageData.openTime and math.clamp(messageData.openTime, 2, math.huge)) or nil;
				})
			end
			...
			if not messageData.noReply then
			...
			task.spawn(function()
				if scheduledOs and scheduledOs-os.time() > 0 then
					repeat
						wait(.1)
					until
						not messageData.active or messageData:isExpired() or (scheduledOs-os.time() <= 0)

					if not messageData.active or not receiver:isInGame() then
					...
					if receiver:isInGame() then
						task.defer(function()
							...
							receiver:makeUI("NotificationV2", {
								...
								desc = (notifyOpts and notifyOpts.desc) or "From "..tostring((sender and sender:toStringDisplayForPlayer(receiver)) or "[unknown]");
		...
		makePoll = function(players, topic, desc, options, canSelectMultiple, tallyProgress, publicResults, expireOs)
			...
			submitPoll.execute = function(plr, chosenOpts)
				...
				if connectedPlayers[plr] and not pollSession.confirmPlayers[plr] and type(chosenOpts) == "table" then
					if #chosenOpts > 0 and ( (not canSelectMultiple and #chosenOpts==1) or (canSelectMultiple and #pollSession.options >= #chosenOpts) ) then
						local checkIds = {}
		...
		newSession = function(expireOs1)
				...
				if self.active and not self:findEvent(eventName) then
					...
					function eventData:killConnections(playerUserId, disconnectId)
						...
						for i, connectionData in pairs(self.playerConnections) do
							...
							if conPlayerId == playerUserId and (not disconnectId or connectionData.disconnectId == disconnectId) then
					...
					function eventData:makeConnection(player, fireEventId, expireOs)
						...
						if playerConnectionsMade+1 > self.maxPlayerConnections then
							return -1
						end
						...
						function connectionData:isActive()
							return self.active and (not self.expireOs or os.time()-self.expireOs < 0) and player:isInGame()
						end
					...
					function eventData:getConnections(player: ParsedPlayer|Player, playerUserId: number?)
						local results = {}
						for i,connectData in pairs(self.playerConnections) do
							if (player and connectData.player == player) or (playerUserId and connectData.player.UserId == playerUserId) then
					...
					function eventData:fireToSpecificPlayers(players, ...)
						...
						for i,connectionData in pairs(eventData.playerConnections) do
							...
							if allowedPlayers[conPlayer.UserId] and connectionData:isActive() and (sessionTab:hasPermission(conPlayer) and eventData:hasPermission(conPlayer)) then
								if subNetwork then
									local networkKey = subNetwork:getPlayerKey(conPlayer)
									if networkKey:isReadyToUse() then
										subNetwork:fire(conPlayer, "FirePlayerEvent", connectionData.fireEventId, os.time(), ...)
									else
										warn("Unable to fire event "..(eventData.name or eventData.id).." without a player key assigned in sub network "..tostring(subNetwork.id))
									end
								else
					...
					function eventData:fire(...)
						eventData._event:fire(...)
					end
					...
					eventData._event:connect(function(...)
						...
						if sessionTab:isActive() and eventData:isActive() then
							for i,connectionData in pairs(eventData.playerConnections) do
								local conPlayer = connectionData.player
								if connectionData:isActive() and (sessionTab:hasPermission(conPlayer) and eventData:hasPermission(conPlayer)) then
									if subNetwork then
										local networkKey = subNetwork:getPlayerKey(conPlayer)
										if networkKey:isReadyToUse() then
											subNetwork:fire(conPlayer, "FirePlayerEvent", connectionData.fireEventId, os.time(), ...)
			...
			function sessionTab:makeCommand(cmdName, cmdFunction, canInvoke, canFire)
				local cmdTab; cmdTab = {
					...
					allowedTriggers = {};
					connectedPlayers = {};
					execute = cmdFunction or function(plr, ...)
						return "error-"..math.random(300)
					end;
				}
			...
			function sessionTab:isActive()
				return self.active and (not self.expireOs or os.time()-self.expireOs < 0)
			end

			function sessionTab:hasPermission(player)
				return self.active and (self.selfConnectedPlayers[player] or self.connectedPlayers[player] or Identity.checkTable(player, self.allowedTriggers))
			end
		...
		getSubNetwork = function(networkName)
			return SubNetworks[networkName] or (function()
				for netName, subNetwork in pairs(SubNetworks) do
					if subNetwork.id:lower() == networkName:lower() or subNetwork.name:lower() == networkName:lower() then
						return subNetwork
					end
				end
			end)()
		...
		newSubNetwork = function(networkName)
			if SubNetworks[networkName] then
				...
				subNetwork.processFunc = function(...)
				...
				subNetwork.networkCommands = {
					...
					Verify = { -- Verifies personal key
						...
						Function = function(plr, args, remoteData)
							...
							if not personalKey.verifyStatus and type(givenAccessKey)=="string" and #givenAccessKey==60 then
								...
								local newPersonalKey = if isETEE then getRandomV3(subNetwork.playerKeySettings.keyLength) else nil
								
								if isETEE then
									personalKey.id = newPersonalKey
								end
				...
				subNetwork.networkRemoteCall = function(plr, invoke, remoteData, ...)
					...
					if remoteCmd and not remoteCmd.Disabled then
						...
						local userHasPermissions = (permissions and server.Roles:hasPermissionFromMember(plr, permissions)) or false
						...
						if (not lockdown or (lockdown and remoteCmd.Lockdown_Allowed)) and canAccess then
							...
							if canUseCommand and cmdFunction then
								...
								local rets = {service.trackTask("_SUBNETWORK_"..tostring(subNetwork.id).."_COMMAND-"..cmdName.."-Invoke:"..tostring(invoke).."-"..plr.UserId, false, cmdFunction, parsed, {unpack(remoteArguments, 2)}, remoteData)}
								local success, errMessage, errTrace = unpack(rets)
								
								table.insert(subNetwork.commandLogs, {
									title = "Player "..plr.UserId.." "..(invoke and "invoked" or "fired").." "..tostring(cmdName);
									desc = (success and "No errors found during process") or "Error found: "..tostring(errMessage);
								...
								if not rets[1] then
									warn("Player "..plr.Name.." encountered an error while running remote "..cmdName.." from network "..tostring(subNetwork.id)..": "..tostring(rets[2]))
				...
				subNetwork._network1 = Network.newCreate("SubNetwork-"..subNetwork.id, {
					...
					networkFunc = function(plr, ...)
						...
						if type(processFunc) ~= "function" then
							return -101,"Invalid_Network_Function"
						end
						...
						if subNetwork.active and cliData and personalKey and personalKey:isActive() and personalKey.trustChecked then
							local expectedPersonalKeyId = if isETEE then hashLib.sha1(personalKey.id) else personalKey.id
							...
							if not didPassRL then
								...
								if expectedPersonalKeyId ~= remoteArguments[1] then
									return
								end
								...
								if subNetwork.networkCommands[tostring(remoteArguments[2])] then
									...
									return subNetwork.networkRemoteCall(plr, invoke, remoteData, unpack(remoteArguments, 2))
								end

								local taskName = "SubNetwork_"..subNetwork.id.."_Process_"..plr.UserId.."-Invoke:"..tostring(invoke and true or false)
								local processRets = {service.trackTask(taskName, false, subNetwork.processFunc, parsedPlr, {unpack(remoteArguments, 2)})}
								...
								if not success then
									...
									if subNetwork.processWarnError then
										warn("Sub Network "..tostring(subNetwork.id).." encountered an error with player "..tostring(plr.UserId)..": "..tostring(errMessage), errTrace)
									end
								else
									if invoke then
										return unpack(processRets, 2)
									end
								end
							end
						end
				...
				function subNetwork:getPlayerKey(player)
				...
				function subNetwork:get(player, ...)
					...
					if personalKey then
						...
						if activeKey and not personalKey:isReadyToUse() then
							personalKey.verified:wait(nil, 300)
						end
						...
						if activeKey and personalKey:isReadyToUse() then
							...
							return unpack(self._network2:runToPlayers({player._object},
								if endToEndEncryption then hashLib.sha1(personalKey.clientAccessKey) else personalKey.clientAccessKey,
