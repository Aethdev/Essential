...
return function(envArgs)
	...
	local memoryStoreKey, memoryStorePlayerData, memoryStoreProtectIndex,
		memoryStoreQueuePlayerDataSaves
	...
	local function Init()
		...
		datastoreDefaultScope = settings.Datastore_Scope or "_"
		...
		datastoreKey = (datastoreProtectIndex and settings.Datastore_Key:sub(1,100)) or settings.Datastore_Key:sub(1,50)
		datastoreDefaultScope = (datastoreProtectIndex and datastoreDefaultScope:sub(1,100)) or datastoreDefaultScope:sub(1,50)
		...
		if settings.Datastore_Allow then
			if datastoreEncryptKeys then
				warn("ENCRYPTED DATASTORE KEYS ENABLED")
				server.Logs.addLog("Script", "Developer settings has enabled datastore encryption which protects the original scopes and keys exposed.")
			else
				server.Logs.addLog("Script", "Developer settings doesn't have datastore encryption enabled. Scripts discovering all datastore keys could potentially have access to this system's Roblox datastore.")
			end
			...
			if datastoreUseCompression then
				...
				server.Logs.addLog("Script", "Developer settings has setting 'Datastore_Compression' disabled which disallows saving very large data. It is RECOMMENDED to have this enabled always.")
				warn("DATASTORE COMPRESSION IS NOT ENABLED. LARGER CHUNKS OF DATA MAY NOT SAVE.")
			end
			
			if datastoreProtectIndex then
				datastoreKey = hashLib.md5(datastoreKey)
				datastoreDefaultScope = hashLib.md5(datastoreDefaultScope)
			end
			
			service.trackTask("Loading datastore", false, function()
				local datastoreOpts = service.New("DataStoreOptions")
				datastoreOpts:SetExperimentalFeatures({["v2"] = true;})
				
				globalDatastore = service.DataStoreService:GetDataStore(datastoreKey, datastoreDefaultScope, datastoreOpts)
				
				if globalDatastore then
					canWrite = pcall(function()
						globalDatastore:SetAsync("__WRITE", true)
					end)
					
					canRead = pcall(function()
						globalDatastore:GetAsync("__READ")
					end)
	...
	local function determineDataSize(dataValue)
		local maxDataSize = 4000000
		
		return (if type(dataValue) == "string" then #dataValue else service.HttpService:JSONEncode((dataValue==nil and "") or dataValue):len())/4000000 * 100
	end
	...
	--[[
		...
		DATASTORE UPDATE OEPRATORS
			...
			Query:
				...
				$gt - If both values are greater than the specified value
	...
	local function createDatastoreProcess(datastore_scope, datastore_key)
		...
		if datastoreProcess[queueName] then
			...
			local dataDatastore = (not datastore_scope and globalDatastore) or datastoreGetDS(datastore_scope)
			...
			dataWriteQueue.processFunc = function(ind, que)
				if ind == #dataWriteQueue._queue then
					if server.Running then
						if processData.valueUpdated and tick()-processData.valueUpdated < dataWriteQueue.processCooldown then
						...
						for i, que in pairs(dataWriteQueue._queue) do							
							...
							if processType == "readAndWrite" then
								...
								for i, readAndWriteCallback in ipairs(listOfUpdateCallbacks) do
									...
									task.spawn(function()
										local success, newValue, newValueKeyInfo = service.nonThreadTask(readAndWriteCallback, oldVal, dataKeyInfo)
										if reachedEnd == false then
											reachedEnd = true
											if type(newValue) == "userdata" then warn("A DataRead&Write callback returned a user data value. Results were not replaced.") return end
											if not dataKeyInfo and type(newValueKeyInfo) ~= "nil" then warn("A DataRead&Write callback returned a custom datastore key while the original datastore key was a nil value. Results were not replaced.") return end
											if dataKeyInfo and not (type(newValueKeyInfo)=="nil" or typeof(newValueKeyInfo) == "Instance" and newValueKeyInfo:IsA("DataStoreKeyInfo")) then
												warn("A DataRead&Write callback returned an invalid datastore key info. Results were not replaced.") return 
											end
							...
							if processType == "tableAdd" then
								...
								if type(oldVal) == "table" then
									if strictForm then
										...
										repeat
											if not checkDuplicateId() then
												entryId = service.getRandom(24)
											end
										until
											checkDuplicateId()
							...
							if processType == "tableRemove" then
								...
								if type(oldVal) == "table" then
									...
									if remType == "valueFromIndex" and type(oldVal) == "table" and type(remVal) ~= "nil" then
							...
							if processType == "tableUpdate" then
								...
								if type(oldVal) == "table" then
									...
									if updateType == "IndexFromTable" then
										local updTabIndFromData, updTabInd, updDataVal = queArguments[3], queArguments[4], queArguments[5]
										
										if not table.find({"string", "number"}, type(updTabIndFromData)) then
											continue
										end

										if not table.find({"string", "number"}, type(updTabInd)) then
											continue
										end

										if not table.find({"nil", "number", "boolean", "string", "table"}, type(updDataVal)) then
											continue
										end
									...
									if updateType == "tableAdd" then
										local tabIndex, tabValue, addOverride = queArguments[3], queArguments[4], queArguments[5]
										
										if not table.find({"string", "number"}, type(tabIndex)) then
											continue
										end

										if not table.find({"number", "boolean", "string", "table"}, type(tabValue)) then
											continue
										end
					...
					for i = 1, datastoreRetryAttempts do
						local suc, newDataValue, dataKeyInfo = pcall(dataDatastore.UpdateAsync, dataDatastore, datastore_key, updateCallback)
				...
				if ind == #dataOverWriteQueue._queue then					
					if determineDataSize(value) > 1 then
						...
						for i, otherQue in pairs(dataOverWriteQueue._queue) do
							if i <= ind then
								otherQue.processed:fire(true, false)
							end
						end
					else
						if datastoreUseCompression then
						...
						for i = 1, datastoreRetryAttempts do
							...
							if success or (not success and i == datastoreRetryAttempts) then
								for i, otherQue in pairs(dataOverWriteQueue._queue) do
									if i <= ind then
										otherQue.processed:fire(true, (success and true))
									end
								end
			...
			dataReadQueue.processFunc = function(ind, que, noCache)
				local canUpdate = (noCache and server.Running) or not processData.valueUpdated or tick()-processData.valueUpdated >= 7
				...
				if not canUpdate then
					...
					if server.Running then
						if processData.valueReadUpdated and tick()-processData.valueReadUpdated < dataReadQueue.processCooldown then
							task.wait(math.max(dataReadQueue.processCooldown-(tick()-processData.valueReadUpdated), 0))
						end
						wait(Datastore.getRequestDelay("read"))
					end
					
					local success, dataOrError, keyInfo = pcall(dataDatastore.GetAsync, dataDatastore, datastore_key)
						...
						if datastoreUseCompression then
							if type(dataOrError) == "string" then
								...
								if studioServer and dataOrError ~= nil then
									warn("COMPRESSION IS ENABLED. GETASYNC DATA WASN'T A STRING, GOT "..type(dataOrError)..". OOPS! ["..queueName.."]")
								end
					...
					if not noCache or not processData.valueReadUpdated or (tick()-processData.valueReadUpdated > 7) then
			...
			dataReadAndWriteQueue.processFunc = function(ind, que)
				if ind == #dataReadAndWriteQueue._queue then
					...
					local processConnection; processConnection = writeQue.processed:connect(function(_, didProcess)
						if didProcess then
			...
			server.Closing:connectOnce(function()
				...
				if #dataReadQueue._queue > 0 then
					task.spawn(dataReadQueue.process, dataReadQueue, true)
				end
				
				if #dataWriteQueue._queue > 0 then
					task.spawn(dataWriteQueue.process, dataWriteQueue, true)
				end
			end)
	...
	server.Datastore = {
			...
			if didLoad and scope then				
				...
				if datastoreProtectIndex then
					if not datastoreEncryptScopes[scope] then
						local encrypted = hashLib.md5(scope)
				...
				return service.DataStoreService:GetDataStore(datastoreKey, scope, datastoreOpts)
			end
		end;
		...
		getRequestDelay = function(reqType)
			...
			repeat
				reqBudget = service.DataStoreService:GetRequestBudgetForRequestType(reqType);
			until
		...
		read = function(scope, key, noCache)
			scope = (type(scope)=="number" and tostring(scope)) or (type(scope)=="string" and scope) or
				(type(scope)=="userdata" and error("Scope is a userdata value", 0)) or datastoreDefaultScope

			key = (type(key)=="number" and tostring(key)) or (type(key)=="string" and key) or
				error("Key is not a number/string value", 0)
			
			if scope then
				scope = scope:sub(1,50)
			end

			key = key:sub(1,50)
			key = (datastoreEncryptKeys and hashLib.md5(key)) or key
		...
		overWrite = function(scope, key, value, readyCallback)
			...
			if didLoad and canWrite and (key and validVal) then
				...
				local readySignal = Signal.new()
				local processCon; processCon = writeQue.processed:connect(function(didRun, stat)
				...
				if readyCallback then
					readySignal:connectOnce(readyCallback)
				end
			end
		end;
		...
		write = function(scope, key, value, readyCallback)
			...
			if didLoad and canWrite and (key and validVal) then
				...
				local readySignal = Signal.new()
				local processCon; processCon = writeQue.processed:connect(function(didRun, didWrite)
		...
		readAndWrite = function(scope, key, updateCallback, successCallback)
			...
			updateCallback = (type(updateCallback)=="function" and updateCallback) or 
				error("Update callback wasn't a function", 0)
			
			successCallback = if not successCallback then nil else (type(successCallback)=="function" and successCallback) or
				error("Update callback wasn't a function", 0)
		...
		encryptWrite = function(scope, key, value, encryptKey, readyCallback)			
			...
			if didLoad and canWrite and (key and validVal) then
				...
				local compressedValue = (datastoreUseCompression and encryptedValue2) or compression.Deflate.Compress(encryptedValue2, compressConfig)
		...
		encryptRead = function(scope, key, noCache, encryptKey)
			...
			encryptKey = (type(key)=="number" and tostring(encryptKey)) or (type(encryptKey)=="string" and encryptKey) or
				error("Encrypt key is not a number/string value", 0)
			...
			if didLoad and canRead then
				...
				if canUpdateCache then
					...
					if valType ~= "string" then
						...
						local decompressedValue = (datastoreUseCompression and decryptValue1) or compression.Deflate.Decompress(decryptValue1, compressConfig)
					...
					if valType ~= "string" then
						...
						local decryptValue1 = (datastoreUseCompression and tostring(dataProcess.currentValue)) or base64Decode(tostring(dataProcess.currentValue))
						local decompressedValue = (datastoreUseCompression and decryptValue1) or compression.Deflate.Decompress(decryptValue1, compressConfig)
						local decryptValue2 = tulirAES.decrypt(encryptKey, decompressedValue)
						local decryptValue3 = decryptValue2 and luaParser.Decode(decryptValue2)[1]
		...
		encryptUpdate = function(scope, key, encryptKey, updateCallback)			
			scope = (type(scope)=="number" and tostring(scope)) or (type(scope)=="string" and scope) or
				(type(scope)=="userdata" and error("Scope was a userdata value", 0)) or datastoreDefaultScope

			key = (type(key)=="number" and tostring(key)) or (type(key)=="string" and key) or
				error("Key was not a number/string value", 0)
				...
				do
					...
					local queEvent = queData.processed:connectOnce(function(_, value, valueKey)
						...
						if not table.find({"number", "boolean", "string", "table", "nil"}, type(newValue)) then
							warn("Datastore encryptUpdate updating key "..originalKey.." didn't return a valid value (expected number/boolean/string/table/nil)")
							return
						end
		...
		tableAdd = function(scope, key, value, strictForm, readyCallback, encryptData)
			...
			if didLoad and (canRead and canWrite) and (key and validVal) then
				...
				local readySignal = Signal.new()
				local processCon; processCon = writeQue.processed:connect(function(didRun, valueId)
		...
		tableRemove = function(scope, key, removeType, removeValue, removeArg1, readyCallback, encryptData)
			...
			key = (type(key)=="number" and tostring(key)) or (type(key)=="string" and key) or
				error("Key is not a number/string", 0)
			...
			if didLoad and (canRead and canWrite) then
				...
				local readySignal = Signal.new()
				local processCon; processCon = writeQue.processed:connect(function(didRun, didRemove)
		...
		tableFind = function(scope, key, value, useEncrypt)
			...
			if didLoad and canRead and (validVal and value) then
				local dataTable = if useEncrypt then Datastore.encryptRead(scope, key, false, datastoreEncryptMasterKey) else Datastore.read(scope, key)
				...
				for ind,entryTab in pairs(dataTable) do
					if type(entryTab) == "table" then
						...
						if entryVal ~= nil and service.checkEquality(entryVal, value) then
							return entryTab.id
						end
					end
				end
		...
		tableUpdate = function(scope, key, updateType, updateArg1, updateArg2, updateArg3, readyCallback)
			...
			if didLoad and (canRead and canWrite) then
				...
				local writeProcess = createDatastoreProcess(scope, key)
				local writeQue = writeProcess.dataWrite:add("tableUpdate", updateType, updateArg1, updateArg2, updateArg3)
				
				if not writeQue then return false end
				
				local readySignal = Signal.new()
				local processCon; processCon = writeQue.processed:connect(function(didRun, runStat)
		...
		addUserIdToData = function(scope, key, userId, readyCallback)
			...
			userId = (type(userId)=="number" and math.floor(userId)==userId and userId) or error("User id must be an integer")
		...
		canWrite = function() return canWrite end;
		canRead = function() return canRead end;
