	...
	local function Init()
		...
		if settings.Datastore_Allow then
			...
			service.trackTask("Loading datastore", false, function()
				...
				globalDatastore =
					service.DataStoreService:GetDataStore(datastoreKey, datastoreDefaultScope, datastoreOpts)

				if globalDatastore then
					canWrite = pcall(function() globalDatastore:SetAsync("__WRITE", true) end)

					canRead = pcall(function() globalDatastore:GetAsync "__READ" end)
	...
	local function determineDataSize(dataValue)
		...
		return (
			if type(dataValue) == "string"
				then #dataValue
				else service.HttpService:JSONEncode((dataValue == nil and "") or dataValue):len()
		)
			/ 4000000
			* 100
	end
