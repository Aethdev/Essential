		...
		for i,obj in pairs(server.Folder.Commands:GetChildren()) do
			local function scan(plug)
				...
				if not plugDisabled then
					loadModule(plug, {script = plug}, false, true)
				end
			end
	...
	server.Commands = {
			...
			for i,cmd in pairs(Cmds.Library) do
				if not cmd.Aliases or #cmd.Aliases == 0 then
					if server.Studio then
						warn("Missing aliases:", i, cmd)
					end
				else
					if not cmd.Id then
						cmd.Id = HashLib.sha1(`{cmd.Aliases[1] or "-."}-{tick()}`)
					end
		...
		get = function(str, index, noPrefix, cmdLib)
			...
			for cmd,tab in pairs((noPrefix and CmdWithoutPrefixCache) or CmdCache) do
				...
				if strMatch then
					if strMatch:lower() == cmd:lower() then
						return tab,cmd
					end
				end
			end
		...
		getName = function(cmdTable)
			return table.find(Cmds.Library, cmdTable) or "Command-"..service.getRandom()
		end;
		...
		CoreCommands = {
						...
						for statName,stat in pairs(Cmds.ifStatementChecks) do
							...
							for d,check in pairs(stat.check) do
								if stat.stringMatch then
									if statement:match(check) then
										return stat,{statement:match(check)}
									end
								else
									if statement:lower() == check:lower() then
										return stat
									end
								end
							end
			...
			startMuteOnAFK = {
				...
				Function = function(plr, args, data)
					if not plr:getPData().__clientSettings.ToggleMuteOnAFK then
						plr:sendData("SendMessage", "<b>Unable to start mute on AFK because you disabled Mute on AFK via client settings.</b>", nil, 2, "Context")
						return 0
					end
					
					if Utility:isMuted(plr.Name) and not Utility:isMutedByMOA(plr) then
						plr:sendData("SendMessage", "<b>Unable to start mute on AFK because of your current muted state.</b>", nil, 2, "Context")
						return 0
					end
			...
			endMuteOnAFK = {
				...
				Function = function(plr, args, data)
					if not plr:getPData().__clientSettings.ToggleMuteOnAFK then
						plr:sendData("SendMessage", "<b>Unable to end mute on AFK because you disabled Mute on AFK via client settings.</b>", nil, 2, "Context")
						return 0
					end
					
					if Utility:isMuted(plr.Name) and not Utility:isMutedByMOA(plr) then
						plr:sendData("SendMessage", "<b>Unable to end mute on AFK because of your current muted state.</b>", nil, 2, "Context")
						return 0
					end
			...
			toggleMuteOnAFK = {
				...
				Function = function(plr, args, data)
					if not plr:getPData().__clientSettings.ToggleMuteOnAFK then
						plr:sendData("SendMessage", "<b>Unable to toggle mute on AFK because you disabled Mute on AFK via client settings.</b>", nil, 2, "Context")
						return 0
					end
					
					if Utility:isMuted(plr.Name) and not Utility:isMutedByMOA(plr) then
						plr:sendData("SendMessage", "<b>Unable to toggle mute on AFK because of your current muted state.</b>", nil, 2, "Context")
						return 0
					end
		...
		Library = {
			showCommands = {
				...
				Function = function(plr, args)
					...
					if #availableCmds == 0 then
						plr:sendData("SendMessage", "<b>Commands list</b>: There are no commands to provide.", nil, 6, "Context")
					else
						plr:makeUI("List", {
						...
						if args[2] and args[2].total > 0 then
							coroutine.wrap(function()
								...
								for i = 1,totalSecs,1 do
									if not Utility:isMuted(target.Name) then
										return
									end
					...
					for i,target in pairs(args[1]) do
						...
						if args[2] and args[2].total > 0 then
							coroutine.wrap(function()
								...
								for i = 1,totalSecs,1 do
									if not Utility:isDeafened(target.Name) then
										return
									end
			...
			chat_undeafenPlayer = {
				...
				Function = function(plr, args)
					for i,target in pairs(args[1]) do
						if Utility:isDeafened(target.Name) then
							Utility:unmutePlayer(target.Name)
						end
					end
				end;
			...
			chatSystem_slowmode = {
				...
				Function = function(plr, args)
					if args[1] and args[1]:lower()=="disable" then
						server.Events.modChangedSlowmode:fire(plr:getInfo(), "Status", (settings.chatSlowmode_Enabled and true), false)
						settings.chatSlowmode_Enabled = false
						plr:sendData("SendMessage", "Slowmode success", "<b>Disabled slowmode</b>. Players can now chat without slowmode.", 5, "Hint")
					elseif args[1] and args[1]:lower()=="enable" then
						server.Events.modChangedSlowmode:fire(plr:getInfo(), "Status", (settings.chatSlowmode_Enabled and true), true)
						settings.chatSlowmode_Enabled = true
						plr:sendData("SendMessage", "Slowmode success", "<b>Enabled slowmode</b>. Players without permission 'Manage_Game' or 'Bypass_Chat_Slowmode' are affected by slowmode.", 5, "Hint")
					elseif args[1] and args[1]:lower()=="view" then
						plr:sendData("SendMessage", "Slowmode stats", "Seconds: "..tostring(settings.chatSlowmode_Interval).." | Status: "..tostring(settings.chatSlowmode_Enabled), 10, "Hint")
					elseif args[1] then
						local number = tonumber(string.match(args[1], "^[%d%p]+$"))
						
						if not number then
							plr:sendData("SendMessage", "Slowmode error", "Unknown option/seconds supplied for slowmode", 5, "Hint")
						else
							server.Events.modChangedSlowmode:fire(plr:getInfo(), "Interval", settings.chatSlowmode_Interval, number)
							settings.chatSlowmode_Interval = number
							plr:sendData("SendMessage", "Slowmode success", "Set seconds to "..tostring(number), 5, "Hint")
						end
					end
				end;
			};
				...
				Function = function(plr, args, data)
					...
					service.startLoop(loopInd, args[2], function()
						if curInt+1 <= args[1] and plr:isInGame() and not goBreak then
							if not Process.playerCommand(plr, args[3], setmetatable(service.cloneTable(data), {__index = {loop = true;}})) then
								goBreak = true
							else
								curInt += 1
							end
