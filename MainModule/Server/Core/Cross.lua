...
return function(envArgs)
	...
	local Subscription;
	local CrossKey;
	local CrossCommands; CrossCommands = {
			...
			if type(data) == "table" then
				...
				if data.input then
					...
					if data.command and data.arguments then
						...
						if realCommand then
							Core.executeCommand(player, realCommand, data.arguments or {})
						end
					end
				end
			end
		...
		CrossChat = function(jobId, speakerName, message, extraTags, channelName)
			...
			if generalChannel then
				...
				local speaker = server.chatService:GetSpeaker("_"..speakerName) or (function()
					tempSpeaker = true
				...
				if tempSpeaker then
					server.chatService:RemoveSpeaker("_"..speakerName)
				end
			end
		end;
		...
		PrivateMessage = function(jobId, topic, message, expireOs, scheduledOs, sender, targets, crossEventId)
			local players = {}
			local fakePlayer = targets and Parser:apifyPlayer({
			...
			for i,plr in pairs(players) do
				...
				if crossEventId then
					pmData.replied:connectOnce(function(reply)
		...
		FireEvent = function(jobId, eventId, ...)
			if eventId and variables.crossEvents[eventId] then
				variables.crossEvents[eventId]:fire(jobId, ...)
			end
		end;
	...
	local CrossSend_RL = setmetatable({
		...
		__index = function(self, ind)
			if ind == "Rates" then
				return (#service.getPlayers()*80) + 300
			end
		end,
	})
			...
			service.trackTask("_CROSSCOM_LISTENER", true, function()
				...
				Subscription = service.MessagingService:SubscribeAsync(CrossKey, function(data)
					...
					if type(actualData) == "string" then
						...
						remoteSettings = (type(remoteSettings)=="table" and remoteSettings) or {
						...
						if #remoteSettings.WhitelistedServers > 0 and not table.find(remoteSettings.WhitelistedServers, (#game.PrivateServerId>0 and game.PrivateServerId) or game.JobId) then
						...
						if crossCmd then
							...
							local suc,ers = service.trackTask("_CROSSCMD_"..tostring(cmdName), false, crossCmd, sender, unpack(dataArguments, 2))
							Logs.addLog("Script", "Ran cross command "..tostring(dataArguments[1])..".")
							
							if not suc then
								Logs.addLog("Script", "Failed to run cross command "..tostring(cmdName)..": "..tostring(ers))
								warn("Cross Command "..tostring(cmdName).." encountered an error: "..tostring(ers))
							end
						end
					end
				end)
	...
	server.Cross = {
		...
		send = function(...)
			if CrossReady and CrossKey then
				...
				if ratePass then
					service.MessagingService:PublishAsync(CrossKey, base64Encode( compression.Deflate.Compress( luaParser.Encode({ {Sender = game.JobId, Arguments = {...}} }) , compressionConfig ) ))
		...
		sendToOtherServers = function(...)
			if CrossReady and CrossKey then
				...
				if ratePass then
					...
					service.MessagingService:PublishAsync(CrossKey, base64Encode( compression.Deflate.Compress( luaParser.Encode({ {Sender = game.JobId, Settings = {IgnoreCurrent = true}, Arguments = {...}} }) , compressionConfig ) ))
				end
			end
		end;
		...
		sendToSpecificServers = function(serverIds, ...)
			...
			if CrossReady and CrossKey then
				...
				if ratePass then
					...
					service.MessagingService:PublishAsync(CrossKey, base64Encode( compression.Deflate.Compress( luaParser.Encode({ {Sender = game.JobId, Settings = {WhitelistedServers = serverIds}, Arguments = {...}} }) , compressionConfig ) ))
				end
			end
		end;
