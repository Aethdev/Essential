...
return function(envArgs)
	...
	local datastoreKey, datastoreDefaultScope, datastoreEncryptKeys,
		datastoreProtectIndex, datastoreEncryptMasterKey, datastoreUseCompression,
		datastoreUseLegacyHash
	...
	local function createHashString(targetString: string, hashType: "key"|"scope"): string
		if datastoreUseLegacyHash then
			return hashLib.md5(targetString)
		end
			...
			service.trackTask("Loading datastore", true, function()
				...
				if globalDatastore then
					local suc,ers = pcall(function()
						globalDatastore:UpdateAsync("__WRITE", function(oldVal)
					...
					if suc then
						...
						server.Logs.addLog("Script", "Developer settings enabled datastore but datastore fails to use UpdateAsync. Retreating to MockDataStoreService..")
	...
	local function determineDataSize(dataValue, customDataSize: number?)
		...
		return ((if type(dataValue) == "string" then #dataValue
			elseif type(dataValue) == "table" then #luaParser.Encode(dataValue)
			else #tostring(dataValue))*bytes_to_megabytes)/maxDataSize * 100
	end
	...
	local function createDatastoreProcess(datastore_scope: string, datastore_key: string, universe_id: boolean)
		...
		if datastoreProcess[queueName] then
			...
			local dataDatastore: GlobalDataStore = (not datastore_scope and globalDatastore) or datastoreGetDS(datastore_scope)
			...
			dataReadQueue.processFunc = function(ind, que, noCache)
				...
				if not canUpdate then
					...
					local success, dataOrError, keyInfo = pcall(dataDatastore.GetAsync, dataDatastore, datastore_key, dtGetOptions)
	...
	server.Datastore = {
		...
		getDatastore = function(scope: string?, useMockService: boolean?)
			if didLoad and scope then				
				...
				return (if useMockService then server.MockDataStoreService else DataStoreService):GetDataStore(datastoreKey, scope, datastoreOpts)
			end
		end;
		...
		canWrite = function() if not didLoad then readyEv:wait() end return canWrite end;
		canRead = function() if not didLoad then readyEv:wait() end return canRead end;
