...
local assertWarn = function(checkMatch: any, message: string): boolean
...
local rawset = function(tab: { [any]: any }, ind: any, val: any)
	...
	assert(
		table.find({ "string", "number", "userdata", "table", "nil" }, type(ind)),
...
local getEnv
getEnv = function(typ: string, exemptions: { [any]: any }): { [any]: any }
...
local loadModule = function(
	...
	if func then
		...
		if noEnv then
			...
			rets = {
				service.trackTask("_MODULE-" .. tostring(module), isThread, setfenv(func, modEnv), unpack(callArgs)),
			}
		end
...
local cleanUp: nil = function()
					...
					if pData then
						...
						do
							...
							activityLogs._pushToSet(server.Logs.formatLog {
								...
								desc = "Duration: "
									.. server.Parser:formatTime(os.time() - (pData.serverData.joined or os.time())),
							})
							activityLogs._recognize()
							--warn("cleanup comes first?")
						end

						coroutine.wrap(function() pData._forceUpdate() end)()
					end
				end
			end
		end
	...
	if server.Closing then server.Closing:fire(os.time()) end
	...
	if server.privateServer_Profile then server.privateServer_Profile:Release() end
...
return service.newProxy {
	__metatable = "Essential",
	__tostring = function() return "Essential" end,
	__call = function(self, data)
		...
		if existingG and type(existingG) == "string" then
			service.TestService:Message(
				"\n-----------\n"
					.. "- ESSENTIAL LOADER\n"
					.. " \n"
					.. "ESSENTIAL IS ALREADY RUNNING FROM ANOTHER SCRIPT ("
					.. existingG:upper()
					.. ")\n"
					.. "-----------\n"
			)
		...
		local serverInfo = {
			...
			type = (server.Studio and "[studio]")
				or (game.PrivateServerOwnerId > 0 and "[personal]")
				or (#game.PrivateServerId > 0 and "[private]")
				or "[public]",
		...
		serverInfo.detailedName = if server.Studio
			then `[studio-{game.PlaceId}]`
			else serverInfo.type .. " " .. (#serverInfo.privateId > 0 and serverInfo.privateId or serverInfo.id)
		...
		serviceSpecific.getPlayers = function(parsePlayers, includeGhosts)
			...
			if includeGhosts then
				for i, rep in pairs(server.Network:getReplicators()) do
					...
					if player and not table.find(results, player) then table.insert(results, player) end
				end
			end
		...
		serviceSpecific.getPlayersByList = function(parsePlayers, list)
			...
			for i, plr in pairs(service.Players:GetPlayers()) do
				if table.find(list, plr.UserId) or table.find(list, plr.Name) then table.insert(results, plr) end
			end
		...
		if not initDisablePlugins then initPackage:RunPlugins() end
