		...
		function contextContainer:sort()
			...
			table.sort(
				...
				function(elementA, elementB)
					return elementA._priority > elementB._priority
						or (elementA._priority == elementB._priority and elementA._joinedGroup < elementB._joinedGroup)
				end
			)
					...
					if element.active and element.showState then
						Promise.promisify(element.updateDisplayContent)(element)
							...
							:catch(function(err)
								...
								warn(
									`Context container encountered an error with Context during update display content: {tostring(
										err
									)}`
								)
							end)
