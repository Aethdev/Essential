...
return function(envArgs, data)
	...
	if client.clientSettingsWindow then
		...
		if openSpecificCategory then
			clientSettingsW:openCategory(openSpecificCategory)
		end
	...
	window.guiData.destroyed:connectOnce(function()
		if client.clientSettingsWindow == window then
			client.clientSettingsWindow = nil
		end
	end)
	client.clientSettingsWindow = window
	
	local clientSettings = client.Settings
	local function manageClientSetting(setting: string, value: any)
		clientSettings[setting] = value
	end
	...
	do
		...
		do
			...
			if isKeybindsLocked then
				...
				keybindsEnabledToggle.toggled:connect(function(state)
					manageClientSetting("KeybindsEnabled", state)
				end)
		...
		do
			...
			if isIcognitoModeLocked then
				...
				incognitoModeToggle:setToggle( client.Policies._clientPolicies.OVERRIDE_INCOGNITO_MODE.value and true or false )
	...
	do
		...
		local function createAliasInteraction(aliasName: string, cmdLine: string)
			...
			aliasActionButton.clicked:connect(function()
				if not didCreatePrompt then
					...
					local tempInputChanged; tempInputChanged = aliasCmdLineTF.inputChanged:connect(function()
					...
					deleteAliasAction.clicked:connect(function()
						if not managingAlias then
							...
							if deleteRetCode == -1 then
								constructUI("Context", {
									text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> failed to delete due to non-existence.";
									plainText = "Alias "..aliasName.." failed to delete due to non-existence.";
									expireOs = osTime()+8;
								})
							elseif deleteRetCode == true then
								constructUI("Context", {
									text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> successfully deleted.";
					...
					createPromptData.confirmed:connect(function()
						if managingAlias then
							...
							local updateRetCode = Network:get("UpdateCommandAlias", aliasName, aliasCmdLineTF.currentInput)
							if updateRetCode == -1 then
								constructUI("Context", {
									text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> failed to update due to non-existence.";
							...
							elseif updateRetCode == true then
								constructUI("Context", {
									text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> successfully updated.";
		...
		createAliasButton.clicked:connect(function()
			if not didCreatePrompt and not managingAlias then
				...
				executeTF:changeHint("Command line for the alias ("..aliasAndShortcutCmdLineCharLimit.." char limit)")
				...
				createPromptData.confirmed:connect(function()
					...
					elseif #aliasCommandLine == 0 then
						...
						elseif createRetCode == -2 then
							constructUI("Context", {
								text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> already exists. Creation canceled.";
								plainText = "Alias "..aliasName.." already exists. Creation canceled.";
								expireOs = osTime()+6;
							})
						elseif createRetCode == -2.5 then
							constructUI("Context", {
								text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> is not a safe string. Pick different phrase(s) for your alias.";
								plainText = "Alias "..aliasName.." is not a safe string. Pick different phrase(s) for your alias.";
								expireOs = osTime()+6;
							})
						elseif createRetCode == -3 then
							constructUI("Context", {
								text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> contains a batch separator. Creation canceled.";
								plainText = "Alias "..aliasName.." contains a batch separator. Creation canceled.";
								expireOs = osTime()+6;
							})
						elseif createRetCode == -4 then
							constructUI("Context", {
								text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> is similar to another CC alias.";
								plainText = "Alias "..aliasName.." is similar to another CC alias.";
								expireOs = osTime()+6;
							})
						else
							constructUI("Context", {
								text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> has successfully been created.";
		...
		deleteAliasButton.clicked:connect(function()
			if not didCreatePrompt and not managingAlias then
				...
				deleteAliasButton.clicked:connect(function()
					...
					if not managingAlias and #aliasName > 0 then
						...
						elseif deleteRetCode == true then
							...
							if aliasButton then
								deleteAliasInteraction(aliasButton)
							end
						end
				...
				deletePromptData.confirmed:connectOnce(function()
				...
				if not didCreatePrompt then
					...
					createPromptData.confirmed:connect(function()
						if managingAlias then
							...
							local updateRetCode = Network:get("UpdateCustomCmdAlias", aliasName, aliasCmdLineTF.currentInput)
							if updateRetCode == -1 then
								constructUI("Context", {
									text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> failed to update due to non-existence.";
									plainText = "Alias "..aliasName.." failed to update due to non-existence.";
							...
							elseif updateRetCode == -2 then
								constructUI("Context", {
									text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> failed to update because the command you specified doesn't exist.";
									plainText = "Alias "..aliasName.." failed to update because the command you specified doesn't exist.";
									expireOs = osTime()+8;
								})
								deleteAliasInteraction(aliasActionButton)
							elseif updateRetCode == true then
				...
				createPromptData.confirmed:connect(function()
					...
					elseif #aliasCommandLine == 0 then
						...
						elseif createRetCode == -5 then
							constructUI("Context", {
								text = "Alias <b>"..Parser:filterForRichText(aliasName).."</b> didn't refer to an existant command.";
	...
	do
		...
		local function createKeybindInteraction(keybindName: string, interactOptions: {
			commandLine: string?,
			duration: number?,
			...
			keybindActionButton.label.TextColor3 = if not interactOptions.enabled then Color3.fromRGB(113, 113, 113) else
				defaultKeybindActionLabelColor
			...
			keybindActionButton.clicked:connect(function()
				if not didCreatePrompt then
					...
					keybindNameTF:changeHint(interactOptions.description or `This is a {if interactOptions.commandKeybindId then `command` else `custom`} keybind`)
					
					local keybindsRec = createPromptData:addButton("Keybinds", nil, `KeybindListener`)
					keybindsRec:changeHint(if isKeybindLocked then `* <i>read only</i>`
						elseif not (interactOptions.commandKeybindId or interactOptions.saveId) then `* <i>not persistent to save after leaving the server</i>`
					...
					if not isKeybindLocked then
						keybindsRec.hotkeysChanged:connectOnce(function()
							createPromptData:changeCanConfirmState(true)
						end)
					end
					...
					keybindsDuration:changeHint(if not interactOptions.commandKeybindId or isKeybindLocked then `* <i>read only</i>` else "* Determines the hold duration of the keybind in seconds to trigger (Maximum: 60 seconds) (Default: 0 seconds)")
					
					if not isKeybindLocked then
						local durationChanged; durationChanged = keybindsDuration.inputChanged:connect(function()
					...
					local executeTF;
					local enabledBF;
					local changeKeybindType: "Update"|"Delete" = if interactOptions.commandKeybindId then `Delete` else `Update`
					...
					if interactOptions.commandKeybindId and interactOptions.commandLine then
						...
						local tempInputChanged; tempInputChanged = executeTF.inputChanged:connect(function()
						...
						enabledBF.clicked:connectOnce(function()
							createPromptData:changeCanConfirmState(true)
						end)
					...
					if not isKeybindLocked then
						local deleteKeybindAction = createPromptData:addButton(if changeKeybindType == "Update" then `Restore default keybinds` else `Delete Keybind`, "Confirm")
						deleteKeybindAction:highlightInput(if changeKeybindType == "Update" then Color3.fromRGB(255, 137, 33) else
						...
						deleteKeybindAction.clicked:connect(function()
							...
							if changeKeybindType == "Update" then
								...
								if keybindData and not service.tableMatch(keybindData.keys, keybindData.defaultKeys) then
								...
								constructUI("Context", {
									text = `Restored default keybinds for keybind {Parser:filterForRichText(keybindName)}`;
							...
							if deleteRetCode == -1 then
								constructUI("Context", {
									text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> failed to delete due to non-existence.";
							...
							elseif deleteRetCode == true then
								constructUI("Context", {
									text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> successfully deleted.";
					...
					createPromptData.confirmed:connect(function()
						...
						if changeKeybindType == "Update" then
							if interactOptions.saveId then
								local isHotkeysSameAsDefault = service.tableMatch(newHotkeys, keybindsRec.defaultHotkeys)
								Network:get("UpdateCustomKeybind", interactOptions.saveId,
									if isHotkeysSameAsDefault then nil else newHotkeys
								)
								
								variables.savedCustomKeybinds[interactOptions.saveId] = if isHotkeysSameAsDefault then nil else newHotkeys
							end
						...
						local updateRetCode = Network:get("UpdateCmdKeybind", interactOptions.commandKeybindId, newHotkeys, newDuration, newExecuteInput, if enabledBF then enabledBF.switchState else true)
						if updateRetCode == -1 then
							constructUI("Context", {
								text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> failed to update due to non-existence.";
						...
						elseif updateRetCode == -2 then
							constructUI("Context", {
								text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> failed to update because the keybind name you specified doesn't exist.";
								plainText = "Keybind "..keybindName.." failed to update because the keybind name you specified doesn't exist.";
								expireOs = osTime()+8;
							})
							deleteKeybindInteraction(keybindActionButton)
						elseif updateRetCode == true then
							constructUI("Context", {
								text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> successfully updated.";
								plainText = "Keybind "..keybindName.." successfully updated.";
								expireOs = osTime()+8;
							})
							interactOptions.enabled = if enabledBF then enabledBF.switchState else interactOptions.enabled
							...
							if cliKeybindData then
								...
								keybindActionButton.label.TextColor3 = if not toggleKeybindState then Color3.fromRGB(113, 113, 113) else
									defaultKeybindActionLabelColor
		...
		createKeybindButton.clicked:connect(function()
			if not didCreatePrompt and not managingKeybind then
				...
				createPromptData.confirmed:connect(function()
					...
					task.wait(1)
					local createRetCode = Network:get("CreateCmdKeybind", keybindName, keybindsRec.savedHotkeys, tonumber(keybindsDuration.currentInput) or 0, keybindCommandLine, enabledBF.switchState)
					if createRetCode == -1 then
						constructUI("Context", {
					...
					elseif createRetCode == -2 then
						constructUI("Context", {
							text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> already exists. Creation canceled.";
							plainText = "Keybind "..keybindName.." already exists. Creation canceled.";
							expireOs = osTime()+6;
						})
					elseif createRetCode == -2.5 then
						constructUI("Context", {
							text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> is not a safe string. Pick different phrase(s) for your keybind.";
							plainText = "Keybind "..keybindName.." is not a safe string. Pick different phrase(s) for your keybind.";
							expireOs = osTime()+6;
						})
					elseif createRetCode == -3 then
						constructUI("Context", {
							text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> doesn't contain valid hotkeys.";
							plainText = "Keybind "..keybindName.." doesn't contain valid hotkeys.";
							expireOs = osTime()+6;
						})
					elseif createRetCode == -4 then
						constructUI("Context", {
							text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> doesn't have hotkeys.";
							plainText = "Keybind "..keybindName.." doesn't have	 hotkeys.";
							expireOs = osTime()+6;
						})
					else
						constructUI("Context", {
							text = "Keybind <b>"..Parser:filterForRichText(keybindName).."</b> has successfully been created.";
					...
					if not managingKeybind and #keybindName > 0 then
						...
						elseif deleteRetCode == true then
							...
							if keybindButton then
								deleteKeybindInteraction(keybindButton)
							end
						end
						deletePromptData:changeCanCloseState(true)
		...
		-- Client keybinds
		for i, keybindData: {_name: string, _saveId: string?, holdDuration: number, locked: boolean?, hidden: boolean?, keys: {[number]: Enum.KeyCode}, _priority: number} in client.Utility.Keybinds.registeredKeybinds do
	...
	do
		...
		local function createButtonInteraction(buttonName: string, cmdLine: string)
			...
			buttonActionButton.clicked:connect(function()
				if not didCreatePrompt then
					...
					local tempInputChanged; tempInputChanged = buttonCmdLineTF.inputChanged:connect(function()
					...
					deleteButtonAction.clicked:connect(function()
						if not managingButton then
							...
							if deleteRetCode == -1 then
								constructUI("Context", {
									text = "Button <b>"..Parser:filterForRichText(buttonName).."</b> failed to delete due to non-existence.";
									plainText = "Button "..buttonName.." failed to delete due to non-existence.";
									expireOs = osTime()+8;
								})
							elseif deleteRetCode == true then
								constructUI("Context", {
									text = "Button <b>"..Parser:filterForRichText(buttonName).."</b> successfully deleted.";
					...
					createPromptData.confirmed:connect(function()
						if managingButton then
							...
							local updateRetCode = Network:get("UpdateCommandButton", buttonName, buttonCmdLineTF.currentInput)
							if updateRetCode == -1 then
								constructUI("Context", {
									text = "Button <b>"..Parser:filterForRichText(buttonName).."</b> failed to update due to non-existence.";
							...
							elseif updateRetCode == true then
								constructUI("Context", {
									text = "Button <b>"..Parser:filterForRichText(buttonName).."</b> successfully updated.";
		...
		createActionButton.clicked:connect(function()
			if not didCreatePrompt and not managingButton then
				...
				executeTF.inputBox.PlaceholderText = ":bring all"
				executeTF.inputPattern = "^%s*(.-)$"
				executeTF:changeHint("Command line for the button ("..aliasAndShortcutCmdLineCharLimit.." char limit)")
				executeTF.charLimit = aliasAndShortcutCmdLineCharLimit
				...
				createPromptData.confirmed:connect(function()
					...
					elseif #buttonCommandLine == 0 then
						...
						elseif createRetCode == -2 then
							constructUI("Context", {
								text = "Button <b>"..Parser:filterForRichText(buttonName).."</b> already exists. Creation canceled.";
								plainText = "Button "..buttonName.." already exists. Creation canceled.";
								expireOs = osTime()+6;
							})
						elseif createRetCode == -2.5 then
							constructUI("Context", {
								text = "Button <b>"..Parser:filterForRichText(buttonName).."</b> is not a safe string. Pick different phrase(s) for your button.";
								plainText = "Button "..buttonName.." is not a safe string. Pick different phrase(s) for your button.";
								expireOs = osTime()+6;
							})
						else
							constructUI("Context", {
								text = "Button <b>"..Parser:filterForRichText(buttonName).."</b> has successfully been created.";
								plainText = "Button "..buttonName.." has successfully been created.";
								expireOs = osTime()+8;
							})
							if windowGuiData.active then
								createButtonInteraction(buttonName, buttonCommandLine)
							end
						end
					end
					managingButton = false
					...
					if not managingButton and #buttonName > 0 then
						...
						if deleteRetCode == -1 then
							...
							if buttonButton then
								deleteButtonInteraction(buttonButton)
							end
						elseif deleteRetCode == true then
							constructUI("Context", {
								text = "Button <b>"..Parser:filterForRichText(buttonName).."</b> successfully deleted.";
	...
	if client.Network:get("HasAdmin") then
		...
		clockTimeProg.progressChanged:connect(function(newValue)
			clockHover.text = tostring(roundNumber(newValue, 0.001))
		end)
		
		clockTimeProg.progressConfirmed:connect(function(confirmValue)
			client.Network:fire("ManageLighting", "ClockTime", confirmValue)
		end)
		
		window.guiData.bindEvent(service.Lighting:GetPropertyChangedSignal"ClockTime", function()
			clockTimeProg:changeProgressByNumber(service.Lighting.ClockTime)
		end)
		...
		musicVolProg.progressChanged:connect(function(newValue)
			musicVolHover.text = tostring(newValue)
		end)
		...
		shutdownServerButton.clicked:connect(function()
			if not onUseShutdownPrompt then
				...
				confirmButtonButton.clicked:connectOnce(function()
					Network:fire("ManageServer", "Shutdown", (#reasonTF.currentInput>0 and reasonTF.currentInput) or nil)
					shutdownPrompt:destroy()
				end)
				
				shutdownPrompt:show()
				shutdownPrompt.hidden:connectOnce(function()
					onUseShutdownPrompt = false
				end)
			end
		end)
	end
	...
	if openSpecificCategory then
		window:openCategory(openSpecificCategory)
	end
