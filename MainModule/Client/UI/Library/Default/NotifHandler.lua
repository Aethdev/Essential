return function(envArgs, data)
	...
	if not notifContainer then
		...
		function notifContainer:checkFrame()
			...
			local expectedCSize = UDim2.new(
				0,
				0,
				0,
				math.clamp(math.ceil(#self:getActiveElements() / 3), 1, 6) * self.frame.Size.Y.Offset
			)

			if self.frame:IsDescendantOf(game) then
				service.TweenService
					:Create(self.frame, TweenInfo.new(self.tweenTime, self.easingStyle, Enum.EasingDirection.Out), {
		...
		function notifContainer:getActiveElements()
			...
			for i, notif in pairs(self._notifs) do
				if notif.visible then table.insert(results, notif) end
			end
				...
				local subscribedEvents = {
					hidden = notif.hidden:connect(function()
						...
						if not notif.visible then
							...
							if activeNotifs_count == 0 then self:hide() end
						end
					end),

					shown = notif.shown:connect(function() self:sort() end),
				}
		...
		function notifContainer:remove(notif)
			...
			if notifIndex then
				...
				if notif.visible then self:sort() end
