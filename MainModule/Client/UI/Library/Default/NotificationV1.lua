...
return function(envArgs, data)
	...
	if not notifContainer then
		notifContainer = client.UI.construct("NotifHandler")
	end
	...
	if runType == "session" then
		...
		if sessionId and (openNotifId or closeNotifId) then
			if openNotifId then
				notif.opened:connectOnce(function(selectedOpts)
					client.Network:fire("ManageSession", sessionId, "RunCommand", openNotifId)
				end)
			end
			
			if closeNotifId then
				notif.closed:connectOnce(function(selectedOpts)
					client.Network:fire("ManageSession", sessionId, "RunCommand", closeNotifId)
				end)
			end
		end
	end
	...
	if type(closeFunc) == "string" then
		closeFunc = client.Loadstring(closeFunc, getEnv(nil, {notif = notif; ui = client.UI; player = service.player;}))
	...
	if openFunc then
		notif.opened:connect(openFunc)
	end
	
	if closeFunc then
		notif.closed:connect(closeFunc)
	end
	
	local openedState
	local closedState
	
	notif.opened:connectOnce(function()
		openedState = true
	end)
	
	notif.closed:connectOnce(function()
		closedState = true
	end)
	...
	if time and time > 0 then
		if not (data.noYield or data.noWait) then
			notif.hidden:wait()
		end
