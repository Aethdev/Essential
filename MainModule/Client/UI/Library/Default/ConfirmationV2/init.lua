return function(envArgs, data)
	...
	if data.returnChoice or data.returnOutput then
		...
		local expectedTimeoutDuration = math.clamp(
			if confirmation.timeDuration and confirmation.timeDuration > 0
				then confirmation.timeDuration + 10
				else timeoutDuration,
			10,
			timeoutDuration
		)
		local returnValues = {
			client.Signal:waitOnSingleEvents(
				{ confirmation.submitted, confirmation.hidden },
				nil,
