return function(envArgs, data: { [any]: any })
	...
	local function isEntryALogType(entryData: {
		...
		if table.find({ "Log", "log" }, entryData.type) then return true end
	end
		...
		if not listWindow._loading then
			...
			for i, optionData in providedList do
				...
				if isEntryALogType(optionData) then
					...
					local showDateAndTime = data.ShowDateAndTime
						or optionData.showDateAndTime
						or (function() -- Advanced timestamp
							local logDateTime = os.date("*t", logSentUnixOs)

							return not (
								logDateTime.year == nowDateTime.year
								and logDateTime.month == nowDateTime.month
					...
					for i, otherOptionData in providedList do
						if
							otherOptionData ~= optionData
							and logSentUnixOs == otherOptionData.sentOs
							and optionData.title == otherOptionData.title
							and optionData.desc == otherOptionData.desc
							and optionData.label == otherOptionData.label
							and not otherOptionData._loaded
						then
					...
					listWindow:createOption {
						...
						label = `{if duplicateLogCount > 0 then `({1 + duplicateLogCount}x) ` else ""}[{Parser:osDate(
							logSentUnixOs,
							nil,
							if showDateAndTime then "longdatetime" else "longtime"
						)}]: {optionData.title}`,
	...
	local function _onRefresh()
		if onRefresh then
			Promise.promisify(onRefresh):catch(
				function(err)
					UI.construct("NotificationV2", {
						title = "List Window "
							.. title
							.. " encountered an error while retrieving new list data in onRefresh process.",
		...
		if
			not variables[globalListRefreshIndex]
			and autoUpdateListData
			and variables.autoUpdatingNumberOfLists + 1 <= variables.maxAutoUpdateLists
			and not listWindow._loading
		then
			task.defer(function()
				if
					not variables[globalListRefreshIndex]
				...
				then
					...
					if not listWindow.disallowedClose then listWindow:toggleCloseDisplay(false) end

					for i, otherListWindow in listOfList do
						if
							otherListWindow ~= listWindow
							and not otherListWindow.hasManualOnRefresh
							and not otherListWindow._refreshing
							and otherListWindow.autoUpdateListData == autoUpdateListData
						then
							otherListWindow:toggleRefreshDisplay(false)
							if not otherListWindow.disallowedClose then otherListWindow:toggleCloseDisplay(false) end
						end
					end
					...
					Promise.promisify(Network.get, Network, "GetList", autoUpdateListData, unpack(autoUpdateListArgs))()
						:andThen(function(listLogsData)
							if not listLogsData or type(listLogsData) ~= "table" then
								service.tweenCreate(
									listWindow.topOptions_Refresh._object,
						...
						:andThen(function()
							for i, otherListWindow in listOfList do
								...
								then
									...
									if not listWindow.disallowedClose then listWindow:toggleCloseDisplay(true) end
								end
							end
						end)
						:catch(
							function(err)
								UI.construct("NotificationV2", {
									title = "List Window "
										.. title
										.. " encountered an error while retrieving new list data in live update process.",
									description = `Report this error to the game developer or Essential maintenance team\n`
										.. `<font color='#e84646'>{tostring(err)}</font>`,
	...
	local function onDestroy()
		...
		if _loopIndex then service.stopLoop(_loopIndex) end
		...
		if windowIndex then table.remove(listOfList, windowIndex) end
	end
	...
	if not listWindow.heartbeatEvent then
		...
		heartbeatEvent:connect(function()
			...
			if listWindow.windowModifier.resizeData and listWindow.windowModifier.resizeData._resizeState then
				return
			end
	...
	local initialSetup
	initialSetup = Promise.promisify(function()
		if dataList then
	...
	listWindow.windowFocused:connect(function()
		...
		for i, otherListWindow in listOfList do
			if
				otherListWindow ~= listWindow
				and otherListWindow.showState
				and (otherListWindow.fullscreenMode and not otherListWindow.minimizeState)
			then
	...
	listWindow.windowFocusLost:connect(function() listWindow.containerData._object.DisplayOrder = 200 end)
