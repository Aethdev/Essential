return function(envArgs, data)
	...
	if responseType == "vote" then
		...
		elseif type(voteCheck) == "table" then
			...
			if checkType == "Session" then
				local sessionId, submitResultsId, checkResultsId =
					voteCheck.sessionId, voteCheck.submitResultsId, voteCheck.checkResultsId
				warn("Results id:", checkResultsId)

				multiChoice.submitted:connectOnce(
				...
				multiChoice.voteCheck = function(selectedOpts)
					...
					local ee =
						client.Network:get("ManageSession", sessionId, "RunCommand", checkResultsId, selectedOpts)
					warn("vote checK:", ee)
					return ee
				end
		...
		if voteCheck then multiChoice.voteCheck = voteCheck end
	else
	...
	elseif not data.noYield then
		return client.Signal:waitOnSingleEvents(
			{ multiChoice.submitted, multiChoice.hiding },
			nil,
			(respTime > 0 and respTime) or nil
		)
	end
end
