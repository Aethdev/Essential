return function(envArgs, data)
	...
	local refreshB, refSpinner
	local currentListTab
	local pageDebounce

	local Trim = function(str) return str:match "^%s*(.-)%s*$" end

	function getPage(tab, pageNum)
		if not PagesEnabled then return tab end
		...
		for i = startPos, endPos do
			if tab[i] ~= nil then table.insert(pageList, tab[i]) end
		end
	...
	function getListTab(Tab)
		...
		if Stacking then
			...
			for ind, ent in next, oldNewTab do
				ent.Text = Trim(ent.Text)
				ent.Desc = Trim(ent.Desc)
				if not lastTab then
					lastTab = ent
		...
		for i, v in next, newTab do
			...
			if v.Duplicates then v.Text = "(x" .. v.Duplicates .. ") " .. v.Text end

			if v.Time then v.Text = "[" .. v.Time .. "] " .. v.Text end
		end
	...
	function doSearch(tab, text)
		...
		for i, v in next, tab do
			if
				text == ""
				or (type(v) == "string" and v:lower():find(text))
				or (
					type(v) == "table"
					and ((v.Text and tostring(v.Text):lower():find(text)) or (v.Filter and v.Filter:lower():find(text)))
				)
			then
	...
	window = client.UI.construct("ADONIS_WINDOW", {
		Name = "List",
		Title = Title,
		Size = Size or { 225, 200 },
		MinSize = { 150, 100 },
		OnRefresh = Update and function()
			Tab = client.Network:get("GetList", UpdateArg)
			if Tab and type(Tab) == "table" then
				refreshB.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				genList(Tab)
				return true
			else
				refreshB.BackgroundColor3 = Color3.fromRGB(255, 69, 69)
			end
		end,
	...
	refreshB = window.Dragger.Refresh
	refSpinner = refreshB.Spinner
		...
		OnClick = function()
			if not pageDebounce then
				...
				if currentListTab then
					...
					if PageCounter > 1 then lastPageButton.Visible = true end

					if PageCounter == maxPages then nextPageButton.Visible = false end
		...
		OnClick = function()
			if not pageDebounce then
				...
				if currentListTab then
					...
					if PageCounter == 1 then lastPageButton.Visible = false end
	...
	search.FocusLost:connect(function(enter) genList(Tab) end)
