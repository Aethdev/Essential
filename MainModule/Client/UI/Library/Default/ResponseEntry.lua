return function(envArgs, data)
	...
	if data.allowRemoteClose then
		...
		registUI.destroyed:connectOnce(function() responseEntry:hide() end)
	end

	if data.submitSessionId and data.submitEntryId then
		responseEntry.canSubmitInput = true
		local event
		event = responseEntry.submitted:connect(function()
			client.Network:fire(
				"ManageSession",
				data.submitSessionId,
				"RunCommand",
				data.submitEntryId,
				responseEntry.currentInput
			)
	...
	if data.time then task.delay(data.time, function()
		responseEntry:submit()
		responseEntry:hide()
	end) end

	if
		responseEntry.canSubmitInputOnReturn
		and not (responseEntry.canSubmitInput or responseEntry.canSubmitInputOnClose)
	then
		responseEntry.canSubmitInput = true
