return function(envArgs, data)
	...
	function container:add(element)
		...
		element._handlerShownEvent = element.shown:connect(function() self:stopInactivityCheck() end)
		...
		if not self._autoUpdateCheck then self:startHeartbeatEvent() end
	...
	function container:startHeartbeatEvent()
		...
		onHeartbeat:connect(function()
			for i, element in self._subscribedElements do
				if element.active and element.showState then element:updateBodyDisplay() end
			end
		end)
	...
	function container:startInactivityCheck()
		...
		self._inactivityCheck =
			Promise.delay(5):andThenCall(container.hide, container):andThenCall(container.stopHeartbeatEvent, container)
	...
	function container:sort()
		...
		table.sort(self._subscribedElements, function(elementA_newest, elementB_oldest)
			...
			return (
				elementA_newest.handlerAddedOn
				and elementA_newest.showStartedOn == elementB_oldest.showStartedOn
				and elementA_newest.handlerAddedOn < elementB_oldest.handlerAddedOn
			)
				or (
					(elementA_newest.showStartedOn or elementA_newest.handlerAddedOn)
					< (elementB_oldest.showStartedOn or elementB_oldest.handlerAddedOn)
				)
		end)
