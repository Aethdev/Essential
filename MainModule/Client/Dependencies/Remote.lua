	...
	client.Remote = {
		...
		Commands = {
			...
			MakeUI = {
				...
				Function = function(args)
					return client.UI.construct(args[1], args[2])
				end;
			};
				...
				Function = function(args)
					...
					if #guis > 0 then
						for i, guiData: {[any]: any} in pairs(guis) do
							if guiData.allowRemoteClose then
								guiData:destroy()
							end
						end
					end
			...
			SetCore = {
				...
				Function = function(args)
					return service.StarterGui:SetCore(args[1], args[2])
				end;
			};
			...
			SetCoreGuiEnabled = {
				...
				Function = function(args)
					service.StarterGui:SetCoreGuiEnabled(args[1], args[2])
				end;
			};
			...
			PlaySound = {
				...
				Function = function(args)
					client.Utility:playSound(args[1], args[2], args[3], args[4], args[5])
				end;
			};
			...
			FaintScreen_Add = {
				...
				Function = function(args)
					...
					service.TweenService:Create(
						colorCorrect,
						TweenInfo.new(args[1] or 0.4, Enum.EasingStyle.Quint),
						{
							Brightness = -0.5;
							Contrast = 1;
						}
						...
						if playerGui then
							...
							playerGui.ChildAdded:connect(function(child)
								...
								if child:IsA"ScreenGui" then
									...
									if guiData and guiData.ignoreAloneState then
										return
									end
								end
			...
			bindKeybinds = {
				...
				Function = function(args)
					client.Utility:makeKeybinds(unpack(args))
				end;
			};
			...
			focusCameraOnPart = {
				...
				Function = function(args)
					local camera = workspace.CurrentCamera or service.New("Camera", {
			...
			unfocusCamera = {
				...
				Function = function(args)
					...
					camera.CameraType = Enum.CameraType.Custom
					camera.CameraSubject = (service.player and service.player.Character and service.player.Character:FindFirstChildOfClass"Humanoid")
			...
			UnTrackPlayer = {
				...
				Function = function(args)
					...
					if type(PlayerUserId) ~= "number" then
						return
					end
		...
		makeSession = function(sessionId, subNetwork)
			...
			if inSession then
				...
				function sessionData:getEvent(eventIdOrName, override)
					...
					if updateCache then
						if eventInfo then
							eventInfo.lastUpdated = os.clock()
						end
						
						local newEventInfo = sessionNetwork:get("ManageSession", sessionId, "GetEvent", eventId or eventIdOrName)
				...
				function sessionData:checkEvent(eventIdOrName, override)
					...
					if updateInfo then
						eventInfo = self:getEvent(eventId or eventIdOrName, override)
					end
				...
				function sessionData:connectEvent(eventIdOrName, ...)
					...
					if not existingEvent or existingEvent.noTable then
						...
						local disconnectId = sessionNetwork:get("ManageSession", sessionId, "ConnectEvent", existingEvent.id, sesEventName, ...)
						...
						if subNetwork then
							sessionEvent.networkId = subNetwork.id
						end
				...
				function sessionData:disconnectEvent(eventIdOrName, disconnectId)
					...
					if not existingEvent or existingEvent.noTable then
						...
						if disconnectId then
							...
							local didDisconnect = sessionNetwork:get("ManageSession", sessionId, "DisconnectEvent", eventIdOrName, disconnectId)
				...
				function sessionData:runCommand(commandIdOrName, invoke, ...)
					...
					local existingCmd = sessionNetwork:get("ManageSession", sessionId, "GetCommand", commandId or commandIdOrName)
		...
		getServerSettings = function(settingList: {[any]: any})
			...
			if not settingList then
				...
				for i, setting in pairs(settingList) do
					local canUpdateCache = not settingsCache[setting] or os.time()-settingsCache[setting].lastUpdated >= 30
		...
		getClientSettings = function()
			return Network:get("GetClientSettings") or {}
		end;
		
		sendRemoteLog = function(logData: string|{
			title: string?, desc: string?, group: string?, richText: boolean?, data: {[any]: any}?
		})
		...
		getPlayers = function(isAdmin: boolean?)
			...
			local cacheUpdateRate = if service.MaxPlayers > 60 then #service.getPlayers()/12 else math.ceil(service.MaxPlayers/5)
			...
			local canUpdate = not variables.players[`_lastUpdated-{cacheIndex}`] or tick()-variables.players[`_lastUpdated-{cacheIndex}`] > cacheUpdateRate
