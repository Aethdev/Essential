...
function utility:checkHoldingKey(keycode, duration)
	...
	if duration > 0 then
		repeat
			...
			wait(0.1)
		until
			(os.clock()-started > duration) or status~=nil
	else
...
function utility:setupInputListener(object)
	if self.inputListeners[object] then
		...
		function inputListener:isKeyCodePressed(keyCode)
			return inputListener.pressedKeyCodes[keyCode] or false
		end

		function inputListener:isInputTypeEnabled(inputType)
			return inputListener.inputTypes[inputType] or false
		end
...
function utility:playSound(name, id, loop, pitch, volume)
	...
	local holder = (utility.soundFolder and utility.soundFolder.Parent == workspace and utility.soundFolder) or service.New("Folder", {
...
do
	...
	function utility.Keybinds:find(bindName: string)
		for i, keybindData: {_name: string} in self.registeredKeybinds do
			if keybindData._name == bindName then
				return keybindData
			end
		end
	end
	...
	function utility.Keybinds:deregister(bindName: string)
		...
		if keybind then
			...
			if keybindIndex then
				table.remove(self.registeredKeybinds, keybindIndex)
			end
			
			table.sort(self.registeredKeybinds, function(oldest, newest)
				if oldest._priority == newest._priority then
					return oldest._created > newest._created
				end
	...
	function utility.Keybinds:register(bindName: string, registerOptions: {
		...
		if not string.match(bindName, `^CommandKeybind%.(.+)$`) then
			assert(string.match(bindName, `^%w+$`)==bindName or string.match(bindName, "^%w+%.%w+$")==bindName, `Bind name is incorrectly formatted as ACB123 OR ACB123.BCD456`)
		else
			assert(registerOptions.trigger == `CommandKeybind`, `Keybind trigger must be CommandKeybind if the category is listed as CommandKeybind`)
		end
		...
		function keybindData:checkTrigger()
			...
			if firstKey and table.find(pressedKeys, firstKey) then
				...
				for i = 2, keysLen, 1 do
					...
					if highestKeyIndex+1 ~= otherKeyPressedIndex then
						return false
					end
		...
		function keybindData:startTrigger()
			...
			if self.holdDuration <= 0 then
				...
				local didPassGlobalRL = utility:checkRate(utility.Keybinds.RateLimits.Global, `Keybind-{bindName}.{self.created}`)
				if not didPassGlobalRL then
					return self
				end
			...
			self._checkPromise = Promise.delay(self.holdDuration)
				:andThen(function() --// Check rate limit
					if self._rateLimit then
						local didPassRL = utility:checkRate(self._rateLimit, `Keybind-{bindName}.{self._created}`)
						if not didPassRL then
							return Promise.reject(`RL_FAIL`)
						end
					end
					
					local didPassGlobalRL = utility:checkRate(utility.Keybinds.RateLimits.Global, `Keybind-{bindName}.{self.created}`)
					if not didPassGlobalRL then
						return Promise.reject(`GLOBAL_RL_FAIL`)
					end
				end)
				...
				:catch(function(err)
					err = tostring(err)
					if err == `RL_FAIL` or err == "GLOBAL_RL_FAIL" then
						self._event:fire(`RateLimited`, err)
					end
				end)
		...
		function keybindData:cancelTrigger()
			if self._checkPromise then
				if self._checkPromise:getStatus() == Promise.Status.Started then
					self._checkPromise:cancel()
				end
				self._checkPromise = nil
			end
		...
		if registerOptions.trigger == "Session" then
			assert(type(registerOptions.sessionId) == "string" and #registerOptions.sessionId > 0, `Session id must be a string and not empty`)
			assert(type(registerOptions.eventId) == "string" and #registerOptions.eventId > 0, `Event id must be a string and not empty`)
		...
		if registerOptions.saveId then
			...
			for i, otherKeybind in self.registeredKeybinds do
				if otherKeybind.saveId == registerOptions.saveId then
					warn(`Keybind {bindName} save Id is in conflict with {otherKeybind._name}. Keybind save id will not persist.`)
			...
			if input.KeyCode ~= Enum.KeyCode.Unknown and table.find(self._pressedKeys, input.KeyCode) then
				...
				Promise.each(self.registeredKeybinds, function(keybind)
					if table.find(keybind.keys, input.KeyCode) then
						if not keybind:checkTrigger() and keybind._checking then
							keybind:cancelTrigger()
						end
					end
				end)
			end
...
function utility:makeKeybinds(bindName, bindKeys, triggerType, arg2, arg3, arg4)
	...
	function setupData:checkTrigger()
		...
		local availableGamepadKeyCodes = if isGamepadEnabled then userInputS:GetSupportedGamepadKeyCodes(Enum.UserInputType.Gamepad1) else nil
		...
		if self.holdDuration > 0 then
			...
			until
				not triggered or (tick()-started >= self.holdDuration)
	...
	if bindName and (bindKeys and #bindKeys>0) and not bindData then
		if triggerType == "Function" then
			local bindFunc = (type(arg2)=="string" and client.Loadstring(arg2, getEnv(nil, {player = service.player;}))) or
				(type(arg2)=="function" and arg2) or nil

			if bindFunc then
				setupData.triggered:connect(function()
					if utility:checkRate(utility.Keybinds.RateLimits.Global, `function`) then
						bindFunc()
					end
				end)
			end
		...
		elseif triggerType == "PersonalKeybind" then
			setupData.triggered:connect(function()
				if not client.clientSettingsWindow and client.Settings.KeybindsEnabled and utility:checkRate(utility.Keybinds.RateLimits.TriggerCommand, `command`) then
		...
		if arg3 == "QuickAction" then
			...
			if options.Label then
				quickIcon:setLabel(options.Label)
			end

			if options.Caption then
				quickIcon:setCaption(options.Caption)
			end

			if options.Tip then
				quickIcon:setTip(options.Tip)
			end

			if options.Image then
				quickIcon:setImage(options.Image)
			end

			quickIcon.selected:Connect(function()
				if setupData.active then
					setupData.triggered:fire()
				end
				quickIcon:deselect()
			end)
		...
		for i,key in pairs(bindKeys) do
			if type(key) == "string" then
				...
				if key:sub(1,8) == "KeyCode-" then
					table.insert(setupData.keybinds, Enum.KeyCode[key:sub(9)])
				end
...
function utility:removeKeybinds(bindName: string)
	...
	if keybindData then
		if keybindData.quickIcon then
			keybindData.quickIcon:destroy()
		end
...
function utility.Notifications:setup()
	local TopbarIcon = client.UI.makeElement("TopbarIcon")
...
function utility.Notifications:create(constructData)
	return client.UI.construct(`NotificationV2`, constructData)
end
...
function utility.Tracking:trackPlayer(player: Player)
	if self._players[player.UserId] then
		...
		if existingInfo._player ~= player then
			existingInfo._player = player
		end
		
		existingInfo._events:findSignal(`Disconnect`)
			:stopRbxEvents()
			:linkRbxEvent(player.Destroying)
		
		existingInfo._events:findSignal(`Character`)
			:stopRbxEvents()
			:linkRbxEvent(player.CharacterAdded)
		
		existingInfo._events:findSignal(`Character`)
			:linkRbxEvent(service.player.CharacterAdded)
		
		if player.Character then
			existingInfo:reConstruct()
		end
	...
	function trackingInfo:reConstruct()
		...
		self._reConstructTask = Promise.promisify(function()
			targetTorso = targetCharacter:WaitForChild("HumanoidRootPart", 20) or targetCharacter:FindFirstChild("Torso")
				or targetCharacter:FindFirstChild("UpperTorso")
			selfTorso = selfCharacter:WaitForChild("HumanoidRootPart", 20) or selfCharacter:FindFirstChild("Torso")
				or selfCharacter:FindFirstChild("UpperTorso")
		end)()
			:andThen(function()
				local RankTag = client.AssetsFolder.TrackingTag:Clone()
				RankTag.Enabled = true
				RankTag.Frame.Rank.Text = if self._player.Name == self._player.DisplayName then self._player.Name
	...
	onCharacterAdded:connect(function(char)
		...
		quickIcon.selected:Connect(function()
			...
			for playerUserId, trackData in self._players do
				table.insert(listOfTrack, {
					...
					type = "Action";
					label = if trackingInfo._player.DisplayName == trackingInfo._player.Name then
						trackingInfo._player.Name else `{trackingInfo._player.DisplayName} (@{trackingInfo._player.Name})`;
					selectable = true;
...
function utility:beginFly()
	if not utility.Fly._state then
		...
		if not utility.Fly._handler then
			utility.Fly._handler = client.Signal:createHandler()
		end
		...
		local function createBodyObjects()
			...
			if not bodyGyro or bodyGyro.Parent ~= humanoidRootPart then
				if bodyGyro then
					service.Debris:AddItem(bodyGyro, 1)
				end
		...
		local function checkIdle()
			return not(
				activeDirection.Left or
				activeDirection.Right or
				activeDirection.Up or 
				activeDirection.Down or 
				activeDirection.Forward or 
		...
		addRbxConnection(inputService.InputBegan, function(input, inGameProcess)
			warn(pcall(function()
			checkInputProcess(input, inGameProcess, true)
			end))
			warn("Input started")
		end)
		...
		addRbxConnection(inputService.InputEnded, function(input, inGameProcess)
			warn(pcall(function()
			checkInputProcess(input, inGameProcess, false)
			end))
			warn("Input ended")
		end)
		...
		service.loopTask("Fly command", 0.1, function()
			...
			local currentCamera = workspace.CurrentCamera or service.New("Camera", {
			...
			if utility.Fly._state and (playerChar and humanoidRootPart) then
				...
				if checkIdle() then
					...
					if curSpeed > maxAccelerationTick then
						curSpeed = maxAccelerationTick
					end
				end
...
function utility:stopFly()
	if utility.Fly._state then
		utility.Fly._state = false
...
function utility.Init(env)
	...
	service.Players.PlayerAdded:Connect(function(player: Player)
		if utility.Tracking._players[player.UserId] then
			utility.Tracking:trackPlayer(player)
		end
	end)
end
