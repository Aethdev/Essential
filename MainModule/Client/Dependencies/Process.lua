...
return function(env)
	...
	client.Process = {
		...
		remoteCall = function(invoke, key, ...)
			...
			local ratePass,didThrottle,canThrottle,curRate,maxRate,throttleResetOs = client.Utility:checkRate(rateData, rateKey)
			...
			if ratePass then
				...
				if isKeyValidType and Network.serverToClientRemoteKey then
					local realKey = if Network:isEndToEndEncrypted() then client.HashLib.sha1(Network.serverToClientRemoteKey) else
						Network.serverToClientRemoteKey
						...
						if cmd and not cmd.Disabled then
							...
							if (not lockdown or (lockdown and cmd.Lockdown_Allowed)) then
								...
								if canUseCommand and cmdFunction then
									if rL_Enabled then
										local passCmdRateCheck,curRemoteRate,maxRemoteRate = client.Utility:checkRate(rL_Data, rateKey)
									...
									local rets = {service.trackTask("_REMCOMMAND-"..cmdName.."-Invoke:"..tostring(invoke).."-"..service.getRandom(), false, cmdFunction, {unpack(params, 2)})}

									if not rets[1] then
										warn("Remote command "..cmdName.." encountered an error while running: "..tostring(rets[2]))
									else
										if invoke then
