return function(env)
	...
	local function Init()
		...
		for i,element in pairs(elementsFolder:GetChildren()) do
			...
			if clElement and element:IsA"ModuleScript" then
				...
				if not success then
					warn("UI Element "..element.Name.." cannot be loaded due to an unexpected error: "..tostring(elementData))
				else
					...
					if type(elementData.Init) == "function" then
						...
						local initSuccess,initErrorResult,errorTrace = service.trackTask("_UI_ELEMENTLOAD-"..elementName, false, setfenv(elementInit, scriptEnv), envArgs)

						if not initSuccess then
							warn("UI Element "..elementName.." init encountered an error: "..tostring(initErrorResult), errorTrace)
						end
	...
	client.UI = {
		...
		construct = function(guiName, data, dataTheme)
			...
			if themeFolder then
				local guiObject = themeFolder:FindFirstChild(guiName) or UI.findConstructByAlias(guiName, dataTheme) or UI.findUIWithCategory(guiName, dataTheme)
				...
				if guiObject then
					...
					set_maxPlacement = (set_maxPlacement and math.clamp(math.floor(math.abs(set_maxPlacement)), 0, math.huge)) or nil
					...
					if not set_disabled and canConstruct then
						...
						if guiClone:IsA"ModuleScript" then
							...
							if guiRetType == "table" then
								...
								if type(loadFunc) == "function" then
									...
									local loadRets = {service.trackTask("_UI_GUI-"..guiName, false, setfenv(loadFunc, scriptEnv), envArgs, data)}
							...
							elseif guiRetType == "function" then
								local envArgs = getEnv(nil, {script = guiClone;})
								local scriptEnv = getEnv("EnvLocals", {script = guiClone;})
								local loadRets = {service.trackTask("_UI_GUI-"..guiName, false, setfenv(guiReturn, scriptEnv), envArgs, data)}

								if not loadRets[1] then
									warn("UI "..guiName.." encountered an error: "..tostring(loadRets[2]), loadRets[3])
								else
									return unpack(loadRets, 2)
								end
							else
						...
						elseif guiClone:IsA"ScreenGui" or guiClone:IsA"GuiObject" then
							...
							if (source and source:IsA"ModuleScript") then
								local container = (guiClone:IsA"ScreenGui" and UI.makeElement("Container",{object = guiClone;})) or UI.makeElement("Container")
								...
								container._object.Name = (setting_guiNameType=="random" and `ESSCU-{service.getRandom()}`) or
									(setting_guiNameType=="chosen" and setting_guiChosenName) or (setting_guiNameType=="hide" and "\0")
								...
								if sourceRetType == "function" then
									...
									local loadRets = {xpcall(setfenv(sourceRet, scriptEnv), function(errMsg) warn(errMsg, debug.traceback(nil, 2)) end, envArgs, data, guiData)}
								...
								elseif sourceRetType == "table" then
									...
									if type(loadFunc) == "function" then
										local envArgs = getEnv(nil, {script = source;})
										local scriptEnv = getEnv("EnvLocals", {script = source;})
										local loadRets = {service.trackTask("_UI_GUI-"..guiName, false, setfenv(loadFunc, scriptEnv), envArgs, data, guiData)}
			...
			local guiData; guiData = {
				...
				ready = function()
					if not guiData._ready then
						...
						if guiData._container then
							guiData._container:show()
						end

						if guiData.Source then
							guiData.Source.Parent = nil
						end
				...
				unRegister = function()
					...
					if uiIndex then
						table.remove(createdUIs, uiIndex)
					end
				end,
		...
		getGuis = function(name)
			...
			for i,guiData in pairs(createdUIs) do
				if not name or guiData.name == name then
					table.insert(results, guiData)
				end
			end
		...
		getGuiData = function(object)
			...
			if unWrapObj then
				for i, guiData in pairs(createdUIs) do
					if guiData.object and rawequal(guiData.object, unWrapObj) then
						return guiData
					end
				end
			end
		...
		protectUI = function(guiObj, checkedProps, killIfTampered)
			...
			proxyMeta.__index = function(_, prop)
				...
				if propType == "function" then
					return setfenv(function(ignore, ...)
						return chosenProp(guiObj, ...)
					end, getfenv(2))
				else
					return chosenProp
				end
				...
				if guiData.active and type(changeType) == "string" then
					if checkedProps[changeType] then
						...
						if oldProp ~= newProp then
							if rawequal(changeType, "Text") and isATextBox and guiObj.TextEditable and guiObj:IsFocused() then
								return	
							end
		...
		modifyObject = function(objData, guiData)
			...
			if modifiedObjects[object] then
				return modifiedObjects[object]
			end
			...
			if objData.modifier then
				return objData.modifier
			end
			...
			function modifyData:setHover(text: string, duration: number, enterCheck: number, textColor: Color3, frameColor: Color3)
				if self.active and guiData._object then
					if not guiData.itemContainer or guiData.itemContainer.Parent ~= guiData._object then
						self:removeHover()

						if guiData.itemContainer then
							service.Delete(guiData.itemContainer)
						end
					...
					if not self.hoverData then
						...
						hoverData.mouseEntered = modifyData:bindEvent(object.MouseEnter, function()
							if hoverData.active and not (hoverData._hovering or hoverData._mouseIn) and #hoverData.text>0 then
								...
								if hoverData.enterCheck > 0 then
									...
									repeat
										if not hoverData._mouseIn then
											return
										end
										wait()
									until
										not hoverData.active or (os.clock()-stCheck >= enterCheck)
								end
								...
								repeat
									...
									local textSize = service.TextService:GetTextSize(hoverData.text, hoverLabel.TextSize, hoverLabel.Font, Vector2.new(400, hoverLabel.AbsoluteSize.Y*3))
								...
								until
								not hoverData.active or not hoverData._hovering or (hoverDuration>0 and os.clock()-stHovering > hoverDuration)
						...
						hoverData.mouseLeave = modifyData:bindEvent(object.MouseLeave, function()
							if hoverData.active then
								if hoverData._hovering then
									hoverData._hovering = false
								end

								if hoverData._mouseIn then
									hoverData._mouseIn = false
								end
							end
						end)
			...
			function modifyData:enableResize(customData: {
				...
				if self.active then
					if not self.resizeContainer or self.resizeContainer.Parent ~= object then
						...
						if self.resizeContainer then
							service.Delete(self.resizeContainer)
						end
					...
					if not self.resizeData then
						...
						if not guiData._container then
							guiData.doScale = false
						end
						...
						function resizeData:update()
							if self._resizeState then
								...
								if startedAt == "bottomRight" then
									local maximumSizeX = math.clamp(roundNumber(objectAbsSize.X+deltaMouse.X, guiIncrement), self.minimumSize.X, self.maximumSize.X)
									local maximumSizeY = math.clamp(roundNumber(objectAbsSize.Y+deltaMouse.Y, guiIncrement), self.minimumSize.Y, self.maximumSize.Y)
								...
								elseif startedAt == "topRight" then
									...
									local maximumSizeX = math.clamp(roundNumber(realSizeX, guiIncrement), self.minimumSize.X, self.maximumSize.X)
									local maximumSizeY = math.clamp(roundNumber(realSizeY, guiIncrement), self.minimumSize.Y, self.maximumSize.Y)
									
									newAbsSize = Vector2.new(maximumSizeX, maximumSizeY)
									newObjPos = Vector2.new(
										objectAbsPos.X,
										math.clamp(roundNumber(objectAbsPos.Y + deltaMouse.Y, guiIncrement), bottomPosY-self.maximumSize.Y, bottomPosY-self.minimumSize.Y)
									)
								...
								elseif startedAt == "bottomMiddle" then
									local realSizeY = objectAbsSize.Y+deltaMouse.Y
									local maximumSizeY = math.clamp(roundNumber(realSizeY, guiIncrement), self.minimumSize.Y, self.maximumSize.X)
								...
								elseif startedAt == "leftSide" then
									...
									newAbsSize = Vector2.new(maximumSizeX, objectAbsSize.Y)
									newObjPos = Vector2.new(
										math.clamp(roundNumber(objectAbsPos.X + deltaMouse.X, guiIncrement), rightPosX-self.maximumSize.X, rightPosX-self.minimumSize.X),
										objectAbsPos.Y
									)
								elseif startedAt == "topLeft" then
									...
									newAbsSize = Vector2.new(maximumSizeX, maximumSizeY)
									newObjPos = Vector2.new(
										math.clamp(roundNumber(objectAbsPos.X + deltaMouse.X, guiIncrement), bottomRightPosX-self.maximumSize.X, bottomRightPosX-self.minimumSize.X),
										math.clamp(roundNumber(objectAbsPos.Y + deltaMouse.Y, guiIncrement), bottomLeftPosY-self.maximumSize.Y, bottomLeftPosY-self.minimumSize.Y)
									)
								elseif startedAt == "bottomLeft" then
									...
									newAbsSize = Vector2.new(maximumSizeX, maximumSizeY)
									newObjPos = Vector2.new(
										math.clamp(roundNumber(objectAbsPos.X + deltaMouse.X, guiIncrement), topRightPosX-self.maximumSize.X, topRightPosX-self.minimumSize.X),
										objectAbsPos.Y
									)
								end
								...
								if self.doTween then
									service.tweenCreate(
										...
										{
											Position = (newObjPos and UDim2.fromOffset(newObjPos.X, newObjPos.Y)) or nil;
											Size = (newAbsSize and UDim2.fromOffset(newAbsSize.X, newAbsSize.Y)) or nil;
										}
									):Play()
								else
									if newAbsSize then
										object.Size = UDim2.fromOffset(newAbsSize.X, newAbsSize.Y)
									end

									if newObjPos then
										object.Position = UDim2.fromOffset(newObjPos.X, newObjPos.Y)
									end
								end
							end
						end
						...
						for button, clickName in pairs(clickArrays) do
							...
							buttonInputBegan:connect(function(input)
								local canResize = (not modifyData.dragData or modifyData.dragData and not modifyData.dragData._started) and
									(resizeData.active and not resizeData._resizeState)
								
								if canResize and checkValidInput(input) and resizeData.active and not resizeData._resizeState then
						...
						userInputChanged:connect(function(input)
							if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
								if resizeData.active and resizeData._resizeState then
									resizeData:update()
								end
							end
						end)
			...
			function modifyData:enableDrag(noTween: boolean, walls: boolean)
				...
				if not self.dragData then
					...
					local function checkBoundaries(newPosition)
						...
						if not guiObject then
							...
							local topLeftPosition = Vector2.new(
								math.clamp(newPosition.X, 0+(anchorPoint.X*absoluteSize.X), (anchorPoint.X*absoluteSize.X)+(guiAbsoluteSize.X-absoluteSize.X)),
								math.clamp(newPosition.Y, 0+(anchorPoint.Y*absoluteSize.Y), (anchorPoint.Y*absoluteSize.Y)+(guiAbsoluteSize.Y-absoluteSize.Y))
							)
							...
							local bottomgRightPosition = Vector2.new(bottomLeftPosition.X+absoluteSize.X, bottomLeftPosition.Y)

							local topLeftPos1Pass = (topLeftPosition.X>=0 and topLeftPosition.Y>=0)
							local topLeftPos2Pass = (topLeftPosition.X<=guiAbsoluteSize.X and topLeftPosition.Y<=guiAbsoluteSize.Y)
							local topRightPos1Pass = (topRightPosition.X>=0 and topRightPosition.Y>=0)
							local topRightPos2Pass = (topRightPosition.X<=guiAbsoluteSize.X and topRightPosition.Y<=guiAbsoluteSize.Y)
							local bottomLeftPos1Pass = (bottomLeftPosition.X>=0 and bottomLeftPosition.Y>=0)
							local bottomLeftPos2Pass = (bottomLeftPosition.X<=guiAbsoluteSize.X and bottomLeftPosition.Y<=guiAbsoluteSize.Y)
							local bottomRightPos1Pass = (bottomgRightPosition.X>=0 and bottomgRightPosition.Y>=0)
							local bottomRightPos2Pass = (bottomgRightPosition.X<=guiAbsoluteSize.X and bottomgRightPosition.Y<=guiAbsoluteSize.Y)

							local isWithinBoundary = topLeftPos1Pass and topLeftPos2Pass and
								topRightPos1Pass and topRightPos2Pass and
								bottomLeftPos1Pass and bottomLeftPos2Pass and
								bottomRightPos1Pass and bottomRightPos2Pass
							...
							task.spawn(function()
								...
								repeat
									...
									local newPosition = UDim2.new(
										objStartPosition.X.Scale --[[0]], roundNumber(objStartPosition.X.Offset + offset.X, dragIncrement),
										objStartPosition.Y.Scale --[[0]], roundNumber(objStartPosition.Y.Offset + offset.Y, dragIncrement)
									)
									...
									service.RunService.Heartbeat:Wait()
								until
								not(self.active and self._started)
							end)
						end
					end
					...
					function dragData:destroy()
						if self.active then
							...
							for i, conName in pairs(connections) do
								...
								if rbxConnection.Connected then
					...
					dragData.mouseEntered = modifyData:bindEvent(dragObject.MouseEnter, function()
						if dragData.active and not dragData._mouseIn then
							dragData._mouseIn = true
						end
					end)

					dragData.mouseLeave = modifyData:bindEvent(dragObject.MouseLeave, function()
						if dragData.active and dragData._mouseIn then
							dragData._mouseIn = false
						end
					end)

					dragData.inputBegan = modifyData:bindEvent(dragObject.InputBegan, function(input)
						if dragData.active and dragData._mouseIn then
							local canDrag = (not modifyData.resizeData or modifyData.resizeData and not modifyData.resizeData._resizeState) and
								(dragData.active and not dragData._started)
							
							if canDrag and input.UserInputType == Enum.UserInputType.MouseButton1 then
								dragData:start()
							end
							
							if canDrag and input.UserInputType == Enum.UserInputType.Touch then
								dragData:start()
							end
						end
					end)
			...
			function modifyData:createTextHistory(initialText: string?)
				if object:IsA"TextBox" and not self.textHistory then
					...
					function textHistory:add(tab)
						if not self.locked then
							if tab.inputText then
								...
								if currentRecord and currentRecord.inputText and currentRecord.inputText == tab.inputText then
									return
								end
							end
							...
							if self.beforeSaving and type(self.beforeSaving) == "function" then
								...
								if not success then
									warn(`UI Modifier TextHistory BeforeSaving function for {object:GetFullName()} encountered an error: {allowSaving}\n{errTrace}`)
									return
								end
					...
					function textHistory:undoLatest()
						...
						if latestRecord then
							...
							if self.currentRecord ~= latestRecord then
								...
								local indexOfCurrentRecord = if currentRecord then table.find(self.records, self.currentRecord) else nil
					...
					function textHistory:changeTextToCurrentChange()
						...
						if historyData then
							object.Text = historyData.inputText
						end
						self.locked = false
					end
					...
					textHistory:add({
						...
						cursorPosition = if self.textHistory.preferLensPosition then utf8.len(object.Text)+1 else object.CursorPosition;
					...
					textChanged:Connect(function()
						if textHistory.active and object:IsFocused() and object.TextEditable then
							textHistory:add({
								...
								cursorPosition = if textHistory.preferLensPosition then utf8.len(object.Text)+1 else object.CursorPosition;
					...
					focusLost:Connect(function(didHitEnter, inputThatTrigger)
						if textHistory.clearAfterFocusLost then
							textHistory:clear()
						end
					end)
					...
					userInputBegan:Connect(function(input, gameProcessed)
						if textHistory.active and not textHistory.locked and not textHistory._ongoingChangeState and object.TextEditable and object:IsDescendantOf(service.playerGui) and object:IsFocused() and gameProcessed then
								...
								if (userInputService:IsKeyDown(Enum.KeyCode.LeftControl) or userInputService:IsKeyDown(Enum.KeyCode.RightControl)) then
									if input.KeyCode == Enum.KeyCode.Z then
										...
										repeat
											...
											--warn("")
										until
											not (userInputService:IsKeyDown(Enum.KeyCode.LeftShift) or userInputService:IsKeyDown(Enum.KeyCode.RightShift)) or
												not (userInputService:IsKeyDown(Enum.KeyCode.LeftControl) or userInputService:IsKeyDown(Enum.KeyCode.RightControl)) or
												not userInputService:IsKeyDown(Enum.KeyCode.Z) or
												not object:IsFocused() or textHistory.currentIndex >= #textHistory.records
									...
									elseif input.KeyCode == Enum.KeyCode.Y then
										...
										until
											not (userInputService:IsKeyDown(Enum.KeyCode.LeftShift) or userInputService:IsKeyDown(Enum.KeyCode.RightShift)) or
												not (userInputService:IsKeyDown(Enum.KeyCode.LeftControl) or userInputService:IsKeyDown(Enum.KeyCode.RightControl)) or
												not userInputService:IsKeyDown(Enum.KeyCode.Y) or
												not object:IsFocused() or textHistory.currentIndex <= 1
										textHistory._ongoingChangeState = false
									end
								end
							end
				...
				for i, constructor in themeFolder:GetChildren() do
					...
					if type(constructorAliases) == "string" then
						...
						for i, constructorAlias in listOfAliases do
							if constructorAlias == aliasName then
								return constructor
							end
						end
					end
			...
			if category and constructNameOrAlias then
				...
				for i, constructor in categoryFolder:GetChildren() do
					...
					if type(constructorAliases) == "string" then
						...
						for i, constructorAlias in listOfAliases do
							if constructorAlias == constructNameOrAlias then
								return constructor
							end
						end
					end
