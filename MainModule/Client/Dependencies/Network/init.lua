...
return function(env)
	...
	local function Init() Utility = client.Utility end
	...
	local function secureNetwork()
		...
		local remoteE_Func = function(...)
			if rawequal(savedRemoteEv, remoteEv) and networkReady then
				...
				local rets = {
					service.trackTask(
						"CLIENTNETWORK-NONINVOKED-" .. service.getRandom(),
						false,
						client.Process.remoteCall,
						false,
						unpack(remoteArguments)
					),
				}

				if not rets[1] then warn("Client network encountered an error: " .. tostring(rets[2])) end
			end
		end
		...
		local remoteF_Func = function(...)
			if rawequal(savedRemoteFunc, remoteFunc) and networkReady then
				...
				local rets = {
					service.trackTask(
						"CLIENTNETWORK-INVOKED-" .. service.getRandom(),
						false,
						client.Process.remoteCall,
						true,
						unpack(remoteArguments)
		...
		service.startLoop(remoteCheckName, true, 0.5, function()
	...
	function network:search()
		...
		if networkReady then networkAbandoned:fire(true) end
		...
		local function canTrust(remoteObject, lookingForRemoteFunc)
			if typeof(remoteObject) == "Instance" then
				...
				if isNetwork then
					...
					if isEvent and not lookingForRemoteFunc then return true end

					if isFunc and lookingForRemoteFunc then return true end
				end
			end
		...
		repeat
			...
			if success then
				secureNetwork()
				if network._verified then networkReady = true end
				network:verify()
	...
	function network:trustCheck()
						...
						if type(typ) == "string" then
							if typ == "TrustCheck" then
								...
								if type(arg) == "table" and not found then
									...
									if
										type(data_remoteEv) == "string"
										and type(data_remoteFunc) == "string"
										and type(data_serverRemoteKey) == "string"
										and type(data_subNetworkTrustCheckerPublicId) == "string"
										and type(data_endToEndEncryption) == "boolean"
										and type(data_remoteRateLimit) == "table"
										and type(data_remoteRateLimit.Reset) == "number"
										and data_remoteRateLimit.Reset > 0
										and type(data_remoteRateLimit.Rates) == "number"
										and data_remoteRateLimit.Rates > 0
									then
		...
		until found or (os.time() - started) > 300
	end
	...
	function network:verify()
		...
		if checkIfNetworksAccessible() then networkReady = true end
	end
	...
	function network:customGet(idleTimeout, ...)
		...
		if not remoteRatePass then
			warn(
				`CLIENT NETWORK HAS REACHED THE RATE LIMIT OF THE MAIN NETWORK. WAITING {remoteRateResetOs - tick()} SECONDS TO GET DATA.`
			)
	...
	function network:fire(...)
		...
		if not remoteRatePass then
			warn(
				`CLIENT NETWORK HAS REACHED THE RATE LIMIT OF THE MAIN NETWORK. WAITING {remoteRateResetOs - tick()} SECONDS TO FIRE DATA.`
			)
			wait(remoteRateResetOs - tick())
			return network:fire(...)
		end
	...
	subNetworkConnectQueue.processFunc = function(ind, que, subNetworkName, subNetworkId)
		...
		if not (subNetworkEntryKey and subNetworkTrustKey) then
			...
			if not checkRegistry then
				...
				subNetwork.serverProcessRateLimit = networkInfo.processRateLimit
					or {
						Rates = 300,
						Reset = 30,
					}
				...
				subNetwork.networkCommands = {
					FirePlayerEvent = {
						...
						Function = function(args)
							...
							if
								existingEvent
								and existingEvent.remoteFire
								and existingEvent.networkId
								and existingEvent.networkId == subNetworkId
							then
					...
					TestFunction = {
						...
						Function = function(args) return "test" end,
					},
				}
				...
				function subNetwork:fire(...)
					if self.active then
						if not (networkReady and self.ready) then
							repeat
								wait(1)
							until not self.active or (networkReady and self.ready)
						...
						if self.serverProcessRLEnabled then
							...
							if not remoteRatePass then
								warn(
									`CLIENT SUB NETWORK {self.id} HAS REACHED THE RATE LIMIT OF THE MAIN NETWORK. WAITING {remoteRateResetOs - tick()} SECONDS TO CUSTOM GET DATA.`
								)
								wait(remoteRateResetOs - tick())
								return self:fire(...)
							end
				...
				function subNetwork:get(...) return self:customGet(600, ...) end
				...
				function subNetwork:_get(idleTimeout: number, ...)
					if self.active then
						...
						trackTask("_SUBNETWORK-" .. tostring(subNetworkId) .. "-GET", true, function()
							...
							local ranRets = {
								service.nonThreadTask(
									service.invokeServer,
									subNetwork._invokeObject,
									if isETEE then hashLib.sha1(subNetwork.entryKey) else subNetwork.entryKey,
									unpack(remoteArguments)
								),
							}
				...
				function subNetwork:customGet(idleTimeout, ...)
					if self.active then
						...
						if self.serverProcessRLEnabled then
							...
							if not remoteRatePass then
								warn(
									`CLIENT SUB NETWORK {self.id} HAS REACHED THE RATE LIMIT OF THE MAIN NETWORK. WAITING {remoteRateResetOs - tick()} SECONDS TO GET DATA.`
								)
								wait(remoteRateResetOs - tick())
				...
				function subNetwork:yield(...) self:get(...) end
				...
				function subNetwork:killSecurity()
					if self._security2 then
						...
						if self._security2[2].Disconnect then self._security2[2]:Disconnect() end
				...
				function subNetwork:makeSecurity()
					...
					local function makeSecureEvent(rbxEvent, func)
						...
						eventData = {
							...
							connection = rbxEvent:Connect(function(...)
								if eventData.active then return func(...) end
							end),
						}
					...
					local function makeSecureFunc(rbxObject, func)
						...
						funcData = {
							...
							rbxFunction = function(...)
								if funcData.active then return func(...) end
							end,
						}

						rbxObject.OnClientInvoke = funcData.rbxFunction

						service.startLoop(secureLoopId, true, 0.5, function()
					...
					table.insert(
						security1,
						makeSecureEvent(subNetRemoteEv:GetPropertyChangedSignal "Parent", function()
							local newParent = subNetRemoteEv.Parent

							if rawequal(newParent, directory) then self:killSecurity() end
						end)
					)

					table.insert(
						security1,
						makeSecureEvent(subNetRemoteFunc:GetPropertyChangedSignal "Parent", function()
							local newParent = subNetRemoteEv.Parent
					...
					local remoteFuncSecure = makeSecureFunc(subNetRemoteFunc, function(accKey, ...)
						if subNetwork.active then
							local expectedAccessKey = if subNetwork.endToEndEncrypted
								then hashLib.sha1(subNetwork.accessKey)
							...
							local taskRets = {
								service.trackTask(
									"SUBNETWORK_" .. subNetworkId .. "_INVOKE",
									false,
									subNetwork.processRemoteCall,
									true,
									unpack(remoteArguments)
								),
							}
					...
					table.insert(
						security2,
						makeSecureEvent(subNetRemoteEv.OnClientEvent, function(accKey, ...)
							if subNetwork.active then
								local expectedAccessKey = if subNetwork.endToEndEncrypted
									then hashLib.sha1(subNetwork.accessKey)
									else subNetwork.accessKey
								...
								local taskRets = {
									service.trackTask(
										"SUBNETWORK_" .. subNetworkId .. "_NONINVOKE",
										false,
										subNetwork.processRemoteCall,
										false,
										unpack(remoteArguments)
					...
					if self.active and not self.trustChecked then
							...
							for i, ins in pairs(directory:GetChildren()) do
								...
								if anInstance and ins:IsA "RemoteFunction" then
									...
									if attributeId and #attributeId > 0 then
										if attributeId == subNetTrustChecker_Id then return ins end
									end
								end
							end
						end
						...
						local function search()
							...
							local function killEvents()
								if #searchEvents > 0 then
									for i, event in pairs(searchEvents) do
										if type(event) == "userdata" and event.Connected then event:Disconnect() end
									end
								end
							end
							...
							if trustCheckerObj then
								local success, didRegister, data_remoteEv, data_remoteFunc, data_disconnectId =
									service.nonThreadTask(
										trustCheckerObj.InvokeServer,
										trustCheckerObj,
										"TrustCheck",
										subNetworkId,
										subNetworkTrustKey
								...
								if success and subNetwork.active then
									if
										type(data_remoteEv) == "string"
										and type(data_remoteFunc) == "string"
										and type(data_disconnectId) == "string"
									then
										found = true
										searchStat = "done"
										--killEvents()
						...
						until found or not self.active or (os.time() - started) > 300

						if found == nil then found = false end
					end
				end
				...
				function subNetwork:search()
					if self.active and self.trustChecked then
						...
						until not self.active or (os.time() - started) > 60

						if not subNetwork._verified and not self.ready then self.connectError:fire() end
					end
				end
				...
				function subNetwork:verify()
					...
					if type(didVerify) ~= "boolean" or not didVerify then
						warn(
							`SUB NETWORK {subNetwork.id} FAILED TO VERIFY. VERIFY STATUS IS EITHER FALSE OR UNKNOWN TYPE.`
						)
						return
					elseif isETEE and hashedOldKey ~= hashedOldEntryKey then
						warn(
							`SUB NETWORK {subNetwork.id} FAILED TO VERIFY. HASHED OLD ENTRY KEY WAS NOT VERIFIED PROPERLY WITH THE RETURNED HASH.`
						)
						return
					elseif isETEE and (type(newPersonalKey) ~= "string" or #newPersonalKey == 0) then
						warn(
							`SUB NETWORK {subNetwork.id} FAILED TO VERIFY. NEW PERSONAL KEY WAS NOT RETURNED PROPERLY.`
						)
						return
					end
				...
				function subNetwork:isAccessible()
					return (self._invokeObject and self._invokeObject.Parent == directory)
						and (self._fireObject and self._fireObject.Parent == directory)
				end

				function subNetwork:canDisconnect(): boolean return self:get "CanDisconnect" or false end
				...
				function subNetwork:disconnect(sendRequest: boolean?): boolean
					if sendRequest then
						...
						if not canDisconnect then
							...
							if not rawequal(didSuccess, true) then return false end
						end
					end
	...
	function network:connectSubNetwork(subNetworkName, subNetworkId)
		...
		if existingNet then return existingNet end
		...
		if not subNetworkId then
			...
			local processCon
			processCon = subNetworkQue.processed:connect(function(didRun, ...)
				if didRun then
	...
	function network:isReady() return networkReady end

	function network:isEndToEndEncrypted() return endToEndEncryption end
