...
return function(envArgs)
	...
	local cliSettings = client.Network:get("GetSettings", {"helpEnabled", "helpIconImage", "consoleEnabled", "consoleIconImage", "hotkeys", "allowClientGlobal"}) or {}
	...
	quickAction = client.UI.makeElement("TopbarIcon")
		:setEnabled(false)
	...
	quickAction.selected:Connect(function()
		if aboutMeIcon.isSelected then
	...
	if cliSettings.helpEnabled then
		local helpIcon = client.UI.makeElement("TopbarIcon")
			:setEnabled(false)
			:setName(service.getRandom())
			:setLabel("Get help")

		if #(cliSettings.helpIconImage or "") > 0 then
			helpIcon:setImage(cliSettings.helpIconImage)
		end
	...
	if cliSettings.consoleEnabled then
		...
		local consoleIcon = client.UI.makeElement("TopbarIcon")
			:setEnabled(false)
			:setName(service.getRandom())
		...
		client.consoleOpened:connect(function()
			quickAction:deselect()
		end)

		local consoleBoxIcon = client.UI.makeElement("TopbarIcon")
			:setEnabled(false)
			:setName(service.getRandom())
			:setLabel("CommandX")
	...
	local cliSettingsIcon = client.UI.makeElement("TopbarIcon")
		:setEnabled(false)
		:setName(service.getRandom())
		:setLabel("Client settings")
	...
	aboutMeIcon = client.UI.makeElement("TopbarIcon")
	...
	local aboutUserIdIcon = client.UI.makeElement("TopbarIcon")
	...
	local aboutDisplayNameIcon = client.UI.makeElement("TopbarIcon")
		:joinDropdown(aboutMeIcon)
		:modifyTheme(topbarIconTheme.Dropdown)
		:setName(service.getRandom())
	...
	local aboutPingIcon = client.UI.makeElement("TopbarIcon")
		:joinDropdown(aboutMeIcon)
		:modifyTheme(topbarIconTheme.Dropdown)
		:setName(service.getRandom())
		:setLabel("Ping: 0ms")
	...
	local aboutFPSIcon = client.UI.makeElement("TopbarIcon")
		:joinDropdown(aboutMeIcon)
		:modifyTheme(topbarIconTheme.Dropdown)
		:setName(service.getRandom())
	...
	local aboutRejoinServerIcon = client.UI.makeElement("TopbarIcon")
		:joinDropdown(aboutMeIcon)
		:modifyTheme(topbarIconTheme.Dropdown)
		:setName(service.getRandom())
	...
	local shortcutTopbar = client.UI.makeElement("TopbarIcon")
	...
	for buttonName, cmdLine in pairs(createdCmdShortcuts) do
		...
		local buttonTopbarIcon = client.UI.makeElement("TopbarIcon")
	...
	client.Policies:connectPolicyChangeEvent("SHORTCUTS_ALLOWED", function(state: boolean, enforce_type: string)
		shortcutTopbar:setEnabled(state)
	end)
	...
	service.loopTask("Check FPS", 5, function()
		if aboutFPSIcon.enabled then
			aboutFPSIcon:setLabel("FPS: "..tostring(workspace:GetRealPhysicsFPS()))
		end
	end)
	...
	quickAction.selected:Connect(function()
		client.Events.quickActionShown:fire()
	end)

	quickAction.deselected:Connect(function()
		client.Events.quickActionHidden:fire()
	end)
	...
	message(`Game settings may require minimized player view. Players with MINIMIZED_TOPBARICONS policy enabled cannot view EC Topbar utility icons.`)
	
	client.Policies:connectPolicyChangeEvent(`MINIMIZED_TOPBARICONS`, function(policyValue: boolean, enforcementType: string)
		if policyValue then
