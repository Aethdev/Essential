return function(envArgs)
	...
	local function addClientLog(res: string) Remote.sendRemoteLog(`SecureProtect :: {res}`) end
	...
	if ExploitTests.TamperTextboxCheck then
		...
		local function initiateTamperTest()
			...
			local function killEvents()
				...
				if next(_testEvents) then killEvents() end
			end
			...
			local function protectElement(object, focusProps)
				...
				objectChanged:connect(function(property)
					...
					warn(
						`Object {tostring(object:GetFullName())} was modified while the test was not finished`,
						property
					)
				end)
		...
		task.delay(120, function()
			if not _testFailed then service.loopTask(tamperLoopName, 120, initiateTamperTest) end
		end)
	end
end
